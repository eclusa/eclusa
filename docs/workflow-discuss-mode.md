...
Eclusa's discuss-phase has two modes for gathering implementation context before planning.
...
eclusa-tools config-set workflow.discuss_mode assumptions

# Switch back to interview mode
eclusa-tools config-set workflow.discuss_mode discuss
```

The setting is per-project (stored in `.eclusa/config.json`).
