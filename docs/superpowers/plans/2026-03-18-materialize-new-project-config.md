...
**Goal:** When `/eclusa:new-project` creates `.eclusa/config.json`, the file contains all effective defaults — not just the 6 user-chosen keys — so developers can see every setting without reading source code.
...
**Tech Stack:** Node.js/CommonJS, existing eclusa-tools CLI, `node:test` for tests.
...
- `git.phase_branch_template: "eclusa/phase-{phase}-{slug}"`
- `git.milestone_branch_template: "eclusa/{milestone}-{slug}"`
...
| File | Action | Purpose |
|------|--------|---------|
| `eclusa/bin/lib/config.cjs` | Modify | Add `buildNewProjectConfig()` + `cmdConfigNewProject()` |
| `eclusa/bin/eclusa-tools.cjs` | Modify | Register `config-new-project` case + update usage string |
| `eclusa/workflows/new-project.md` | Modify | Steps 2a + 5: replace inline JSON write with CLI call |
...
- Modify: `eclusa/bin/lib/config.cjs`
...
describe('config-new-project command', () => {
  ...
  test('works with empty choices — all defaults materialized', () => {
    const result = runEclusaTools(['config-new-project', '{}'], tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
In `eclusa/bin/lib/config.cjs`, add the following after the `validateKnownConfigKeyPath` function (around line 35) and before `ensureConfigFile`:
 ...
 *   2. User-level defaults from ~/.eclusa/defaults.json (if present)
...
function buildNewProjectConfig(cwd, userChoices) {
  ...
  const braveKeyFile = path.join(homedir, '.eclusa', 'brave_api_key');
  const hasBraveSearch = !!(process.env.BRAVE_API_KEY || fs.existsSync(braveKeyFile));

  // Load user-level defaults from ~/.eclusa/defaults.json if available
  const globalDefaultsPath = path.join(homedir, '.eclusa', 'defaults.json');
  let userDefaults = {};
  try {
...
/**
 * Command: create a fully-materialized .eclusa/config.json for a new project.
 *
 * Accepts user-chosen settings as a JSON string (the keys the user explicitly
 * configured during /eclusa:new-project). All remaining keys are filled from
 * hardcoded defaults and optional ~/.eclusa/defaults.json.
 *
 * Idempotent: if config.json already exists, returns { created: false }.
 */
function cmdConfigNewProject(cwd, choicesJson, raw) {
  const configPath = path.join(cwd, '.eclusa', 'config.json');
  const planningDir = path.join(cwd, '.eclusa');
  ...
  try {
    ...
    error('Failed to create .eclusa directory: ' + err.message);
  }
  ...
  try {
    ...
    output({ created: true, path: '.eclusa/config.json' }, raw, 'created');
  } catch (err) {
...
```bash
cd /Users/diego/Dev/eclusa
git add eclusa/bin/lib/config.cjs tests/config.test.cjs
git commit -m "feat: add config-new-project command for full config materialization"
```

---

## Task 2: Register `config-new-project` in eclusa-tools.cjs

**Files:**

- Modify: `eclusa/bin/eclusa-tools.cjs`

- [ ] **Step 2.1: Add the case to the switch in eclusa-tools.cjs**
...
```bash
cd /Users/diego/Dev/eclusa
node eclusa/bin/eclusa-tools.cjs config-new-project '{"mode":"interactive","granularity":"standard"}' --cwd /tmp/eclusa-smoke-$(date +%s)
```

Expected: outputs `{"created":true,"path":".eclusa/config.json"}` (or similar).

Clean up: `rm -rf /tmp/eclusa-smoke-*`
...
```bash
cd /Users/diego/Dev/eclusa
git add eclusa/bin/eclusa-tools.cjs
git commit -m "feat: register config-new-project in eclusa-tools CLI router"
```
...
- Modify: `eclusa/workflows/new-project.md`
...
```markdown
Create `.eclusa/config.json` with mode set to "yolo":
...
```markdown
Create `.eclusa/config.json` using the CLI (fills in all defaults automatically):

```bash
mkdir -p .eclusa
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" config-new-project "$(cat <<'CHOICES'
{
  "mode": "yolo",
...
```markdown
Create `.eclusa/config.json` with all settings:
...
```bash
cd /Users/diego/Dev/eclusa
grep -n "config-new-project\|config\.json\|CHOICES" eclusa/workflows/new-project.md
```
...
```bash
cd /Users/diego/Dev/eclusa
git add eclusa/workflows/new-project.md
...
node /Users/diego/Dev/eclusa/eclusa/bin/eclusa-tools.cjs init new-project --cwd "$TMP"

# Step 5 simulation: create full config
node /Users/diego/Dev/eclusa/eclusa/bin/eclusa-tools.cjs config-new-project '{
  "mode": "interactive",
  "granularity": "standard",
  "parallelization": true,
  "commit_docs": true,
...
node /Users/diego/Dev/eclusa/eclusa/bin/eclusa-tools.cjs config-new-project "$CHOICES" --cwd "$TMP"
FIRST=$(cat "$TMP/.eclusa/config.json")

# Second call should be no-op
node /Users/diego/Dev/eclusa/eclusa/bin/eclusa-tools.cjs config-new-project "$CHOICES" --cwd "$TMP"
SECOND=$(cat "$TMP/.eclusa/config.json")
...
node /Users/diego/Dev/eclusa/eclusa/bin/eclusa-tools.cjs config-get workflow.plan_check --cwd "$TMP"
# Expected: false

node /Users/diego/Dev/eclusa/eclusa/bin/eclusa-tools.cjs config-get git.branching_strategy --cwd "$TMP"
# Expected: "none"
...
**Problem:**
`/eclusa:new-project` creates `.eclusa/config.json` with only the 6 keys
...
2. **Implicit expansion**: the first time `/eclusa:settings` or `config-set`
   writes to the config, those keys still aren't added. The config only
   reflects a fraction of the effective configuration.

**Solution:**
Add `config-new-project` CLI command to `eclusa-tools.cjs`. The command:
...
A developer opening `.eclusa/config.json` for the first time can now see
`git.branching_strategy: "none"` and immediately understand that branching
is available and configurable, without reading the Eclusa source.
```
