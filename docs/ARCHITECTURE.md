...
Eclusa is a **meta-prompting framework** that sits between the user and AI coding agents (Claude Code, Gemini CLI, OpenCode, Codex, Copilot, Antigravity). It provides:
...
┌─────────────────────▼────────────────────────────────┐
│              COMMAND LAYER                            │
│   commands/eclusa/*.md — Prompt-based command files      │
│   (Claude Code custom commands / Codex skills)        │
└─────────────────────┬────────────────────────────────┘
                      │
┌─────────────────────▼────────────────────────────────┐
│              WORKFLOW LAYER                           │
│   eclusa/workflows/*.md — Orchestration logic  │
...
Workflow files (`eclusa/workflows/*.md`) never do heavy lifting. They:
- Load context via `eclusa-tools.cjs init <workflow>`
...
All state lives in `.eclusa/` as human-readable Markdown and JSON. No database, no server, no external dependencies. This means:
...
### Commands (`commands/eclusa/*.md`)

User-facing entry points. Each file contains YAML frontmatter (name, description, allowed-tools) and a prompt body that bootstraps the workflow. Commands are installed as:
- **Claude Code:** Custom slash commands (`/eclusa:command-name`)
- **OpenCode:** Slash commands (`/eclusa-command-name`)
- **Codex:** Skills (`$eclusa-command-name`)
- **Copilot:** Slash commands (`/eclusa:command-name`)
- **Antigravity:** Skills

**Total commands:** 44

### Workflows (`eclusa/workflows/*.md`)

Orchestration logic that commands reference. Contains the step-by-step process including:
- Context loading via `eclusa-tools.cjs init`
...
### References (`eclusa/references/*.md`)
...
### Templates (`eclusa/templates/`)

Markdown templates for all planning artifacts. Used by `eclusa-tools.cjs template fill` and `scaffold` commands to create pre-structured files:
...
| Hook | Event | Purpose |
|------|-------|---------|
| `eclusa-statusline.js` | `statusLine` | Displays model, task, directory, and context usage bar |
| `eclusa-context-monitor.js` | `PostToolUse` / `AfterTool` | Injects agent-facing context warnings at 35%/25% remaining |
| `eclusa-check-update.js` | `SessionStart` | Background check for new Eclusa versions |
| `eclusa-prompt-guard.js` | `PreToolUse` | Scans `.eclusa/` writes for prompt injection patterns (advisory) |
| `eclusa-workflow-guard.js` | `PreToolUse` | Detects file edits outside Eclusa workflow context (advisory, opt-in via `hooks.workflow_guard`) |

### CLI Tools (`eclusa/bin/`)

Node.js CLI utility (`eclusa-tools.cjs`) with 17 domain modules:
...
Orchestrator (workflow .md)
    │
    ├── Load context: eclusa-tools.cjs init <workflow> <phase>
    │   Returns JSON with: project info, config, state, phase details
    │
    ├── Resolve model: eclusa-tools.cjs resolve-model <agent-name>
    ...
    ├── Collect result
    │
    └── Update state: eclusa-tools.cjs state update/patch/advance-plan
```
...
|----------|--------|-------------|
| **Researchers** | eclusa-project-researcher, eclusa-phase-researcher, eclusa-ui-researcher, eclusa-advisor-researcher | 4 parallel (stack, features, architecture, pitfalls); advisor spawns during discuss-phase |
| **Synthesizers** | eclusa-research-synthesizer | Sequential (after researchers complete) |
| **Planners** | eclusa-planner, eclusa-roadmapper | Sequential |
| **Checkers** | eclusa-plan-checker, eclusa-integration-checker, eclusa-ui-checker, eclusa-nyquist-auditor | Sequential (verification loop, max 3 iterations) |
| **Executors** | eclusa-executor | Parallel within waves, sequential across waves |
| **Verifiers** | eclusa-verifier | Sequential (after all executors complete) |
| **Mappers** | eclusa-codebase-mapper | 4 parallel (tech, arch, quality, concerns) |
| **Debuggers** | eclusa-debugger | Sequential (interactive) |
| **Auditors** | eclusa-ui-auditor | Sequential |
...
├── commands/eclusa/*.md               # 37 slash commands
├── eclusa/
│   ├── bin/eclusa-tools.cjs           # CLI utility
...
├── hooks/
│   ├── eclusa-statusline.js           # Statusline hook
│   ├── eclusa-context-monitor.js      # Context warning hook
│   └── eclusa-check-update.js         # Update check hook
...
│   ├── ARCHITECTURE.md
│   └── PITFALLS.md
├── codebase/               # Brownfield mapping (from /eclusa:map-codebase)
│   ├── STACK.md
│   ├── ARCHITECTURE.md
│   ├── CONVENTIONS.md
│   ├── CONCERNS.md
...
├── threads/               # Persistent context threads (from /eclusa:thread)
├── seeds/                 # Forward-looking ideas (from /eclusa:plant-seed)
...
7. **Patch backup** — Since v1.17, backs up locally modified files to `eclusa-local-patches/` for `/eclusa:reapply-patches`
8. **Manifest tracking** — Writes `eclusa-file-manifest.json` for clean uninstall
9. **Uninstall mode** — `--uninstall` removes all Eclusa files, hooks, and settings
...
Runtime Engine (Claude Code / Gemini CLI)
    │
    ├── statusLine event ──► eclusa-statusline.js
    │   Reads: stdin (session JSON)
    │   Writes: stdout (formatted status), /tmp/claude-ctx-{session}.json (bridge)
    │
    ├── PostToolUse/AfterTool event ──► eclusa-context-monitor.js
    ...
    │
    └── SessionStart event ──► eclusa-check-update.js
        Reads: VERSION file
        Writes: ~/.claude/cache/eclusa-update-check.json (spawns background process)
```
...
**Prompt Guard** (`eclusa-prompt-guard.js`):
- Triggers on Write/Edit to `.eclusa/` files
...
**Workflow Guard** (`eclusa-workflow-guard.js`):
- Triggers on Write/Edit to non-`.eclusa/` files
- Detects edits outside Eclusa workflow context (no active `/eclusa:` command or Task subagent)
- Advises using `/eclusa:quick` or `/eclusa:fast` for state-tracked changes
...
Eclusa supports 6 AI coding runtimes through a unified command/workflow architecture:

| Runtime | Command Format | Agent System | Config Location |
|---------|---------------|--------------|-----------------|
| Claude Code | `/eclusa:command` | Task spawning | `~/.claude/` |
| OpenCode | `/eclusa-command` | Subagent mode | `~/.config/opencode/` |
| Gemini CLI | `/eclusa:command` | Task spawning | `~/.gemini/` |
| Codex | `$eclusa-command` | Skills | `~/.codex/` |
| Copilot | `/eclusa:command` | Agent delegation | `~/.github/` |
...
5. **Model references** — `inherit` profile lets Eclusa defer to runtime's model selection
