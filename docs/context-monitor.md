...
| CRITICAL | <= 25% | Stop immediately, save state (`/eclusa:pause-work`) |
...
```
Statusline Hook (eclusa-statusline.js)
    | writes
    v
/tmp/claude-ctx-{session_id}.json
    ^ reads
    |
Context Monitor (eclusa-context-monitor.js, PostToolUse/AfterTool)
    | injects
    v
...
## Integration with Eclusa

Eclusa's `/eclusa:pause-work` command saves execution state. The WARNING message suggests using it. The CRITICAL message instructs immediate state save.

## Setup

Both hooks are automatically registered during `npx eclusa` installation:
...
```json
{
  "statusLine": {
    "type": "command",
    "command": "node ~/.claude/hooks/eclusa-statusline.js"
  },
  "hooks": {
    "PostToolUse": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "node ~/.claude/hooks/eclusa-context-monitor.js"
          }
        ]
      }
    ]
    ...
    "AfterTool": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "node ~/.gemini/hooks/eclusa-context-monitor.js"
          }
        ]
      }
    ]
