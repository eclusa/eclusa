# Referência de Recursos do Eclusa

Visão em Português dos recursos centrais do Eclusa.  
...
- **UAT guiado** via `/eclusa:verify-work`
- **Suporte brownfield** com `/eclusa:map-codebase`
...
- **`/eclusa:ui-phase`**: contrato visual antes da execução
- **`/eclusa:ui-review`**: auditoria visual em 6 pilares
...
- **Diagnóstico forense** com `/eclusa:forensics`
- **Relatório de sessão** com `/eclusa:session-report`
...
| Cenário | Comandos |
|--------|----------|
| Projeto novo | `/eclusa:new-project` -> `/eclusa:discuss-phase` -> `/eclusa:plan-phase` -> `/eclusa:execute-phase` |
| Correção rápida | `/eclusa:quick` |
| Código existente | `/eclusa:map-codebase` -> `/eclusa:new-project` |
| Fechamento de release | `/eclusa:audit-milestone` -> `/eclusa:complete-milestone` |
