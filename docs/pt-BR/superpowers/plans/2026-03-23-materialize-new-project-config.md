...
Este plano formaliza a materialização explícita da configuração do projeto durante `/eclusa:new-project`, garantindo que escolhas feitas na inicialização sejam persistidas de forma determinística em `.eclusa/config.json`.
...
- escrita idempotente de `.eclusa/config.json`
...
- após `/eclusa:new-project`, `config.json` reflete as escolhas feitas
