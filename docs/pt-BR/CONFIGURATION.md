# Referência de Configuração do Eclusa

Configurações do projeto ficam em `.eclusa/config.json`.  
...
|------|--------|-----------|
| `planning.commit_docs` | `true` | Comitar `.eclusa/` no git |
...
| `git.phase_branch_template` | string | `eclusa/phase-{phase}-{slug}` | Nome para branch por fase |
| `git.milestone_branch_template` | string | `eclusa/{milestone}-{slug}` | Nome para branch de milestone |
| `git.quick_branch_template` | string ou `null` | `null` | Branch opcional para `/eclusa:quick` |
