...
| `git.phase_branch_template` | `"eclusa/phase-{phase}-{slug}"` | 阶段策略的分支模板 |
| `git.milestone_branch_template` | `"eclusa/{milestone}-{slug}"` | 里程碑策略的分支模板 |
</config_schema>
...
**当 `commit_docs: false`：**
- 跳过 `.eclusa/` 文件的所有 `git add`/`git commit`
- 用户必须将 `.eclusa/` 添加到 `.gitignore`
- 适用于：OSS 贡献、客户项目、保持规划私有

**使用 eclusa-tools.cjs（推荐）：**
...
**自动检测：** 如果 `.eclusa/` 被 gitignore，无论 config.json 如何，`commit_docs` 自动为 `false`。这防止用户在 `.gitignore` 中有 `.eclusa/` 时出现 git 错误。
...
- 直接路径搜索有效：`rg "pattern" .eclusa/` 找到文件
- 广泛搜索跳过 gitignored：`rg "pattern"` 跳过 `.eclusa/`

**当 `search_gitignored: true`:**
- 在应该包含 `.eclusa/` 的广泛 rg 搜索中添加 `--no-ignore`
- 仅在搜索整个仓库并期望 `.eclusa/` 匹配时需要

**注意：** 大多数 Eclusa 操作使用直接文件读取或显式路径，无论 gitignore 状态如何都有效。
...
3. **已存在的跟踪文件：** 如果 `.eclusa/` 之前被跟踪：
   ```bash
   git rm -r --cached .eclusa/
   git commit -m "chore: stop tracking planning docs"
   ```

4. **分支合并：** 当使用 `branching_strategy: phase` 或 `milestone` 时，`complete-milestone` 工作流在 `commit_docs: false` 时自动从暂存区移除 `.eclusa/` 文件，然后才进行合并提交。
...
- 分支名来自 `phase_branch_template`（如 `eclusa/phase-03-authentication`）
- 所有计划提交到该分支
- 阶段完成后用户手动合并分支
...
- 分支名来自 `milestone_branch_template`（如 `eclusa/v1.0-mvp`）
- 里程碑中所有阶段提交到同一分支
