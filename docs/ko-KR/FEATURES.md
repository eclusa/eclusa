...
**명령어:** `/eclusa:new-project [--auto @file.md]`
...
- REQ-INIT-06: `.eclusa/PROJECT.md`가 이미 존재하는 경우 재초기화를 방지해야 합니다.
...
- 기존 코드베이스 컨텍스트(`/eclusa:map-codebase`에서)가 있으면 로드됩니다.

---

### 2. Phase Discussion

**명령어:** `/eclusa:discuss-phase [N] [--auto] [--batch]`
...
**명령어:** `/eclusa:plan-phase [N] [--auto] [--skip-research] [--skip-verify]`
...
- REQ-PLAN-07: 프론트엔드 페이즈가 감지되고 UI-SPEC.md가 없는 경우 `/eclusa:ui-phase` 실행을 촉구해야 합니다(UI 안전 게이트).
...
**명령어:** `/eclusa:audit-milestone`, `/eclusa:complete-milestone`, `/eclusa:new-milestone [name]`
...
**갭 해소.** `/eclusa:plan-milestone-gaps`는 감사에서 식별된 갭을 해소하는 페이즈를 생성합니다.
...
**명령어:** `/eclusa:add-phase`, `/eclusa:insert-phase [N]`, `/eclusa:remove-phase [N]`
...
**명령어:** `/eclusa:quick [--full] [--discuss] [--research]`

**목적:** Eclusa 보증을 제공하지만 더 빠른 경로로 임시 작업을 실행합니다.
...
- REQ-QUICK-08: 빠른 작업을 `.eclusa/quick/YYMMDD-xxx-slug/`에 추적해야 합니다.
...
**목적:** 자유형 텍스트를 분석하고 적절한 Eclusa 명령어로 라우팅합니다.

**요구사항.**
- REQ-DO-01: 자연어 입력에서 사용자 의도를 파악해야 합니다.
- REQ-DO-02: 의도를 가장 적합한 Eclusa 명령어에 매핑해야 합니다.
...
- REQ-NEXT-04: 프로젝트가 없으면 `/eclusa:new-project`를 제안해야 합니다.
- REQ-NEXT-05: 모든 페이즈가 완료되면 `/eclusa:complete-milestone`을 제안해야 합니다.

**상태 감지 로직.**
| 상태 | 액션 |
|-------|--------|
| `.eclusa/` 디렉토리 없음 | `/eclusa:new-project` 제안 |
| 페이즈에 CONTEXT.md 없음 | `/eclusa:discuss-phase` 실행 |
| 페이즈에 PLAN.md 파일 없음 | `/eclusa:plan-phase` 실행 |
| 계획 있지만 SUMMARY.md 없음 | `/eclusa:execute-phase` 실행 |
| 실행되었지만 VERIFICATION.md 없음 | `/eclusa:verify-work` 실행 |
| 모든 페이즈 완료 | `/eclusa:complete-milestone` 제안 |
...
- REQ-POSTVER-03: `/eclusa:verify-work`가 처리할 문제를 기록해야 합니다.
...
**명령어:** `/eclusa:health [--repair]`

**목적:** `.eclusa/` 디렉토리 무결성을 검증하고 문제를 자동으로 복구합니다.
...
**실행 시점.** `/eclusa:execute-phase` 중 검증자 단계 전에 자동으로 실행됩니다.
...
**실행 시점.** `/eclusa:plan-phase`의 계획 검사기 루프 후 자동으로 실행됩니다.
...
- REQ-CTX-06: 컨텍스트 모니터는 Eclusa 활성 프로젝트와 비활성 프로젝트를 구분해야 합니다.
...
**명령어:** `/eclusa:pause-work`, `/eclusa:resume-work`, `/eclusa:progress`
...
**생성 산출물.** `.eclusa/reports/SESSION_REPORT.md`
...
**명령어:** `/eclusa:set-profile <quality|balanced|budget|inherit>`
...
**명령어:** `/eclusa:map-codebase [area]`

**목적:** 새 프로젝트를 시작하기 전에 기존 코드베이스를 분석하여 Eclusa가 무엇이 존재하는지 이해하도록 합니다.

**요구사항.**
- REQ-MAP-01: 각 분석 영역에 대한 병렬 매퍼 에이전트를 생성해야 합니다.
- REQ-MAP-02: `.eclusa/codebase/`에 구조화된 문서를 생성해야 합니다.
- REQ-MAP-03: 기술 스택, 아키텍처 패턴, 코딩 규범, 문제점을 감지해야 합니다.
- REQ-MAP-04: 이후 `/eclusa:new-project`는 코드베이스 매핑을 로드하고 추가하는 내용에 대한 질문에 집중해야 합니다.
...
**명령어:** `/eclusa:debug [description]`

**목적:** 컨텍스트 초기화 전반에 걸쳐 영구적인 상태로 체계적인 디버깅을 수행합니다.

**요구사항.**
- REQ-DEBUG-01: `.eclusa/debug/`에 디버그 세션 파일을 작성해야 합니다.
...
- REQ-DEBUG-05: 해결된 세션은 `.eclusa/debug/knowledge-base.md`에 추가되어야 합니다.
...
**명령어:** `/eclusa:add-todo [desc]`, `/eclusa:check-todos`

**목적:** 세션 중 나중에 처리할 아이디어와 작업을 캡처합니다.

**요구사항.**
- REQ-TODO-01: 현재 대화 컨텍스트에서 할 일을 캡처해야 합니다.
- REQ-TODO-02: 할 일은 `.eclusa/todos/pending/`에 저장되어야 합니다.
- REQ-TODO-03: 완료된 할 일은 `.eclusa/todos/done/`으로 이동해야 합니다.
...
**목적:** 변경 로그 미리보기와 함께 Eclusa를 최신 버전으로 업데이트합니다.
...
- REQ-UPDATE-04: 로컬에서 수정된 파일을 `eclusa-local-patches/`에 백업해야 합니다.
- REQ-UPDATE-05: `/eclusa:reapply-patches`는 업데이트 후 로컬 수정사항을 복원해야 합니다.
...
- REQ-SETTINGS-02: `.eclusa/config.json`을 업데이트해야 합니다.
- REQ-SETTINGS-03: 전역 기본값으로 저장하는 것을 지원해야 합니다(`~/.eclusa/defaults.json`).
...
| `planning.commit_docs` | boolean | `true` | `.eclusa/` 파일을 git에 커밋 |
...
- REQ-GIT-07: `.eclusa/` 파일에 대한 `commit_docs` 설정을 준수해야 합니다.
- REQ-GIT-08: `.gitignore`에서 `.eclusa/`을 자동 감지하고 커밋을 건너뛰어야 합니다.
...
**목적:** 6가지 다른 AI 코딩 에이전트 런타임에서 Eclusa를 실행합니다.
...
- REQ-RUNTIME-05: 제거는 다른 구성에 영향을 주지 않고 모든 Eclusa 파일을 깔끔하게 제거해야 합니다.
...
**명령어:** `/eclusa:profile-user [--questionnaire] [--refresh]`
...
- `/eclusa:dev-preferences` 명령어 — 모든 세션에서 선호도 로드
...
- `eclusa-user-profiler` 에이전트 — 세션 데이터에서 행동 분석
...
모든 `/eclusa:progress` 호출은 현재 마일스톤의 모든 페이즈에서 미결 항목(pending, skipped, blocked, human_needed)을 스캔합니다. 실행 가능한 링크가 포함된 비차단 경고 섹션을 표시합니다.
...
**요구사항.**
- REQ-DEBT-01: `/eclusa:progress`에서 모든 이전 페이즈의 미결 UAT/검증 항목을 표시해야 합니다.
...
- REQ-DEBT-06: `/eclusa:audit-uat`는 모든 페이즈를 스캔하고 테스트 가능성별로 항목을 분류하며 사람 테스트 계획을 생성해야 합니다.
...
**명령어:** `/eclusa:fast [task description]`
...
- REQ-FAST-03: 상태 일관성을 위해 `.eclusa/quick/`에 작업을 추적해야 합니다.
- REQ-FAST-04: 연구, 다단계 계획, 또는 검증이 필요한 작업에는 사용해서는 안 됩니다.

**`/eclusa:quick`과 비교하여 사용 시점.**
- `/eclusa:fast` — 2분 이내에 실행 가능한 한 문장 작업(오타, 구성 변경, 작은 추가)
- `/eclusa:quick` — 연구, 다단계 계획, 또는 검증이 필요한 모든 것

---

### 42. Cross-AI Peer Review

**명령어:** `/eclusa:review --phase N [--gemini] [--claude] [--codex] [--all]`
...
- REQ-REVIEW-05: 검토는 `/eclusa:plan-phase --reviews`가 사용할 수 있어야 합니다.
...
**명령어:** `/eclusa:add-backlog <description>`, `/eclusa:review-backlog`, `/eclusa:plant-seed <idea>`
...
- REQ-BACKLOG-02: `/eclusa:discuss-phase`와 `/eclusa:plan-phase`가 작동할 수 있도록 페이즈 디렉토리를 즉시 생성해야 합니다.
- REQ-BACKLOG-03: `/eclusa:review-backlog`는 항목별로 승격, 유지, 제거 액션을 지원해야 합니다.
- REQ-BACKLOG-04: 승격된 항목은 활성 마일스톤 순서로 번호가 다시 매겨져야 합니다.
- REQ-SEED-01: 시드는 표시 조건에 대한 전체 이유와 시기를 캡처해야 합니다.
- REQ-SEED-02: `/eclusa:new-milestone`은 시드를 스캔하고 일치하는 항목을 표시해야 합니다.

**생성 산출물.**
| 산출물 | 설명 |
|----------|-------------|
| `.eclusa/phases/999.x-slug/` | 백로그 항목 디렉토리 |
| `.eclusa/seeds/SEED-NNN-slug.md` | 트리거 조건이 있는 시드 |

---

### 44. Persistent Context Threads

**명령어:** `/eclusa:thread [name | description]`

**목적:** 여러 세션에 걸쳐 있지만 특정 페이즈에 속하지 않는 작업을 위한 가벼운 교차 세션 지식 저장소입니다. `/eclusa:pause-work`보다 더 가볍습니다 — 페이즈 상태나 계획 컨텍스트가 없습니다.

**요구사항.**
- REQ-THREAD-01: 생성, 나열, 재개 모드를 지원해야 합니다.
- REQ-THREAD-02: 스레드는 `.eclusa/threads/`에 마크다운 파일로 저장되어야 합니다.
...
**생성 산출물.** `.eclusa/threads/{slug}.md` — 지속적 컨텍스트 스레드

---

### 45. PR Branch Filtering

**명령어:** `/eclusa:pr-branch [target branch]`

**목적:** `.eclusa/` 커밋을 필터링하여 풀 리퀘스트에 적합한 깔끔한 브랜치를 생성합니다. 검토자는 Eclusa 계획 산출물이 아닌 코드 변경사항만 봅니다.

**요구사항.**
- REQ-PRBRANCH-01: `.eclusa/` 파일만 수정하는 커밋을 식별해야 합니다.
...
**목적:** Eclusa 계획 산출물에 대한 심층 방어 보안. Eclusa가 LLM 시스템 프롬프트가 되는 마크다운 파일을 생성하기 때문에, 이 파일로 흘러드는 사용자 제어 텍스트는 잠재적인 간접 프롬프트 주입 벡터입니다.
...
**2. 프롬프트 주입 가드 훅** (`eclusa-prompt-guard.js`)
`.eclusa/`을 대상으로 하는 Write/Edit 호출에서 주입 패턴을 스캔하는 PreToolUse 훅입니다. 정당한 작업을 차단하지 않고 인식을 위해 감지를 기록하는 권고 전용입니다.

**3. 워크플로우 가드 훅** (`eclusa-workflow-guard.js`)
Claude가 Eclusa 워크플로우 컨텍스트 밖에서 파일 편집을 시도하는 것을 감지하는 PreToolUse 훅입니다. 직접 편집 대신 `/eclusa:quick` 또는 `/eclusa:fast` 사용을 권고합니다. `hooks.workflow_guard`로 구성 가능합니다(기본값: false).
...
**목적:** 모노저장소 및 멀티 저장소 설정에 대한 자동 감지 및 프로젝트 루트 해석. `.eclusa/`이 저장소 경계를 넘어 해석되어야 하는 워크스페이스를 지원합니다.
...
**목적:** `/eclusa:discuss-phase` 중에 `DISCUSSION-LOG.md`를 자동 생성하여 논의 중에 내려진 결정의 전체 감사 추적을 제공합니다.
...
**명령어:** `/eclusa:forensics [description]`

**목적:** 실패하거나 막힌 Eclusa 워크플로우의 사후 조사.

**요구사항.**
- REQ-FORENSICS-01: git 히스토리에서 이상(막힌 루프, 긴 간격, 반복된 커밋)을 분석해야 합니다.
- REQ-FORENSICS-02: 산출물 무결성을 확인해야 합니다(완료된 페이즈에 예상 파일이 있는지).
- REQ-FORENSICS-03: `.eclusa/forensics/`에 저장된 마크다운 보고서를 생성해야 합니다.
...
| 산출물 | 설명 |
|----------|-------------|
| `.eclusa/forensics/report-{timestamp}.md` | 사후 조사 보고서 |

**프로세스.**
1. **스캔** — git 히스토리에서 이상 분석: 막힌 루프, 커밋 사이의 긴 간격, 반복된 동일 커밋
2. **무결성 확인** — 완료된 페이즈에 예상 산출물 파일이 있는지 확인
3. **보고** — `.eclusa/forensics/`에 저장된 조사 결과가 담긴 마크다운 보고서 생성
...
**명령어:** `/eclusa:milestone-summary [version]`
...
**명령어:** `/eclusa:workstreams`

**목적:** 마일스톤의 다른 영역에서 동시 작업을 위한 병렬 워크스트림.

**요구사항.**
- REQ-WS-01: 별도의 `.eclusa/workstreams/{name}/` 디렉토리에 워크스트림 상태를 격리해야 합니다.
...
| 산출물 | 설명 |
|----------|-------------|
| `.eclusa/workstreams/{name}/` | 격리된 워크스트림 디렉토리 구조 |

**프로세스.**
1. **생성** — 격리된 `.eclusa/workstreams/{name}/` 디렉토리로 명명된 워크스트림 초기화
2. **전환** — 이후 Eclusa 명령어를 위한 활성 워크스트림 컨텍스트 변경
...
**명령어:** `/eclusa:discuss-phase` with `workflow.discuss_mode: 'assumptions'`
...
**일부:** `/eclusa:new-project` 및 `/eclusa:progress`

**목적:** UI 중심 프로젝트를 자동으로 감지하고 `/eclusa:ui-phase` 권장사항을 표시합니다.

**요구사항.**
- REQ-UI-DETECT-01: 프로젝트 설명에서 UI 신호를 감지해야 합니다(키워드, 프레임워크 참조).
- REQ-UI-DETECT-02: 해당하는 경우 ROADMAP.md 페이즈에 `ui_hint`를 주석으로 추가해야 합니다.
- REQ-UI-DETECT-03: UI 중심 페이즈의 다음 단계에서 `/eclusa:ui-phase`를 제안해야 합니다.
- REQ-UI-DETECT-04: `/eclusa:ui-phase`를 필수로 만들어서는 안 됩니다.

**프로세스.**
1. **감지** — UI 신호(키워드, 프레임워크 참조)에 대한 프로젝트 설명 및 기술 스택 스캔
2. **주석** — ROADMAP.md의 해당 페이즈에 `ui_hint` 표시 추가
3. **표시** — UI 중심 페이즈의 다음 단계에 `/eclusa:ui-phase` 권장사항 포함
...
3. **설치** — 단일 세션에서 선택된 모든 런타임에 Eclusa 구성
