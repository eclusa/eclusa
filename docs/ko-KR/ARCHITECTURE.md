...
Eclusa는 사용자와 AI 코딩 에이전트(Claude Code, Gemini CLI, OpenCode, Codex, Copilot, Antigravity) 사이에 위치하는 **메타 프롬프팅 프레임워크**입니다. 다음을 제공합니다.
...
워크플로우 파일(`eclusa/workflows/*.md`)은 무거운 작업을 직접 수행하지 않습니다. 다음 작업만 담당합니다.
- `eclusa-tools.cjs init <workflow>`로 컨텍스트를 로드합니다
...
모든 상태는 `.eclusa/`에 사람이 읽을 수 있는 Markdown과 JSON으로 저장됩니다. 데이터베이스, 서버, 외부 의존성이 없습니다. 이를 통해 다음이 가능합니다.
...
- **Claude Code:** 커스텀 슬래시 명령어 (`/eclusa:command-name`)
- **OpenCode:** 슬래시 명령어 (`/eclusa-command-name`)
- **Codex:** Skills (`$eclusa-command-name`)
- **Copilot:** 슬래시 명령어 (`/eclusa:command-name`)
- **Antigravity:** Skills
...
- `eclusa-tools.cjs init`을 통한 컨텍스트 로드
...
모든 계획 아티팩트를 위한 Markdown 템플릿입니다. `eclusa-tools.cjs template fill`과 `scaffold` 명령어가 사전 구조화된 파일을 생성하는 데 사용합니다.
...
| 훅 | 이벤트 | 목적 |
|------|-------|---------|
| `eclusa-statusline.js` | `statusLine` | 모델, 작업, 디렉터리, 컨텍스트 사용 바 표시 |
| `eclusa-context-monitor.js` | `PostToolUse` / `AfterTool` | 잔여 35%/25% 시점에 에이전트 대면 컨텍스트 경고 주입 |
| `eclusa-check-update.js` | `SessionStart` | 새 Eclusa 버전을 백그라운드에서 확인 |
| `eclusa-prompt-guard.js` | `PreToolUse` | `.eclusa/` 쓰기 작업에서 프롬프트 인젝션 패턴 스캔 (권고용) |
| `eclusa-workflow-guard.js` | `PreToolUse` | Eclusa 워크플로우 컨텍스트 외부의 파일 편집 감지 (권고용, `hooks.workflow_guard`로 활성화) |

### CLI Tools (`eclusa/bin/`)

17개의 도메인 모듈을 포함하는 Node.js CLI 유틸리티(`eclusa-tools.cjs`)입니다.
...
| 카테고리 | 에이전트 | 병렬성 |
|----------|--------|-------------|
| **Researchers** | eclusa-project-researcher, eclusa-phase-researcher, eclusa-ui-researcher, eclusa-advisor-researcher | 4개 병렬 (stack, features, architecture, pitfalls); advisor는 discuss-phase 중 생성됨 |
| **Synthesizers** | eclusa-research-synthesizer | 순차적 (조사자 완료 후) |
| **Planners** | eclusa-planner, eclusa-roadmapper | 순차적 |
| **Checkers** | eclusa-plan-checker, eclusa-integration-checker, eclusa-ui-checker, eclusa-nyquist-auditor | 순차적 (검증 루프, 최대 3회 반복) |
| **Executors** | eclusa-executor | 웨이브 내 병렬, 웨이브 간 순차적 |
| **Verifiers** | eclusa-verifier | 순차적 (모든 executor 완료 후) |
| **Mappers** | eclusa-codebase-mapper | 4개 병렬 (tech, arch, quality, concerns) |
| **Debuggers** | eclusa-debugger | 순차적 (대화형) |
| **Auditors** | eclusa-ui-auditor | 순차적 |
...
├── commands/eclusa/*.md               # 37개 슬래시 명령어
├── eclusa/
│   ├── bin/eclusa-tools.cjs           # CLI 유틸리티
...
├── hooks/
│   ├── eclusa-statusline.js           # 상태표시줄 훅
│   ├── eclusa-context-monitor.js      # 컨텍스트 경고 훅
│   └── eclusa-check-update.js         # 업데이트 확인 훅
├── settings.json                   # 훅 등록
└── VERSION                         # 설치된 버전 번호
```
...
├── research/               # /eclusa:new-project의 도메인 조사
│   ├── SUMMARY.md
│   ├── STACK.md
│   ├── FEATURES.md
│   ├── ARCHITECTURE.md
│   └── PITFALLS.md
├── codebase/               # 브라운필드 매핑 (/eclusa:map-codebase에서)
│   ├── STACK.md
│   ├── ARCHITECTURE.md
│   ├── CONVENTIONS.md
│   ├── CONCERNS.md
...
│   ├── pending/            # 캡처된 아이디어
│   └── done/               # 완료된 할 일
├── threads/               # 영구 컨텍스트 스레드 (/eclusa:thread에서)
├── seeds/                 # 미래 지향적 아이디어 (/eclusa:plant-seed에서)
├── debug/                  # 활성 디버그 세션
│   ├── *.md                # 활성 세션
│   ├── resolved/           # 보관된 세션
│   └── knowledge-base.md   # 영구 디버그 학습 내용
├── ui-reviews/             # /eclusa:ui-review의 스크린샷 (gitignored)
...
7. **패치 백업** — v1.17부터 로컬 수정 파일을 `eclusa-local-patches/`에 백업하여 `/eclusa:reapply-patches`에 사용
8. **매니페스트 추적** — 깔끔한 제거를 위해 `eclusa-file-manifest.json` 작성
9. **제거 모드** — `--uninstall`로 모든 Eclusa 파일, 훅, 설정 제거
...
- `.eclusa/` 파일에 Write/Edit 시 트리거됩니다
...
- `.eclusa/` 외부 파일에 Write/Edit 시 트리거됩니다
- Eclusa 워크플로우 컨텍스트 외부의 편집을 감지합니다 (활성 `/eclusa:` 명령어 또는 Task 서브에이전트 없음)
- 상태 추적 변경을 위해 `/eclusa:quick` 또는 `/eclusa:fast` 사용을 권고합니다
...
Eclusa는 통합된 명령어/워크플로우 아키텍처를 통해 6개의 AI 코딩 런타임을 지원합니다.

| 런타임 | 명령어 형식 | 에이전트 시스템 | 설정 위치 |
|---------|---------------|--------------|-----------------|
| Claude Code | `/eclusa:command` | Task 생성 | `~/.claude/` |
| OpenCode | `/eclusa-command` | Subagent 모드 | `~/.config/opencode/` |
| Gemini CLI | `/eclusa:command` | Task 생성 | `~/.gemini/` |
| Codex | `$eclusa-command` | Skills | `~/.codex/` |
| Copilot | `/eclusa:command` | 에이전트 위임 | `~/.github/` |
...
5. **모델 참조** — `inherit` 프로필을 통해 Eclusa가 런타임의 모델 선택에 위임합니다
