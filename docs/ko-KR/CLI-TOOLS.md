...
> `eclusa-tools.cjs`에 대한 프로그래밍 방식 API 레퍼런스입니다. 워크플로우와 에이전트가 내부적으로 사용합니다. 사용자 대면 명령어는 [Command Reference](COMMANDS.md)를 참조하세요.

---

## 개요

`eclusa-tools.cjs`는 Eclusa의 약 50개 명령어, 워크플로우, 에이전트 파일에서 반복되는 인라인 bash 패턴을 대체하는 Node.js CLI 유틸리티입니다. config 파싱, 모델 해석, 단계 조회, git 커밋, 요약 검증, 상태 관리, 템플릿 작업을 중앙화합니다.

**위치:** `eclusa/bin/eclusa-tools.cjs`
**모듈:** `eclusa/bin/lib/`의 15개 도메인 모듈
...
`.eclusa/STATE.md`를 관리합니다 — 프로젝트의 살아있는 메모리입니다.
...
에이전트 이름: `eclusa-planner`, `eclusa-executor`, `eclusa-phase-researcher`, `eclusa-project-researcher`, `eclusa-research-synthesizer`, `eclusa-verifier`, `eclusa-plan-checker`, `eclusa-integration-checker`, `eclusa-roadmapper`, `eclusa-debugger`, `eclusa-codebase-mapper`, `eclusa-nyquist-auditor`
...
**대용량 페이로드 처리:** 출력이 약 50KB를 초과하면 CLI가 임시 파일에 쓰고 `@file:/tmp/eclusa-init-XXXXX.json`을 반환합니다. 워크플로우는 `@file:` 접두사를 확인하고 디스크에서 읽습니다.
