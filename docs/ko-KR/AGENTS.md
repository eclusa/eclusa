...
Eclusa는 멀티 에이전트 아키텍처를 사용합니다. 가벼운 오케스트레이터(워크플로우 파일)가 전문화된 에이전트를 새로운 컨텍스트 윈도우로 생성합니다. 각 에이전트는 집중된 역할, 제한된 도구 접근 권한을 가지며 특정 결과물을 생성합니다.
...
| 속성 | 값 |
|----------|-------|
| **생성 주체** | `/eclusa:new-project`, `/eclusa:new-milestone` |
| **병렬성** | 4개 인스턴스 (stack, features, architecture, pitfalls) |
| **도구** | Read, Write, Bash, Grep, Glob, WebSearch, WebFetch, mcp (context7) |
| **모델 (balanced)** | Sonnet |
| **생성물** | `.eclusa/research/STACK.md`, `FEATURES.md`, `ARCHITECTURE.md`, `PITFALLS.md` |
...
| 속성 | 값 |
|----------|-------|
| **생성 주체** | `/eclusa:new-project` (4개 조사자 완료 후) |
| **병렬성** | 단일 인스턴스 (조사자 이후 순차적) |
| **도구** | Read, Write, Bash |
| **모델 (balanced)** | Sonnet |
| **색상** | Purple |
| **생성물** | `.eclusa/research/SUMMARY.md` |
...
| 속성 | 값 |
|----------|-------|
| **생성 주체** | `/eclusa:plan-phase`, `/eclusa:quick` |
| **병렬성** | 단일 인스턴스 |
...
**역할:** 원자적 커밋, 이탈 처리, 체크포인트 프로토콜로 Eclusa 계획을 실행합니다.

| 속성 | 값 |
|----------|-------|
| **생성 주체** | `/eclusa:execute-phase`, `/eclusa:quick` |
...
| 속성 | 값 |
|----------|-------|
| **생성 주체** | `/eclusa:plan-phase` (검증 루프, 최대 3회 반복) |
| **병렬성** | 단일 인스턴스 (반복적) |
...
| 속성 | 값 |
|----------|-------|
| **생성 주체** | `/eclusa:ui-phase` (검증 루프, 최대 2회 반복) |
| **병렬성** | 단일 인스턴스 |
| **도구** | Read, Bash, Glob, Grep |
| **모델 (balanced)** | Sonnet |
| **색상** | `#22D3EE` (cyan) |
...
| 속성 | 값 |
|----------|-------|
| **생성 주체** | `/eclusa:execute-phase` (모든 executor 완료 후) |
| **병렬성** | 단일 인스턴스 |
...
- `/eclusa:verify-work`가 처리할 문제를 기록합니다
...
| 속성 | 값 |
|----------|-------|
| **생성 주체** | `/eclusa:debug`, `/eclusa:verify-work` (실패 시) |
| **병렬성** | 단일 인스턴스 (대화형) |
| **도구** | Read, Write, Edit, Bash, Grep, Glob, WebSearch |
| **모델 (balanced)** | Sonnet |
| **색상** | Orange |
| **생성물** | `.eclusa/debug/*.md`, 지식 베이스 업데이트 |
...
| 속성 | 값 |
|----------|-------|
| **생성 주체** | `/eclusa:profile-user` |
| **병렬성** | 단일 인스턴스 |
| **도구** | Read |
| **모델 (balanced)** | Sonnet |
| **색상** | Magenta |
| **생성물** | `USER-PROFILE.md`, `/eclusa:dev-preferences`, `CLAUDE.md` 프로필 섹션 |
