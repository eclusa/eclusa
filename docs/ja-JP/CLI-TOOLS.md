# Eclusa CLI ツールリファレンス

> `eclusa-tools.cjs` のプログラマティック API リファレンスです。ワークフローやエージェントが内部的に使用します。ユーザー向けコマンドについては、[コマンドリファレンス](COMMANDS.md) を参照してください。

---

## 概要

`eclusa-tools.cjs` は、Eclusa の約50個のコマンド、ワークフロー、エージェントファイル全体で繰り返し使われるインライン bash パターンを置き換える Node.js CLI ユーティリティです。設定の解析、モデル解決、フェーズ検索、git コミット、サマリー検証、状態管理、テンプレート操作を一元化しています。

**配置場所:** `eclusa/bin/eclusa-tools.cjs`
**モジュール:** `eclusa/bin/lib/` 内の15個のドメインモジュール
...
`.eclusa/STATE.md` を管理します — プロジェクトの生きた記憶です。
...
エージェント名: `eclusa-planner`, `eclusa-executor`, `eclusa-phase-researcher`, `eclusa-project-researcher`, `eclusa-research-synthesizer`, `eclusa-verifier`, `eclusa-plan-checker`, `eclusa-integration-checker`, `eclusa-roadmapper`, `eclusa-debugger`, `eclusa-codebase-mapper`, `eclusa-nyquist-auditor`
...
**大容量ペイロードの処理:** 出力が約50KBを超える場合、CLI は一時ファイルに書き出し、`@file:/tmp/eclusa-init-XXXXX.json` を返します。ワークフローは `@file:` プレフィックスを確認し、ディスクから読み込みます:
