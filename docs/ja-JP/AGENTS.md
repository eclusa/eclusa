...
Eclusa はマルチエージェントアーキテクチャを採用しており、軽量なオーケストレーター（ワークフローファイル）が新しいコンテキストウィンドウを持つ専門エージェントをスポーンします。各エージェントは特定の役割に特化し、限定的なツールアクセス権を持ち、特定の成果物を生成します。
...
| プロパティ | 値 |
|------------|-----|
| **スポーン元** | `/eclusa:new-project`, `/eclusa:new-milestone` |
| **並列数** | 4インスタンス（stack, features, architecture, pitfalls） |
| **ツール** | Read, Write, Bash, Grep, Glob, WebSearch, WebFetch, mcp (context7) |
| **モデル (balanced)** | Sonnet |
| **生成物** | `.eclusa/research/STACK.md`, `FEATURES.md`, `ARCHITECTURE.md`, `PITFALLS.md` |
...
| プロパティ | 値 |
|------------|-----|
| **スポーン元** | `/eclusa:new-project`（4つのリサーチャー完了後） |
| **並列数** | 単一インスタンス（リサーチャー後に順次実行） |
| **ツール** | Read, Write, Bash |
| **モデル (balanced)** | Sonnet |
| **カラー** | Purple |
| **生成物** | `.eclusa/research/SUMMARY.md` |
...
| プロパティ | 値 |
|------------|-----|
| **スポーン元** | `/eclusa:plan-phase`, `/eclusa:quick` |
| **並列数** | 単一インスタンス |
...
**役割:** アトミックコミット、逸脱処理、チェックポイントプロトコルを使用して Eclusa 計画を実行する。

| プロパティ | 値 |
|------------|-----|
| **スポーン元** | `/eclusa:execute-phase`, `/eclusa:quick` |
...
| プロパティ | 値 |
|------------|-----|
| **スポーン元** | `/eclusa:plan-phase`（検証ループ、最大3回の反復） |
| **並列数** | 単一インスタンス（反復型） |
...
| プロパティ | 値 |
|------------|-----|
| **スポーン元** | `/eclusa:ui-phase`（検証ループ、最大2回の反復） |
| **並列数** | 単一インスタンス |
| **ツール** | Read, Bash, Glob, Grep |
| **モデル (balanced)** | Sonnet |
| **カラー** | `#22D3EE`（シアン） |
...
| プロパティ | 値 |
|------------|-----|
| **スポーン元** | `/eclusa:execute-phase`（すべてのエグゼキューター完了後） |
| **並列数** | 単一インスタンス |
...
- `/eclusa:verify-work` で対処すべき問題をログに記録
...
| プロパティ | 値 |
|------------|-----|
| **スポーン元** | `/eclusa:debug`, `/eclusa:verify-work`（失敗時） |
| **並列数** | 単一インスタンス（インタラクティブ） |
| **ツール** | Read, Write, Edit, Bash, Grep, Glob, WebSearch |
| **モデル (balanced)** | Sonnet |
| **カラー** | Orange |
| **生成物** | `.eclusa/debug/*.md`、ナレッジベースの更新 |
...
| プロパティ | 値 |
|------------|-----|
| **スポーン元** | `/eclusa:profile-user` |
| **並列数** | 単一インスタンス |
| **ツール** | Read |
| **モデル (balanced)** | Sonnet |
| **カラー** | Magenta |
| **生成物** | `USER-PROFILE.md`、`/eclusa:dev-preferences`、`CLAUDE.md` プロファイルセクション |
