...
**Command:** `/eclusa:new-project [--auto @file.md]`
...
- REQ-INIT-06: System MUST prevent re-initialization when `.eclusa/PROJECT.md` already exists
...
- Existing codebase context (from `/eclusa:map-codebase`) is loaded if present

---

### 2. Phase Discussion

**Command:** `/eclusa:discuss-phase [N] [--auto] [--batch]`
...
**Command:** `/eclusa:plan-phase [N] [--auto] [--skip-research] [--skip-verify]`
...
- REQ-PLAN-07: System MUST prompt user to run `/eclusa:ui-phase` if frontend phase detected and no UI-SPEC.md exists (UI safety gate)
...
**Command:** `/eclusa:execute-phase <N>`
...
- REQ-UIREVIEW-02: System MUST capture screenshots via Playwright CLI to `.eclusa/ui-reviews/`
...
**Commands:** `/eclusa:audit-milestone`, `/eclusa:complete-milestone`, `/eclusa:new-milestone [name]`
...
**Gap Closure:** `/eclusa:plan-milestone-gaps` creates phases to close gaps identified by audit.
...
**Commands:** `/eclusa:add-phase`, `/eclusa:insert-phase [N]`, `/eclusa:remove-phase [N]`
...
**Command:** `/eclusa:quick [--full] [--discuss] [--research]`

**Purpose:** Ad-hoc task execution with Eclusa guarantees but a faster path.
...
- REQ-QUICK-08: System MUST track quick tasks in `.eclusa/quick/YYMMDD-xxx-slug/`
...
**Command:** `/eclusa:autonomous [--from N]`
...
**Purpose:** Analyze freeform text and route to the appropriate Eclusa command.

**Requirements:**
- REQ-DO-01: System MUST parse user intent from natural language input
- REQ-DO-02: System MUST map intent to the best matching Eclusa command
...
- REQ-NEXT-04: System MUST suggest `/eclusa:new-project` if no project exists
- REQ-NEXT-05: System MUST suggest `/eclusa:complete-milestone` when all phases are complete

**State Detection Logic:**
| State | Action |
|-------|--------|
| No `.eclusa/` directory | Suggest `/eclusa:new-project` |
| Phase has no CONTEXT.md | Run `/eclusa:discuss-phase` |
| Phase has no PLAN.md files | Run `/eclusa:plan-phase` |
| Phase has plans but no SUMMARY.md | Run `/eclusa:execute-phase` |
| Phase executed but no VERIFICATION.md | Run `/eclusa:verify-work` |
| All phases complete | Suggest `/eclusa:complete-milestone` |
...
- REQ-NYQ-05: System MUST support retroactive validation via `/eclusa:validate-phase`
...
**Retroactive Validation (`/eclusa:validate-phase [N]`):**
...
- REQ-POSTVER-03: System MUST log issues for `/eclusa:verify-work` to address
...
**Command:** `/eclusa:health [--repair]`

**Purpose:** Validate `.eclusa/` directory integrity and auto-repair issues.
...
**When:** Runs automatically during `/eclusa:execute-phase` before the verifier step.
...
**When:** Runs automatically at the end of `/eclusa:plan-phase` after the plan checker loop.
...
- REQ-CTX-06: Context monitor MUST differentiate Eclusa-active vs non-Eclusa-active projects
...
**Commands:** `/eclusa:pause-work`, `/eclusa:resume-work`, `/eclusa:progress`
...
**Command:** `/eclusa:set-profile <quality|balanced|budget|inherit>`
...
**Command:** `/eclusa:map-codebase [area]`

**Purpose:** Analyze an existing codebase before starting a new project, so Eclusa understands what exists.

**Requirements:**
- REQ-MAP-01: System MUST spawn parallel mapper agents for each analysis area
- REQ-MAP-02: System MUST produce structured documents in `.eclusa/codebase/`
- REQ-MAP-03: System MUST detect: tech stack, architecture patterns, coding conventions, concerns
- REQ-MAP-04: Subsequent `/eclusa:new-project` MUST load codebase mapping and focus questions on what's being added
...
**Command:** `/eclusa:debug [description]`

**Purpose:** Systematic debugging with persistent state across context resets.

**Requirements:**
- REQ-DEBUG-01: System MUST create debug session file in `.eclusa/debug/`
...
- REQ-DEBUG-05: Resolved sessions MUST append to `.eclusa/debug/knowledge-base.md`
...
**Commands:** `/eclusa:add-todo [desc]`, `/eclusa:check-todos`
...
- REQ-TODO-02: Todos MUST be stored in `.eclusa/todos/pending/`
- REQ-TODO-03: Completed todos MUST move to `.eclusa/todos/done/`
...
**Command:** `/eclusa:update`

**Purpose:** Update Eclusa to the latest version with changelog preview.
...
- REQ-UPDATE-04: System MUST back up locally modified files to `eclusa-local-patches/`
- REQ-UPDATE-05: `/eclusa:reapply-patches` MUST restore local modifications after update

---

### 32. Settings Management

**Command:** `/eclusa:settings`
...
- REQ-SETTINGS-02: System MUST update `.eclusa/config.json`
- REQ-SETTINGS-03: System MUST support saving as global defaults (`~/.eclusa/defaults.json`)
...
| `planning.commit_docs` | boolean | `true` | Commit `.eclusa/` files to git |
...
- REQ-GIT-07: System MUST respect `commit_docs` setting for `.eclusa/` files
- REQ-GIT-08: System MUST auto-detect `.eclusa/` in `.gitignore` and skip commits
...
**Purpose:** Run Eclusa across 6 different AI coding agent runtimes.
...
- REQ-RUNTIME-05: Uninstall MUST cleanly remove all Eclusa files without affecting other configurations
...
**Statusline Display:**
```
[⬆ /eclusa:update │] model │ [current task │] directory [█████░░░░░ 50%]
```
...
**Command:** `/eclusa:profile-user [--questionnaire] [--refresh]`
...
- `eclusa-user-profiler` agent — Behavioral analysis from session data
...
Every `/eclusa:progress` call scans ALL phases in the current milestone for outstanding items (pending, skipped, blocked, human_needed). Displays a non-blocking warning section with actionable links.
...
**Requirements:**
- REQ-DEBT-01: System MUST surface outstanding UAT/verification items from ALL prior phases in `/eclusa:progress`
...
- REQ-DEBT-06: `/eclusa:audit-uat` MUST scan all phases, categorize items by testability, and produce a human test plan
...
**Command:** `/eclusa:fast [task description]`
...
- REQ-FAST-03: System MUST track the task in `.eclusa/quick/` for state consistency
- REQ-FAST-04: System MUST NOT be used for tasks requiring research, multi-step planning, or verification

**When to use vs `/eclusa:quick`:**
- `/eclusa:fast` — One-sentence tasks executable in under 2 minutes (typo, config change, small addition)
- `/eclusa:quick` — Anything needing research, multi-step planning, or verification

---

### 42. Cross-AI Peer Review

**Command:** `/eclusa:review --phase N [--gemini] [--claude] [--codex] [--all]`
...
- REQ-REVIEW-05: Reviews MUST be consumable by `/eclusa:plan-phase --reviews`
...
**Commands:** `/eclusa:add-backlog <description>`, `/eclusa:review-backlog`, `/eclusa:plant-seed <idea>`
...
- REQ-BACKLOG-02: Phase directories MUST be created immediately so `/eclusa:discuss-phase` and `/eclusa:plan-phase` work on them
- REQ-BACKLOG-03: `/eclusa:review-backlog` MUST support promote, keep, and remove actions per item
...
- REQ-SEED-02: `/eclusa:new-milestone` MUST scan seeds and present matches

**Produces:**
| Artifact | Description |
|----------|-------------|
| `.eclusa/phases/999.x-slug/` | Backlog item directory |
| `.eclusa/seeds/SEED-NNN-slug.md` | Seed with trigger conditions |

---

### 44. Persistent Context Threads

**Command:** `/eclusa:thread [name | description]`

**Purpose:** Lightweight cross-session knowledge stores for work that spans multiple sessions but doesn't belong to any specific phase. Lighter weight than `/eclusa:pause-work` — no phase state, no plan context.

**Requirements:**
- REQ-THREAD-01: System MUST support create, list, and resume modes
- REQ-THREAD-02: Threads MUST be stored in `.eclusa/threads/` as markdown files
...
**Produces:** `.eclusa/threads/{slug}.md` — Persistent context thread

---

### 45. PR Branch Filtering

**Command:** `/eclusa:pr-branch [target branch]`

**Purpose:** Create a clean branch suitable for pull requests by filtering out `.eclusa/` commits. Reviewers see only code changes, not Eclusa planning artifacts.

**Requirements:**
- REQ-PRBRANCH-01: System MUST identify commits that only modify `.eclusa/` files
...
**Purpose:** Defense-in-depth security for Eclusa's planning artifacts. Because Eclusa generates markdown files that become LLM system prompts, user-controlled text flowing into these files is a potential indirect prompt injection vector.
...
**2. Prompt Injection Guard Hook** (`eclusa-prompt-guard.js`)
PreToolUse hook that scans Write/Edit calls targeting `.eclusa/` for injection patterns. Advisory-only — logs detection for awareness without blocking legitimate operations.

**3. Workflow Guard Hook** (`eclusa-workflow-guard.js`)
PreToolUse hook that detects when Claude attempts file edits outside a Eclusa workflow context. Advises using `/eclusa:quick` or `/eclusa:fast` instead of direct edits. Configurable via `hooks.workflow_guard` (default: false).
...
**Purpose:** Auto-detection and project root resolution for monorepos and multi-repo setups. Supports workspaces where `.eclusa/` may need to resolve across repository boundaries.
...
**Purpose:** Auto-generate `DISCUSSION-LOG.md` during `/eclusa:discuss-phase` for full audit trail of decisions made during discussion.
...
**Command:** `/eclusa:forensics [description]`

**Purpose:** Post-mortem investigation of failed or stuck Eclusa workflows.
...
- REQ-FORENSICS-03: System MUST generate a markdown report saved to `.eclusa/forensics/`
...
| Artifact | Description |
|----------|-------------|
| `.eclusa/forensics/report-{timestamp}.md` | Post-mortem investigation report |
...
3. **Report** — Generate markdown report with findings, saved to `.eclusa/forensics/`
...
**Command:** `/eclusa:milestone-summary [version]`
...
**Command:** `/eclusa:workstreams`

**Purpose:** Parallel workstreams for concurrent work on different milestone areas.

**Requirements:**
- REQ-WS-01: System MUST isolate workstream state in separate `.eclusa/workstreams/{name}/` directories
...
| Artifact | Description |
|----------|-------------|
| `.eclusa/workstreams/{name}/` | Isolated workstream directory structure |

**Process:**
1. **Create** — Initialize a named workstream with isolated `.eclusa/workstreams/{name}/` directory
2. **Switch** — Change active workstream context for subsequent Eclusa commands
...
**Command:** `/eclusa:discuss-phase` with `workflow.discuss_mode: 'assumptions'`
...
**Part of:** `/eclusa:new-project` and `/eclusa:progress`

**Purpose:** Automatically detect UI-heavy projects and surface `/eclusa:ui-phase` recommendation.
...
- REQ-UI-DETECT-03: System MUST suggest `/eclusa:ui-phase` in next steps for UI-heavy phases
- REQ-UI-DETECT-04: System MUST NOT make `/eclusa:ui-phase` mandatory
...
3. **Surface** — Include `/eclusa:ui-phase` recommendation in next steps for UI-heavy phases
...
3. **Install** — Configure Eclusa for all selected runtimes in a single session
