...
const ECLUSA_CODEX_MARKER = '# Eclusa Agent Configuration \u2014 managed by eclusa installer';
const ECLUSA_CODEX_HOOKS_OWNERSHIP_PREFIX = '# Eclusa codex_hooks ownership: ';

// Copilot instructions marker constants
const ECLUSA_COPILOT_INSTRUCTIONS_MARKER = '<!-- Eclusa Configuration \u2014 managed by eclusa installer -->';
const ECLUSA_COPILOT_INSTRUCTIONS_CLOSE_MARKER = '<!-- /Eclusa Configuration -->';

const CODEX_AGENT_SANDBOX = {
  'eclusa-executor': 'workspace-write',
  'eclusa-planner': 'workspace-write',
  'eclusa-phase-researcher': 'workspace-write',
  'eclusa-project-researcher': 'workspace-write',
  'eclusa-research-synthesizer': 'workspace-write',
  'eclusa-verifier': 'workspace-write',
  'eclusa-codebase-mapper': 'workspace-write',
  'eclusa-roadmapper': 'workspace-write',
  'eclusa-debugger': 'workspace-write',
  'eclusa-plan-checker': 'read-only',
  'eclusa-integration-checker': 'read-only',
};
...
const banner = '\n' +
  ...
  dim + '   ━━━╋━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╋━━━' + reset + '\n' +
  '\n' +
  '  Eclusa ' + dim + 'v' + pkg.version + reset + '\n' +
  '  From noisy intent to constrained, test-backed code.\n' +
  dim + '  Built on GSD by TÂCHES.' + reset + '\n';
...
if (hasHelp) {
  console.log(`  ${yellow}Usage:${reset} npx eclusa [options]\n\n  ${yellow}Options:${reset}\n    ${cyan}-g, --global${reset}              Install globally (to config directory)\n    ${cyan}-l, --local${reset}               Install locally (to current directory)\n    ${cyan}--claude${reset}                  Install for Claude Code only\n    ${cyan}--opencode${reset}                Install for OpenCode only\n    ${cyan}--gemini${reset}                  Install for Gemini only\n    ${cyan}--codex${reset}                   Install for Codex only\n    ${cyan}--copilot${reset}                 Install for Copilot only\n    ${cyan}--antigravity${reset}             Install for Antigravity only\n    ${cyan}--cursor${reset}                  Install for Cursor only\n    ${cyan}--windsurf${reset}                Install for Windsurf only\n    ${cyan}--all${reset}                     Install for all runtimes\n    ${cyan}--sdk${reset}                     Also install Eclusa SDK CLI (eclusa-sdk)\n    ${cyan}-u, --uninstall${reset}           Uninstall Eclusa (remove all Eclusa files)\n    ${cyan}-c, --config-dir <path>${reset}   Specify custom config directory\n    ${cyan}-h, --help${reset}                Show this help message\n    ${cyan}--force-statusline${reset}        Replace existing statusline config\n\n  ${yellow}Examples:${reset}\n    ${dim}# Interactive install (prompts for runtime and location)${reset}\n    npx eclusa\n\n    ${dim}# Install for Claude Code globally${reset}\n    npx eclusa --claude --global\n\n    ${dim}# Install for Gemini globally${reset}\n    npx eclusa --gemini --global\n\n    ${dim}# Install for Codex globally${reset}\n    npx eclusa --codex --global\n\n    ${dim}# Install for Copilot globally${reset}\n    npx eclusa --copilot --global\n\n    ${dim}# Install for Copilot locally${reset}\n    npx eclusa --copilot --local\n\n    ${dim}# Install for Antigravity globally${reset}\n    npx eclusa --antigravity --global\n\n    ${dim}# Install for Antigravity locally${reset}\n    npx eclusa --antigravity --local\n\n    ${dim}# Install for Cursor globally${reset}\n    npx eclusa --cursor --global\n\n    ${dim}# Install for Cursor locally${reset}\n    npx eclusa --cursor --local\n\n    ${dim}# Install for Windsurf globally${reset}\n    npx eclusa --windsurf --global\n\n    ${dim}# Install for Windsurf locally${reset}\n    npx eclusa --windsurf --local\n\n    ${dim}# Install for all runtimes globally${reset}\n    npx eclusa --all --global\n\n    ${dim}# Install to custom config directory${reset}\n    npx eclusa --codex --global --config-dir ~/.codex-work\n\n    ${dim}# Install to current project only${reset}\n    npx eclusa --claude --local\n\n    ${dim}# Uninstall Eclusa from Cursor globally${reset}\n    npx eclusa --cursor --global --uninstall\n\n  ${yellow}Notes:${reset}\n    The --config-dir option is useful when you have multiple configurations.\n    It takes priority over CLAUDE_CONFIG_DIR / GEMINI_CONFIG_DIR / CODEX_HOME / COPILOT_CONFIG_DIR / ANTIGRAVITY_CONFIG_DIR / CURSOR_CONFIG_DIR / WINDSURF_CONFIG_DIR environment variables.\n`);
  process.exit(0);
}
...
function convertClaudeToCopilotContent(content, isGlobal = false) {
  ...
  c = c.replace(/\.\/\.claude\//g, './.github/');
  c = c.replace(/\.claude\//g, '.github/');
  // CONV-07: Command name conversion (all eclusa: references → eclusa-)
  c = c.replace(/eclusa:/g, 'eclusa-');
...
function convertClaudeToAntigravityContent(content, isGlobal = false) {
  ...
  c = c.replace(/\.\/\.claude\//g, './.agent/');
  c = c.replace(/\.claude\//g, '.agent/');
  // Command name conversion (all eclusa: references → eclusa-)
  c = c.replace(/eclusa:/g, 'eclusa-');
  // Runtime-neutral agent name replacement (#766)
...
function convertSlashCommandsToCursorSkillMentions(content) {
  // Keep leading "/" for slash commands; only normalize eclusa: -> eclusa-.
  // This preserves rendered "next step" commands like "/eclusa-execute-phase 17".
  return content.replace(/eclusa:/gi, 'eclusa-');
}
...
function convertClaudeToCursorMarkdown(content) {
  ...
  converted = converted.replace(/\$ARGUMENTS\b/g, '{{ECLUSA_ARGS}}');
...
- Treat all user text after the skill mention as \`{{ECLUSA_ARGS}}\`.
- If no arguments are present, treat \`{{ECLUSA_ARGS}}\` as empty.
...
## C. Tool Usage
Use these Cursor tools when executing Eclusa workflows:
...
function convertClaudeCommandToCursorSkill(content, skillName) {
  ...
  const { frontmatter, body } = extractFrontmatterAndBody(converted);
  let description = `Run Eclusa workflow ${skillName}.`;
  if (frontmatter) {
...
## C. Tool Usage
Use these Windsurf tools when executing Eclusa workflows:
- \`Shell\` for running commands (terminal operations)
- \`StrReplace\` for editing existing files
- \`Read\`, \`Write\`, \`Glob\`, \`Grep\`, \`Task\`, \`WebSearch\`, \`WebFetch\`, \`TodoWrite\` as needed
...
function convertSlashCommandsToCodexSkillMentions(content) {
  let converted = content.replace(/\/eclusa:([a-z0-9-]+)/gi, (_, commandName) => {
    return `$eclusa-${String(commandName).toLowerCase()}`;
  });
  converted = converted.replace(/\/eclusa-help\b/g, '$eclusa-help');
  return converted;
}
...
- Treat all user text after \`${invocation}\` as \`{{ECLUSA_ARGS}}\`.
- If no arguments are present, treat \`{{ECLUSA_ARGS}}\` as empty.

## B. AskUserQuestion → request_user_input Mapping
Eclusa workflows use \`AskUserQuestion\` (Claude Code syntax). Translate to Codex \`request_user_input\`:
...
Eclusa workflows use \`Task(...)\` (Claude Code syntax). Translate to Codex collaboration tools:

Direct mapping:
- \`Task(subagent_type="X", prompt="Y")\` → \`spawn_agent(agent_type="X", message="Y")\`
- \`Task(model="...")\` → omit (Codex uses per-role config, not inline model selection)
- \`fork_context: false\` by default — Eclusa agents load their own context via \`<files_to_read>\` blocks
...
function generateCodexAgentToml(agentName, agentContent) {
  ...
  const resolvedDescription = toSingleLine(
    extractFrontmatterField(frontmatterText, 'description') || `Eclusa agent ${resolvedName}`
  );
...
/**
 * Generate the Eclusa config block for Codex config.toml.
...
function stripCodexEclusaAgentSections(content) {
  return content.replace(/^\[agents\.eclusa-[^\]]+\]\n(?:(?!\[)[^\n]*\n?)*/gm, '');
}

/**
 * Strip Eclusa sections from Codex config.toml content.
 * Returns cleaned content, or null if file would be empty.
 */
function stripEclusaFromCodexConfig(content) {
  const eol = detectLineEnding(content);
  const markerIndex = content.indexOf(ECLUSA_CODEX_MARKER);
  const codexHooksOwnership = getManagedCodexHooksOwnership(content);

  if (markerIndex !== -1) {
    // Has Eclusa marker — remove everything from marker to EOF
    let before = content.substring(0, markerIndex);
    before = stripCodexHooksFeatureAssignments(before, codexHooksOwnership);
    // Also strip Eclusa-injected feature keys above the marker (Case 3 inject)
  ...
  // No marker but may have Eclusa-injected feature keys
  let cleaned = content;
  ...
  // Remove [agents.eclusa-*] sections (from header to next section or EOF)
  cleaned = stripCodexEclusaAgentSections(cleaned);
...
function getManagedCodexHooksOwnership(content) {
  ...
  const afterMarker = content.slice(markerIndex + ECLUSA_CODEX_MARKER.length);
  const match = afterMarker.match(/^\r?\n# Eclusa codex_hooks ownership: (section|root_dotted)\r?\n/);
...
function setManagedCodexHooksOwnership(content, ownership) {
  ...
  const eol = detectLineEnding(content);
  const markerEnd = markerIndex + ECLUSA_CODEX_MARKER.length;
  ...
  return content.slice(0, markerEnd) +
    eol +
    `${ECLUSA_CODEX_HOOKS_OWNERSHIP_PREFIX}${ownership}${eol}` +
    remainder;
}

function isLegacyEclusaAgentsSection(body) {
...
function stripLeakedEclusaCodexSections(content) {
  const leakedSections = getTomlTableSections(content)
    .filter((section) =>
      section.path.startsWith('agents.eclusa-') ||
      (
        section.path === 'agents' &&
        isLegacyEclusaAgentsSection(content.slice(section.headerEnd, section.end))
      )
    );
...
/**
 * Merge Eclusa config block into an existing or new config.toml.
 * Three cases: new file, existing with Eclusa marker, existing without marker.
 */
function mergeCodexConfig(configPath, eclusaBlock) {
  // Case 1: No config.toml — create fresh
  if (!fs.existsSync(configPath)) {
    fs.writeFileSync(configPath, eclusaBlock + '\n');
    return;
  }

  const existing = fs.readFileSync(configPath, 'utf8');
  const eol = detectLineEnding(existing);
  const normalizedEclusaBlock = eclusaBlock.replace(/\r?\n/g, eol);
  const markerIndex = existing.indexOf(ECLUSA_CODEX_MARKER);

  // Case 2: Has Eclusa marker — truncate and re-append
  if (markerIndex !== -1) {
    let before = existing.substring(0, markerIndex).trimEnd();
    if (before) {
      // Strip any Eclusa-managed sections that leaked above the marker from previous installs
      before = stripLeakedEclusaCodexSections(before).trimEnd();

      fs.writeFileSync(configPath, before + eol + eol + normalizedEclusaBlock + eol);
    } else {
      fs.writeFileSync(configPath, normalizedEclusaBlock + eol);
    }
    return;
  }

  // Case 3: No marker — append Eclusa block
  let content = stripLeakedEclusaCodexSections(existing).trimEnd();
  if (content) {
    content = content + eol + eol + normalizedEclusaBlock + eol;
  } else {
    content = normalizedEclusaBlock + eol;
  }
...
/**
 * Repair config.toml files corrupted by pre-#1346 Eclusa installs.
...
/**
 * Merge Eclusa instructions into copilot-instructions.md.
 ...
 * @param {string} eclusaContent - Template content (without markers)
 */
function mergeCopilotInstructions(filePath, eclusaContent) {
  const eclusaBlock = ECLUSA_COPILOT_INSTRUCTIONS_MARKER + '\n' +
    eclusaContent.trim() + '\n' +
    ECLUSA_COPILOT_INSTRUCTIONS_CLOSE_MARKER;

  // Case 1: No file — create fresh
  if (!fs.existsSync(filePath)) {
    fs.writeFileSync(filePath, eclusaBlock + '\n');
    return;
  }

  const existing = fs.readFileSync(filePath, 'utf8');
  const openIndex = existing.indexOf(ECLUSA_COPILOT_INSTRUCTIONS_MARKER);
  const closeIndex = existing.indexOf(ECLUSA_COPILOT_INSTRUCTIONS_CLOSE_MARKER);

  // Case 2: Has Eclusa markers — replace between markers
  if (openIndex !== -1 && closeIndex !== -1) {
    const before = existing.substring(0, openIndex).trimEnd();
    const after = existing.substring(closeIndex + ECLUSA_COPILOT_INSTRUCTIONS_CLOSE_MARKER.length).trimStart();
    let newContent = '';
    if (before) newContent += before + '\n\n';
    newContent += eclusaBlock;
    if (after) newContent += '\n\n' + after;
    newContent += '\n';
...
/**
 * Strip Eclusa section from copilot-instructions.md content.
 * Returns cleaned content, or null if file should be deleted (was Eclusa-only).
 * @param {string} content - File content
 * @returns {string|null} - Cleaned content or null if empty
 */
function stripEclusaFromCopilotInstructions(content) {
  const openIndex = content.indexOf(ECLUSA_COPILOT_INSTRUCTIONS_MARKER);
  const closeIndex = content.indexOf(ECLUSA_COPILOT_INSTRUCTIONS_CLOSE_MARKER);

  if (openIndex !== -1 && closeIndex !== -1) {
    const before = content.substring(0, openIndex).trimEnd();
    const after = content.substring(closeIndex + ECLUSA_COPILOT_INSTRUCTIONS_CLOSE_MARKER.length).trimStart();
...
function installCodexConfig(targetDir, agentsSrc) {
  ...
  const agentEntries = fs.readdirSync(agentsSrc).filter(f => f.startsWith('eclusa-') && f.endsWith('.md'));
  const agents = [];

  // Compute the Codex Eclusa install path (absolute, so subagents with empty $HOME work — #820)
  const codexEclusaPath = `${path.resolve(targetDir, 'eclusa').replace(/\\/g, '/')}/`;

  for (const file of agentEntries) {
    let content = fs.readFileSync(path.join(agentsSrc, file), 'utf8');
    // Replace full .claude/eclusa prefix so path resolves to codex Eclusa install
    content = content.replace(/~\/\.claude\/eclusa\//g, codexEclusaPath);
    content = content.replace(/\$HOME\/\.claude\/eclusa\//g, codexEclusaPath);
  ...
  const eclusaBlock = generateCodexConfigBlock(agents, targetDir);
  mergeCodexConfig(configPath, eclusaBlock);
...
function convertClaudeToGeminiAgent(content) {
  ...
  // when they can't be resolved. Eclusa agents use ${PHASE}, ${PLAN}, etc. as
...
function convertClaudeToOpencodeFrontmatter(content, { isAgent = false } = {}) {
  ...
  // Replace /eclusa:command with /eclusa-command for opencode (flat command structure)
  convertedContent = convertedContent.replace(/\/eclusa:/g, '/eclusa-');
 ...
 * OpenCode expects: command/eclusa-help.md (invoked as /eclusa-help)
 * Source structure: commands/eclusa/help.md
 * 
 * @param {string} srcDir - Source directory (e.g., commands/eclusa/)
 * @param {string} destDir - Destination directory (e.g., command/)
 * @param {string} prefix - Prefix for filenames (e.g., 'eclusa')
...
function copyFlattenedCommands(srcDir, destDir, prefix, pathPrefix, runtime) {
  ...
  // Remove old eclusa-*.md files before copying new ones
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      // e.g., commands/eclusa/debug/start.md -> command/eclusa-debug-start.md
      copyFlattenedCommands(srcPath, destDir, `${prefix}-${entry.name}`, pathPrefix, runtime);
    } else if (entry.name.endsWith('.md')) {
      // Flatten: help.md -> eclusa-help.md
...
function listCodexSkillNames(skillsDir, prefix = 'eclusa-') {
...
function copyCommandsAsCodexSkills(srcDir, skillsDir, prefix, pathPrefix, runtime) {
  ...
  // Remove previous Eclusa Codex skills to avoid stale command skills.
...
function copyCommandsAsCursorSkills(srcDir, skillsDir, prefix, pathPrefix, runtime) {
  ...
  // Remove previous Eclusa Cursor skills to avoid stale command skills
  const existing = fs.readdirSync(skillsDir, { withFileTypes: true });
  for (const entry of existing) {
    if (entry.isDirectory() && entry.name.startsWith(`${prefix}-`)) {
      fs.rmSync(path.join(skillsDir, entry.name), { recursive: true });
...
function copyCommandsAsWindsurfSkills(srcDir, skillsDir, prefix, pathPrefix, runtime) {
  ...
  // Remove previous Eclusa Windsurf skills to avoid stale command skills
  const existing = fs.readdirSync(skillsDir, { withFileTypes: true });
  for (const entry of existing) {
    if (entry.isDirectory() && entry.name.startsWith(`${prefix}-`)) {
      fs.rmSync(path.join(skillsDir, entry.name), { recursive: true });
...
function copyCommandsAsCopilotSkills(srcDir, skillsDir, prefix, isGlobal = false) {
  ...
  // Remove previous Eclusa Copilot skills
  const existing = fs.readdirSync(skillsDir, { withFileTypes: true });
  for (const entry of existing) {
    if (entry.isDirectory() && entry.name.startsWith(`${prefix}-`)) {
      fs.rmSync(path.join(skillsDir, entry.name), { recursive: true });
...
/**
 * Recursively install Eclusa commands as Antigravity skills.
 ...
 * @param {string} prefix - Skill name prefix (e.g. 'eclusa')
 * @param {boolean} isGlobal - Whether this is a global install
 */
function copyCommandsAsAntigravitySkills(srcDir, skillsDir, prefix, isGlobal = false) {
  ...
  // Remove previous Eclusa Antigravity skills
  const existing = fs.readdirSync(skillsDir, { withFileTypes: true });
  for (const entry of existing) {
    if (entry.isDirectory() && entry.name.startsWith(`${prefix}-`)) {
      fs.rmSync(path.join(skillsDir, entry.name), { recursive: true });
...
function copyWithPathReplacement(srcDir, destDir, pathPrefix, runtime, isCommand = false, isGlobal = false) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      jsContent = jsContent.replace(/eclusa:/gi, 'eclusa-');
...
/**
 * Clean up orphaned files from previous Eclusa versions
 */
function cleanupOrphanedFiles(configDir) {
  const orphanedFiles = [
    'hooks/eclusa-notify.sh',  // Removed in v1.6.x
    'hooks/statusline.js',  // Renamed to eclusa-statusline.js in v1.9.0
  ];
...
function cleanupOrphanedHooks(settings) {
  const orphanedHookPatterns = [
    'eclusa-notify.sh',  // Removed in v1.6.x
    'hooks/statusline.js',  // Renamed to eclusa-statusline.js in v1.9.0
    'eclusa-intel-index.js',  // Removed in v1.9.2
    'eclusa-intel-session.js',  // Removed in v1.9.2
    'eclusa-intel-prune.js',  // Removed in v1.9.2
  ];
  ...
  // Fix #330: Update statusLine if it points to old Eclusa statusline.js path
  // Only match the specific old Eclusa path pattern (hooks/statusline.js),
  ...
  if (settings.statusLine && settings.statusLine.command &&
    ...
    settings.statusLine.command = settings.statusLine.command.replace(
      /hooks([\/\\])statusline\.js/,
      'hooks$1eclusa-statusline.js'
    );
    console.log(`  ${green}✓${reset} Updated statusline path (hooks/statusline.js → hooks/eclusa-statusline.js)`);
  }
...
/**
 * Uninstall Eclusa from the specified directory for a specific runtime
 * Removes only Eclusa-specific files/directories, preserves user content
...
function uninstall(isGlobal, runtime = 'claude') {
  ...
  console.log(`  Uninstalling Eclusa from ${cyan}${runtimeLabel}${reset} at ${cyan}${locationLabel}${reset}\n`);
  ...
  // 1. Remove Eclusa commands/skills
  if (isOpencode) {
    // OpenCode: remove command/eclusa-*.md files
    const commandDir = path.join(targetDir, 'command');
    if (fs.existsSync(commandDir)) {
      const files = fs.readdirSync(commandDir);
      for (const file of files) {
        if (file.startsWith('eclusa-') && file.endsWith('.md')) {
          fs.unlinkSync(path.join(commandDir, file));
          removedCount++;
        }
      }
      console.log(`  ${green}✓${reset} Removed Eclusa commands from command/`);
    }
  } else if (isCodex || isCursor || isWindsurf) {
    // Codex/Cursor/Windsurf: remove skills/eclusa-*/SKILL.md skill directories
    const skillsDir = path.join(targetDir, 'skills');
    if (fs.existsSync(skillsDir)) {
      let skillCount = 0;
      const entries = fs.readdirSync(skillsDir, { withFileTypes: true });
      for (const entry of entries) {
        if (entry.isDirectory() && entry.name.startsWith('eclusa-')) {
          fs.rmSync(path.join(skillsDir, entry.name), { recursive: true });
          skillCount++;
        }
      }
    ...
    // Codex-only: remove Eclusa agent .toml config files and config.toml sections
    if (isCodex) {
    ...
    if (fs.existsSync(codexAgentsDir)) {
      ...
      for (const file of tomlFiles) {
        if (file.startsWith('eclusa-') && file.endsWith('.toml')) {
    ...
    // Codex: clean Eclusa sections from config.toml
    const configPath = path.join(targetDir, 'config.toml');
    if (fs.existsSync(configPath)) {
      const content = fs.readFileSync(configPath, 'utf8');
      const cleaned = stripEclusaFromCodexConfig(content);
      if (cleaned === null) {
        // File is empty after stripping — delete it
        fs.unlinkSync(configPath);
        removedCount++;
        console.log(`  ${green}✓${reset} Removed config.toml (was Eclusa-only)`);
      } else if (cleaned !== content) {
        fs.writeFileSync(configPath, cleaned);
        removedCount++;
        console.log(`  ${green}✓${reset} Cleaned Eclusa sections from config.toml`);
      }
    }
    }
  } else if (isCopilot) {
    // Copilot: remove skills/eclusa-*/ directories (same layout as Codex skills)
    const skillsDir = path.join(targetDir, 'skills');
    if (fs.existsSync(skillsDir)) {
      let skillCount = 0;
      const entries = fs.readdirSync(skillsDir, { withFileTypes: true });
    ...
    // Copilot: clean Eclusa section from copilot-instructions.md
    const instructionsPath = path.join(targetDir, 'copilot-instructions.md');
    if (fs.existsSync(instructionsPath)) {
      const content = fs.readFileSync(instructionsPath, 'utf8');
      const cleaned = stripEclusaFromCopilotInstructions(content);
      if (cleaned === null) {
        fs.unlinkSync(instructionsPath);
        removedCount++;
        console.log(`  ${green}✓${reset} Removed copilot-instructions.md (was Eclusa-only)`);
      } else if (cleaned !== content) {
        fs.writeFileSync(instructionsPath, cleaned);
        removedCount++;
        console.log(`  ${green}✓${reset} Cleaned Eclusa section from copilot-instructions.md`);
      }
    }
  } else if (isAntigravity) {
    // Antigravity: remove skills/eclusa-*/ directories (same layout as Copilot skills)
    const skillsDir = path.join(targetDir, 'skills');
    if (fs.existsSync(skillsDir)) {
      let skillCount = 0;
      const entries = fs.readdirSync(skillsDir, { withFileTypes: true });
    ...
    // Cursor: remove skills/eclusa-*/ directories (same layout as Codex skills)
    const skillsDir = path.join(targetDir, 'skills');
    if (fs.existsSync(skillsDir)) {
      let skillCount = 0;
      const entries = fs.readdirSync(skillsDir, { withFileTypes: true });
    ...
    // Windsurf: remove skills/eclusa-*/ directories (same layout as Cursor skills)
    const skillsDir = path.join(targetDir, 'skills');
    if (fs.existsSync(skillsDir)) {
      let skillCount = 0;
      const entries = fs.readdirSync(skillsDir, { withFileTypes: true });
    ...
    const eclusaCommandsDir = path.join(targetDir, 'commands', 'eclusa');
    if (fs.existsSync(eclusaCommandsDir)) {
      fs.rmSync(eclusaCommandsDir, { recursive: true });
      removedCount++;
      console.log(`  ${green}✓${reset} Removed commands/eclusa/`);
    }
  }

  // 2. Remove eclusa directory
  const eclusaDir = path.join(targetDir, 'eclusa');
  if (fs.existsSync(eclusaDir)) {
    fs.rmSync(eclusaDir, { recursive: true });
    removedCount++;
    console.log(`  ${green}✓${reset} Removed eclusa/`);
  }

  // 3. Remove Eclusa agents (eclusa-*.md files only)
  ...
  if (fs.existsSync(agentsDir)) {
    ...
    if (agentCount > 0) {
      removedCount++;
      console.log(`  ${green}✓${reset} Removed ${agentCount} Eclusa agents`);
    }
  }

  // 4. Remove Eclusa hooks
  const hooksDir = path.join(targetDir, 'hooks');
  if (fs.existsSync(hooksDir)) {
    const eclusaHooks = ['eclusa-statusline.js', 'eclusa-check-update.js', 'eclusa-check-update.sh', 'eclusa-context-monitor.js', 'eclusa-prompt-guard.js'];
    let hookCount = 0;
    for (const hook of eclusaHooks) {
    ...
    if (hookCount > 0) {
      removedCount++;
      console.log(`  ${green}✓${reset} Removed ${hookCount} Eclusa hooks`);
    }
  }

  // 5. Remove Eclusa package.json (CommonJS mode marker)
  ...
  if (fs.existsSync(pkgJsonPath)) {
    try {
      ...
      if (content === '{"type":"commonjs"}') {
        ...
        removedCount++;
        console.log(`  ${green}✓${reset} Removed Eclusa package.json`);
      }
    } catch (e) {
      // Ignore read errors
    }
  ...
  // 6. Clean up settings.json (remove Eclusa hooks and statusline)
  const settingsPath = path.join(targetDir, 'settings.json');
  if (fs.existsSync(settingsPath)) {
    let settings = readSettings(settingsPath);
    let settingsModified = false;

    // Remove Eclusa statusline if it references our hook
    if (settings.statusLine && settings.statusLine.command &&
        settings.statusLine.command.includes('eclusa-statusline')) {
      delete settings.statusLine;
      settingsModified = true;
      console.log(`  ${green}✓${reset} Removed Eclusa statusline from settings`);
    }

    // Remove Eclusa hooks from SessionStart
    if (settings.hooks && settings.hooks.SessionStart) {
      const before = settings.hooks.SessionStart.length;
      settings.hooks.SessionStart = settings.hooks.SessionStart.filter(entry => {
        if (entry.hooks && Array.isArray(entry.hooks)) {
          // Filter out Eclusa hooks
          const hasEclusaHook = entry.hooks.some(h =>
            h.command && (h.command.includes('eclusa-check-update') || h.command.includes('eclusa-statusline'))
          );
          return !hasEclusaHook;
        }
        return true;
      });
      if (settings.hooks.SessionStart.length < before) {
        settingsModified = true;
        console.log(`  ${green}✓${reset} Removed Eclusa hooks from settings`);
      }
    ...
    // Remove Eclusa hooks from PostToolUse and AfterTool (Gemini uses AfterTool)
    for (const eventName of ['PostToolUse', 'AfterTool']) {
      if (settings.hooks && settings.hooks[eventName]) {
        ...
        settings.hooks[eventName] = settings.hooks[eventName].filter(entry => {
          if (entry.hooks && Array.isArray(entry.hooks)) {
            const hasEclusaHook = entry.hooks.some(h =>
              h.command && h.command.includes('eclusa-context-monitor')
            );
            return !hasEclusaHook;
          }
          return true;
    ...
    // Remove Eclusa hooks from PreToolUse and BeforeTool (Gemini uses BeforeTool)
    for (const eventName of ['PreToolUse', 'BeforeTool']) {
      if (settings.hooks && settings.hooks[eventName]) {
        ...
        settings.hooks[eventName] = settings.hooks[eventName].filter(entry => {
          if (entry.hooks && Array.isArray(entry.hooks)) {
            const hasEclusaHook = entry.hooks.some(h =>
              h.command && h.command.includes('eclusa-prompt-guard')
            );
            return !hasEclusaHook;
          }
          return true;
  ...
  if (isOpencode) {
    ...
    if (fs.existsSync(configPath)) {
      try {
        ...
        // Remove Eclusa permission entries
        if (config.permission) {
          for (const permType of ['read', 'external_directory']) {
            if (config.permission[permType]) {
              const keys = Object.keys(config.permission[permType]);
              for (const key of keys) {
                if (key.includes('eclusa')) {
        ...
        if (modified) {
          ...
          removedCount++;
          console.log(`  ${green}✓${reset} Removed Eclusa permissions from ${path.basename(configPath)}`);
        }
      } catch (e) {
  ...
  if (removedCount === 0) {
    console.log(`  ${yellow}⚠${reset} No Eclusa files found to remove.`);
  }

  console.log(`
  ${green}Done!${reset} Eclusa has been uninstalled from ${runtimeLabel}.
...
/**
 * Configure OpenCode permissions to allow reading Eclusa reference docs
 * This prevents permission prompts when Eclusa accesses the eclusa directory
...
function configureOpencodePermissions(isGlobal = true) {
  ...
  // Build the Eclusa path using the actual config directory
  // Use ~ shorthand if it's in the default location, otherwise use full path
  const defaultConfigDir = path.join(os.homedir(), '.config', 'opencode');
  const eclusaPath = opencodeConfigDir === defaultConfigDir
    ? '~/.config/opencode/eclusa/*'
    : `${opencodeConfigDir.replace(/\\/g, '/')}/eclusa/*`;
  ...
  if (config.permission.read[eclusaPath] !== 'allow') {
    config.permission.read[eclusaPath] = 'allow';
    modified = true;
  }
  ...
  if (config.permission.external_directory[eclusaPath] !== 'allow') {
    config.permission.external_directory[eclusaPath] = 'allow';
    modified = true;
  }
  ...
  // Write config back
  fs.writeFileSync(configPath, JSON.stringify(config, null, 2) + '\n');
  console.log(`  ${green}✓${reset} Configured read permission for Eclusa docs`);
}
...
const PATCHES_DIR_NAME = 'eclusa-local-patches';
const MANIFEST_NAME = 'eclusa-file-manifest.json';
...
function writeManifest(configDir, runtime = 'claude') {
  ...
  const isCursor = runtime === 'cursor';
  const isWindsurf = runtime === 'windsurf';
  const eclusaDir = path.join(configDir, 'eclusa');
  const commandsDir = path.join(configDir, 'commands', 'eclusa');
  const opencodeCommandDir = path.join(configDir, 'command');
  const codexSkillsDir = path.join(configDir, 'skills');
  const agentsDir = path.join(configDir, 'agents');
  const manifest = { version: pkg.version, timestamp: new Date().toISOString(), files: {} };

  const eclusaHashes = generateManifest(eclusaDir);
  for (const [rel, hash] of Object.entries(eclusaHashes)) {
    manifest.files['eclusa/' + rel] = hash;
  }
  if (!isOpencode && !isCodex && !isCopilot && !isAntigravity && !isCursor && !isWindsurf && fs.existsSync(commandsDir)) {
    const cmdHashes = generateManifest(commandsDir);
    for (const [rel, hash] of Object.entries(cmdHashes)) {
      manifest.files['commands/eclusa/' + rel] = hash;
    }
  }
  ...
  if (!isCodex && !isCopilot) {
    ...
    if (fs.existsSync(hooksDir)) {
      for (const file of fs.readdirSync(hooksDir)) {
        if (file.startsWith('eclusa-') && file.endsWith('.js')) {
...
/**
 * Detect user-modified Eclusa files by comparing against install manifest.
 * Backs up modified files to eclusa-local-patches/ for reapply after update.
 */
function saveLocalPatches(configDir) {
  ...
  if (modified.length > 0) {
    ...
    console.log('  ' + yellow + 'i' + reset + '  Found ' + modified.length + ' locally modified Eclusa file(s) — backed up to ' + PATCHES_DIR_NAME + '/');
  ...
  if (meta.files && meta.files.length > 0) {
    const reapplyCommand = (runtime === 'opencode' || runtime === 'copilot')
      ...
      : runtime === 'codex'
        ? '$eclusa-reapply-patches'
        : runtime === 'cursor'
          ? 'eclusa-reapply-patches (mention the skill name)'
          : '/eclusa:reapply-patches';
    console.log('');
...
function install(isGlobal, runtime = 'claude') {
  ...
  // Path prefix for file references in markdown content (e.g. eclusa-tools.cjs).
  // Replaces $HOME/.claude/ or ~/.claude/ so the result is <pathPrefix>eclusa/bin/...
  ...
  // Save any locally modified Eclusa files before they get wiped
  saveLocalPatches(targetDir);

  // Clean up orphaned files from previous versions
  cleanupOrphanedFiles(targetDir);

  // OpenCode uses command/ (flat), Codex uses skills/, Claude/Gemini use commands/eclusa/
  if (isOpencode) {
    // OpenCode: flat structure in command/ directory
    const commandDir = path.join(targetDir, 'command');
    fs.mkdirSync(commandDir, { recursive: true });
    
    // Copy commands/eclusa/*.md as command/eclusa-*.md (flatten structure)
    const eclusaSrc = path.join(src, 'commands', 'eclusa');
    copyFlattenedCommands(eclusaSrc, commandDir, 'eclusa', pathPrefix, runtime);
    if (verifyInstalled(commandDir, 'command/eclusa-*')) {
      const count = fs.readdirSync(commandDir).filter(f => f.startsWith('eclusa-')).length;
      console.log(`  ${green}✓${reset} Installed ${count} commands to command/`);
    } else {
      failures.push('command/eclusa-*');
    }
  } else if (isCodex) {
    const skillsDir = path.join(targetDir, 'skills');
    const eclusaSrc = path.join(src, 'commands', 'eclusa');
    copyCommandsAsCodexSkills(eclusaSrc, skillsDir, 'eclusa', pathPrefix, runtime);
    const installedSkillNames = listCodexSkillNames(skillsDir);
    if (installedSkillNames.length > 0) {
      console.log(`  ${green}✓${reset} Installed ${installedSkillNames.length} skills to skills/`);
    } else {
      failures.push('skills/eclusa-*');
    }
  } else if (isCopilot) {
    const skillsDir = path.join(targetDir, 'skills');
    const eclusaSrc = path.join(src, 'commands', 'eclusa');
    copyCommandsAsCopilotSkills(eclusaSrc, skillsDir, 'eclusa', isGlobal);
    if (fs.existsSync(skillsDir)) {
      const count = fs.readdirSync(skillsDir, { withFileTypes: true })
        .filter(e => e.isDirectory() && e.name.startsWith('eclusa-')).length;
      if (count > 0) {
    ...
    const skillsDir = path.join(targetDir, 'skills');
    const eclusaSrc = path.join(src, 'commands', 'eclusa');
    copyCommandsAsAntigravitySkills(eclusaSrc, skillsDir, 'eclusa', isGlobal);
    if (fs.existsSync(skillsDir)) {
      const count = fs.readdirSync(skillsDir, { withFileTypes: true })
        .filter(e => e.isDirectory() && e.name.startsWith('eclusa-')).length;
      if (count > 0) {
    ...
    const skillsDir = path.join(targetDir, 'skills');
    const eclusaSrc = path.join(src, 'commands', 'eclusa');
    copyCommandsAsCursorSkills(eclusaSrc, skillsDir, 'eclusa', pathPrefix, runtime);
    ...
    const skillsDir = path.join(targetDir, 'skills');
    const eclusaSrc = path.join(src, 'commands', 'eclusa');
    copyCommandsAsWindsurfSkills(eclusaSrc, skillsDir, 'eclusa', pathPrefix, runtime);
    const installedSkillNames = listCodexSkillNames(skillsDir); // reuse — same dir structure
    if (installedSkillNames.length > 0) {
      console.log(`  ${green}✓${reset} Installed ${installedSkillNames.length} skills to skills/`);
    } else {
    ...
    const eclusaSrc = path.join(src, 'commands', 'eclusa');
    const eclusaDest = path.join(commandsDir, 'eclusa');
    copyWithPathReplacement(eclusaSrc, eclusaDest, pathPrefix, runtime, true, isGlobal);
    if (verifyInstalled(eclusaDest, 'commands/eclusa')) {
      console.log(`  ${green}✓${reset} Installed commands/eclusa`);
    } else {
      failures.push('commands/eclusa');
    }
  }

  // Copy eclusa skill with path replacement
  const skillSrc = path.join(src, 'eclusa');
  const skillDest = path.join(targetDir, 'eclusa');
  copyWithPathReplacement(skillSrc, skillDest, pathPrefix, runtime, false, isGlobal);
  if (verifyInstalled(skillDest, 'eclusa')) {
    console.log(`  ${green}✓${reset} Installed eclusa`);
  } else {
    failures.push('eclusa');
  }
  ...
  if (fs.existsSync(agentsSrc)) {
    ...
    // Remove old Eclusa agents (eclusa-*.md) before copying new ones
  ...
  const changelogDest = path.join(targetDir, 'eclusa', 'CHANGELOG.md');
  ...
  // Write VERSION file
  const versionDest = path.join(targetDir, 'eclusa', 'VERSION');
  ...
  if (!isCodex && !isCopilot && !isCursor && !isWindsurf) {
    // Write package.json to force CommonJS mode for Eclusa scripts
    ...
    if (fs.existsSync(hooksSrc)) {
      ...
      for (const entry of hookEntries) {
        ...
        if (fs.statSync(srcFile).isFile()) {
          ...
          // and stamp the current Eclusa version into the hook version header
          if (entry.endsWith('.js')) {
            let content = fs.readFileSync(srcFile, 'utf8');
            content = content.replace(/'\.claude'/g, configDirReplacement);
            content = content.replace(/\{\{ECLUSA_VERSION\}\}/g, pkg.version);
  ...
  // targetDir is e.g. ~/.claude/eclusa/, parent is the config dir
  const updateCacheFile = path.join(path.dirname(targetDir), 'cache', 'eclusa-update-check.json');
  ...
  if (isCodex) {
    ...
    try {
      ...
      const updateCheckScript = path.resolve(targetDir, 'eclusa', 'hooks', 'eclusa-update-check.js').replace(/\\/g, '/');
      const hookBlock =
        `${eol}# Eclusa Hooks${eol}` +
        `[[hooks]]${eol}` +
        `event = "SessionStart"${eol}` +
        `command = "node ${updateCheckScript}"${eol}`;

      if (hasEnabledCodexHooksFeature(configContent) && !configContent.includes('eclusa-update-check')) {
  ...
  if (isCopilot) {
    ...
    const templatePath = path.join(targetDir, 'eclusa', 'templates', 'copilot-instructions.md');
    const instructionsPath = path.join(targetDir, 'copilot-instructions.md');
  ...
  const statuslineCommand = isGlobal
    ? buildHookCommand(targetDir, 'eclusa-statusline.js')
    : 'node ' + dirName + '/hooks/eclusa-statusline.js';
  const updateCheckCommand = isGlobal
    ? buildHookCommand(targetDir, 'eclusa-check-update.js')
    : 'node ' + dirName + '/hooks/eclusa-check-update.js';
  const contextMonitorCommand = isGlobal
    ? buildHookCommand(targetDir, 'eclusa-context-monitor.js')
    : 'node ' + dirName + '/hooks/eclusa-context-monitor.js';
  const promptGuardCommand = isGlobal
    ? buildHookCommand(targetDir, 'eclusa-prompt-guard.js')
    : 'node ' + dirName + '/hooks/eclusa-prompt-guard.js';
  ...
  if (!isOpencode) {
    ...
    const hasEclusaUpdateHook = settings.hooks.SessionStart.some(entry =>
      entry.hooks && entry.hooks.some(h => h.command && h.command.includes('eclusa-check-update'))
    );

    if (!hasEclusaUpdateHook) {
    ...
    const hasContextMonitorHook = settings.hooks[postToolEvent].some(entry =>
      entry.hooks && entry.hooks.some(h => h.command && h.command.includes('eclusa-context-monitor'))
    );
    ...
    if (!hasContextMonitorHook) {
      ...
      for (const entry of settings.hooks[postToolEvent]) {
        if (entry.hooks && entry.hooks.some(h => h.command && h.command.includes('eclusa-context-monitor'))) {
          let migrated = false;
          if (!entry.matcher) {
          ...
          for (const h of entry.hooks) {
            if (h.command && h.command.includes('eclusa-context-monitor') && !h.timeout) {
              h.timeout = 10;
              migrated = true;
            }
          }
    ...
    const hasPromptGuardHook = settings.hooks[preToolEvent].some(entry =>
      entry.hooks && entry.hooks.some(h => h.command && h.command.includes('eclusa-prompt-guard'))
    );
...
function finishInstall(settingsPath, settings, statuslineCommand, shouldInstallStatusline, runtime = 'claude', isGlobal = true) {
  ...
  // For non-Claude runtimes, set resolve_model_ids: "omit" in ~/.eclusa/defaults.json
  ...
  // See #1156.
  if (runtime !== 'claude') {
    const eclusaDir = path.join(os.homedir(), '.eclusa');
    const defaultsPath = path.join(eclusaDir, 'defaults.json');
    try {
      ...
      if (defaults.resolve_model_ids !== 'omit') {
        ...
        console.log(`  ${green}✓${reset} Set resolve_model_ids: "omit" in ~/.eclusa/defaults.json`);
      }
    } catch (e) {
      console.log(`  ${yellow}⚠${reset} Could not write ~/.eclusa/defaults.json: ${e.message}`);
    }
  }
  ...
  let command = '/eclusa:new-project';
  if (runtime === 'opencode') command = '/eclusa-new-project';
  if (runtime === 'codex') command = '$eclusa-new-project';
  if (runtime === 'copilot') command = '/eclusa-new-project';
  if (runtime === 'antigravity') command = '/eclusa-new-project';
  if (runtime === 'cursor') command = 'eclusa-new-project (mention the skill name)';
  console.log(`
  ${green}Done!${reset} Open a blank directory in ${program} and run ${cyan}${command}${reset}.

  ${cyan}Join the community:${reset} https://discord.gg/eclusa
`);
}
...
function handleStatusline(settings, isInteractive, callback) {
  ...
  Eclusa includes a statusline showing:
    • Model name
    • Current task (from todo list)
    • Context window usage (color-coded)

  ${cyan}1${reset}) Keep existing
  ${cyan}2${reset}) Replace with Eclusa statusline
`);
...
/**
 * Install the Eclusa SDK globally via npm.
 * @returns {boolean} true if install succeeded
 */
function installSdk() {
  const sdkVersion = pkg.version;
  const sdkPkg = `@eclusa/sdk@${sdkVersion}`;
  console.log(`\n  ${cyan}Installing Eclusa SDK...${reset}`);
  console.log(`  ${dim}npm install -g ${sdkPkg}${reset}\n`);
  try {
    require('child_process').execSync(`npm install -g ${sdkPkg}`, { stdio: 'inherit' });
    console.log(`\n  ${green}✓${reset} Eclusa SDK installed (${cyan}eclusa-sdk${reset} command available)`);
    return true;
  } catch (e) {
...
/**
 * Prompt the user to optionally install the Eclusa SDK.
...
function promptSdk(callback) {
  ...
  console.log(`
  ${yellow}Also install the Eclusa SDK?${reset}

  The SDK provides a standalone CLI for autonomous execution:
    ${dim}eclusa-sdk init @prd.md${reset}    Bootstrap a project from a PRD
    ${dim}eclusa-sdk auto${reset}            Run full autonomous lifecycle
    ${dim}eclusa-sdk run "prompt"${reset}    Execute a milestone from text

  ${cyan}1${reset}) No
  ${cyan}2${reset}) Yes ${dim}(runs: npm install -g @eclusa/sdk)${reset}
`);
...
/**
 * Install Eclusa for all selected runtimes
 */
...
if (process.env.ECLUSA_TEST_MODE) {
  module.exports = {
    yamlIdentifier,
    ...
    ECLUSA_COPILOT_INSTRUCTIONS_MARKER,
    ECLUSA_COPILOT_INSTRUCTIONS_CLOSE_MARKER,
    mergeCopilotInstructions,
    stripEclusaFromCopilotInstructions,
...
} // end of else block for ECLUSA_TEST_MODE
