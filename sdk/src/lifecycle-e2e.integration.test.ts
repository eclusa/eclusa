/**
 * E2E lifecycle integration test — proves Eclusa.runPhase() drives
 * the full phase lifecycle: discuss → research → plan → execute → verify → advance
 * after bootstrapping a real project via InitRunner.
 *
 * This is the capstone proof that `eclusa-sdk auto` works end-to-end
 * without human intervention. InitRunner bootstraps the project,
 * then Eclusa.runPhase() drives Phase 1 through the complete lifecycle.
 *
...
import type { EclusaEvent, PhaseRunnerResult, RoadmapAnalysis } from './types.js';
...
describe.skipIf(!cliAvailable)('E2E Lifecycle: InitRunner → Eclusa.runPhase() full lifecycle', () => {
  let tmpDir: string;
  ...
  beforeAll(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'eclusa-sdk-lifecycle-e2e-'));
    ...
    const initEventStream = new EclusaEventStream();
    ...
    if (initSuccess) {
      try {
        ...
        try {
          const phasesDir = join(tmpDir, '.eclusa', 'phases');
  ...
  it('Eclusa.runPhase() drives Phase 1 through the full lifecycle without human intervention', async () => {
    ...
    const roadmapPath = join(tmpDir, '.eclusa', 'ROADMAP.md');
    ...
    // Construct Eclusa with autoMode: true
    const eclusa = new Eclusa({
      projectDir: tmpDir,
      autoMode: true,
    });
    eclusa.onEvent((e: EclusaEvent) => events.push(e));

    // Run the discovered first phase with tight budget to minimize cost
    const result: PhaseRunnerResult = await eclusa.runPhase(phase1Number!, {
      maxTurnsPerStep: 10,
    ...
    if (phaseComplete.type === EclusaEventType.PhaseComplete) {
    ...
    const stepStartEvents = events.filter(
      (e): e is Extract<EclusaEvent, { type: EclusaEventType.PhaseStepStart }> =>
        e.type === EclusaEventType.PhaseStepStart,
    );
    ...
    const stepCompleteEvents = events.filter(
      (e): e is Extract<EclusaEvent, { type: EclusaEventType.PhaseStepComplete }> =>
        e.type === EclusaEventType.PhaseStepComplete,
    );
