/**
 * Eclusa Tools Bridge — shells out to `eclusa-tools.cjs` for state management.
 *
 * All `.eclusa/` state operations go through eclusa-tools.cjs rather than
...
export class EclusaToolsError extends Error {
  constructor(
    message: string,
...
export class EclusaTools {
  ...
  constructor(opts: {
    ...
    this.eclusaToolsPath =
      opts.eclusaToolsPath ?? resolveEclusaToolsPath(opts.projectDir);
  ...
  /**
   * Execute a eclusa-tools command and return parsed JSON output.
   * Handles the `@file:` prefix pattern for large results.
   */
  async exec(command: string, args: string[] = []): Promise<unknown> {
    const fullArgs = [this.eclusaToolsPath, command, ...args];
    ...
    return new Promise<unknown>((resolve, reject) => {
        ...
        async (error, stdout, stderr) => {
          ...
          if (error) {
            ...
            if (error.killed || (error as NodeJS.ErrnoException).code === 'ETIMEDOUT') {
              reject(
                new EclusaToolsError(
                  `eclusa-tools timed out after ${this.timeoutMs}ms: ${command} ${args.join(' ')}`,
                  command,
                  args,
                  null,
                  stderrStr,
            ...
            reject(
              new EclusaToolsError(
                `eclusa-tools exited with code ${error.code ?? 'unknown'}: ${command} ${args.join(' ')}${stderrStr ? `\n${stderrStr}` : ''}`,
                command,
                args,
          ...
          try {
            ...
            reject(
              new EclusaToolsError(
                `Failed to parse eclusa-tools output for "${command}": ${parseErr instanceof Error ? parseErr.message : String(parseErr)}\nRaw output: ${raw.slice(0, 500)}`,
                command,
                args,
                0,
                stderrStr,
      ...
      child.on('error', (err) => {
        reject(
          new EclusaToolsError(
            `Failed to execute eclusa-tools: ${err.message}`,
            command,
            args,
            null,
            '',
  ...
  /**
   * Parse eclusa-tools output, handling `@file:` prefix.
   */
  ...
  /**
   * Execute a eclusa-tools command and return raw stdout without JSON parsing.
   * Use for commands like `config-set` that return plain text, not JSON.
   */
  async execRaw(command: string, args: string[] = []): Promise<string> {
    const fullArgs = [this.eclusaToolsPath, command, ...args, '--raw'];
  ...
  /**
   * Query phase state from eclusa-tools.cjs `init phase-op`.
  ...
  /**
   * Get a config value from eclusa-tools.cjs.
   */
  ...
  /**
   * Begin phase state tracking in eclusa-tools.cjs.
   */
  ...
  /**
   * Query new-project init state from eclusa-tools.cjs `init new-project`.
  ...
  /**
   * Set a config value via eclusa-tools.cjs `config-set`.
   * Handles type coercion (booleans, numbers, JSON) on the eclusa-tools side.
...
/**
 * Resolve eclusa-tools.cjs path with repo-local fallback.
 * Probe order: repo-local → global home directory.
 */
export function resolveEclusaToolsPath(projectDir: string): string {
  const localPath = join(projectDir, '.claude', 'eclusa', 'bin', 'eclusa-tools.cjs');
  if (existsSync(localPath)) return localPath;
  return join(homedir(), '.claude', 'eclusa', 'bin', 'eclusa-tools.cjs');
}
