...
import { EclusaTools, EclusaToolsError } from './eclusa-tools.js';
import {
  PhaseStepType,
  EclusaEventType,
  PhaseType,
  ...
  type EclusaPhaseStartEvent,
  type EclusaPhaseStepStartEvent,
  type EclusaPhaseStepCompleteEvent,
  type EclusaPhaseCompleteEvent,
} from './types.js';
...
describe('Phase lifecycle types', () => {
  ...
  // ─── EclusaEventType phase lifecycle values ───────────────────────────────

  describe('EclusaEventType phase lifecycle events', () => {
    it('includes PhaseStart', () => {
      expect(EclusaEventType.PhaseStart).toBe('phase_start');
    });

    it('includes PhaseStepStart', () => {
      expect(EclusaEventType.PhaseStepStart).toBe('phase_step_start');
    });

    it('includes PhaseStepComplete', () => {
      expect(EclusaEventType.PhaseStepComplete).toBe('phase_step_complete');
    });

    it('includes PhaseComplete', () => {
      expect(EclusaEventType.PhaseComplete).toBe('phase_complete');
    });
  });
  ...
  describe('PhaseOpInfo interface', () => {
    it('accepts a valid phase-op output object', () => {
      const info: PhaseOpInfo = {
        phase_found: true,
        phase_dir: '.eclusa/phases/05-Skill-Scaffolding',
        phase_number: '5',
        ...
        commit_docs: true,
        context_path: '.eclusa/phases/05-Skill-Scaffolding/CONTEXT.md',
        research_path: '.eclusa/phases/05-Skill-Scaffolding/RESEARCH.md',
      };
    ...
    it('matches the documented init phase-op JSON shape', () => {
      // Simulate parsing JSON from eclusa-tools.cjs
      const raw = JSON.parse(JSON.stringify({
        phase_found: true,
        phase_dir: '.eclusa/phases/03-Auth',
        phase_number: '3',
        phase_name: 'Auth',
        phase_slug: 'auth',
        padded_phase: '03',
        ...
        planning_exists: true,
        commit_docs: true,
        context_path: '.eclusa/phases/03-Auth/CONTEXT.md',
        research_path: '.eclusa/phases/03-Auth/RESEARCH.md',
      }));
  ...
  describe('Phase lifecycle event interfaces', () => {
    it('EclusaPhaseStartEvent has correct shape', () => {
      const event: EclusaPhaseStartEvent = {
    ...
    it('EclusaPhaseStepStartEvent has correct shape', () => {
      const event: EclusaPhaseStepStartEvent = {
    ...
    it('EclusaPhaseStepCompleteEvent has correct shape', () => {
      const event: EclusaPhaseStepCompleteEvent = {
    ...
    it('EclusaPhaseStepCompleteEvent can include error', () => {
      const event: EclusaPhaseStepCompleteEvent = {
        type: EclusaEventType.PhaseStepComplete,
        timestamp: new Date().toISOString(),
        sessionId: 'test-session',
    ...
    it('EclusaPhaseCompleteEvent has correct shape', () => {
      const event: EclusaPhaseCompleteEvent = {
...
// ─── EclusaTools typed methods ──────────────────────────────────────────────────

describe('EclusaTools typed methods', () => {
  let tmpDir: string;
  let fixtureDir: string;

  beforeEach(async () => {
    tmpDir = join(tmpdir(), `eclusa-tools-phase-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    fixtureDir = join(tmpDir, 'fixtures');
    await mkdir(fixtureDir, { recursive: true });
    await mkdir(join(tmpDir, '.eclusa'), { recursive: true });
  });
  ...
  describe('initPhaseOp()', () => {
    it('returns typed PhaseOpInfo from eclusa-tools output', async () => {
      ...
      const tools = new EclusaTools({ projectDir: tmpDir, eclusaToolsPath: scriptPath });
  ...
  describe('configGet()', () => {
    it('returns string value from eclusa-tools config', async () => {
      const scriptPath = await createScript(
        'config-get.cjs',
        `
        const args = process.argv.slice(2);
