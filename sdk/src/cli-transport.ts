/**
 * CLI Transport — renders Eclusa events as rich ANSI-colored output to a Writable stream.
 *
...
import { EclusaEventType, type EclusaEvent, type TransportHandler } from './types.js';
...
export class CLITransport implements TransportHandler {
  ...
  /** Format and write a Eclusa event as a rich ANSI-colored line. Never throws. */
  ...
  private formatEvent(event: EclusaEvent): string {
    const time = formatTime(event.timestamp);

    switch (event.type) {
      case EclusaEventType.SessionInit:
        return `[${time}] [INIT] Session started — model: ${event.model}, tools: ${event.tools.length}, cwd: ${event.cwd}`;

      case EclusaEventType.SessionComplete:
      ...
      case EclusaEventType.SessionError:
        return `[${time}] ${RED}✗ Session failed — subtype: ${event.errorSubtype}, errors: [${event.errors.join(', ')}]${RESET}`;

      case EclusaEventType.ToolCall:
        return `[${time}] [TOOL] ${event.toolName}(${truncate(JSON.stringify(event.input), 80)})`;

      case EclusaEventType.PhaseStart:
        return `${BOLD}${CYAN}━━━ Eclusa ► PHASE ${event.phaseNumber}: ${event.phaseName} ━━━${RESET}`;

      case EclusaEventType.PhaseComplete:
      ...
      case EclusaEventType.CostUpdate: {
      ...
      case EclusaEventType.MilestoneStart:
        return `${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n${BOLD}  Eclusa Milestone — ${event.phaseCount} phases${RESET}\n${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}`;

      case EclusaEventType.MilestoneComplete:
        return `${BOLD}━━━ Milestone complete — success: ${event.success}, cost: ${usd(event.totalCostUsd)}, running: ${usd(this.runningCostUsd)} ━━━${RESET}`;

      case EclusaEventType.AssistantText:
        return `${DIM}[${time}] ${truncate(event.text, 200)}${RESET}`;

      case EclusaEventType.InitResearchSpawn:
