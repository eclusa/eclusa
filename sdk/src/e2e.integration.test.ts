...
import { Eclusa, parsePlanFile, EclusaEventType } from './index.js';
import type { EclusaEvent } from './index.js';
...
describe.skipIf(!cliAvailable)('E2E: Single plan execution', () => {
  ...
  beforeAll(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'eclusa-sdk-e2e-'));
  ...
  it('executes a single plan and returns a valid PlanResult', async () => {
    const eclusa = new Eclusa({ projectDir: tmpDir, maxBudgetUsd: 1.0, maxTurns: 20 });
    const result = await eclusa.executePlan('sample-plan.md');
    ...
    expect(outputContent).toContain('hello from eclusa-sdk');
  ...
  it('proves session isolation (R014) — different session IDs for sequential runs', async () => {
    ...
    const tmpDir2 = await mkdtemp(join(tmpdir(), 'eclusa-sdk-e2e-'));
    await cp(fixturesDir, tmpDir2, { recursive: true });

    try {
      const eclusa1 = new Eclusa({ projectDir: tmpDir, maxBudgetUsd: 1.0, maxTurns: 20 });
      const eclusa2 = new Eclusa({ projectDir: tmpDir2, maxBudgetUsd: 1.0, maxTurns: 20 });

      const result1 = await eclusa1.executePlan('sample-plan.md');
      const result2 = await eclusa2.executePlan('sample-plan.md');
...
describe.skipIf(!cliAvailable)('E2E: Event stream during plan execution (R007)', () => {
  ...
  beforeAll(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'eclusa-sdk-e2e-stream-'));
    await cp(fixturesDir, tmpDir, { recursive: true });
  });
  ...
  it('event stream emits events during plan execution (R007)', async () => {
    ...
    const initEvents = events.filter(e => e.type === EclusaEventType.SessionInit);
    expect(initEvents.length).toBeGreaterThanOrEqual(1);

    // (b) At least one tool_call event received
    const toolCallEvents = events.filter(e => e.type === EclusaEventType.ToolCall);
    expect(toolCallEvents.length).toBeGreaterThanOrEqual(1);

    // (c) Exactly one session_complete event with cost >= 0
    const completeEvents = events.filter(e => e.type === EclusaEventType.SessionComplete);
    expect(completeEvents).toHaveLength(1);
    const completeEvent = completeEvents[0]!;
    if (completeEvent.type === EclusaEventType.SessionComplete) {
    ...
    const initIdx = events.findIndex(e => e.type === EclusaEventType.SessionInit);
    const toolCallIdx = events.findIndex(e => e.type === EclusaEventType.ToolCall);
    const completeIdx = events.findIndex(e => e.type === EclusaEventType.SessionComplete);
    ...
    const costEvents = events.filter(e => e.type === EclusaEventType.CostUpdate);
...
describe('E2E: Error handling', () => {
  it('returns failure for nonexistent plan path', async () => {
    const tmpDir = await mkdtemp(join(tmpdir(), 'eclusa-sdk-e2e-err-'));

    try {
      const eclusa = new Eclusa({ projectDir: tmpDir });
      await expect(eclusa.executePlan('nonexistent-plan.md')).rejects.toThrow();
    } finally {
      await rm(tmpDir, { recursive: true, force: true });
    }
  });
