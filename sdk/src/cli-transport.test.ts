...
import { EclusaEventType, type EclusaEvent, type EclusaEventBase } from './types.js';
...
function makeBase(overrides: Partial<EclusaEventBase> = {}): Omit<EclusaEventBase, 'type'> {
  return {
...
describe('CLITransport', () => {
  ...
  it('formats PhaseStart as bold cyan banner and PhaseComplete with running cost', () => {
    ...
    expect(lines[0]).toBe(`${BOLD}${CYAN}━━━ Eclusa ► PHASE 01: Authentication ━━━${RESET}`);
  ...
  it('formats MilestoneStart as bold banner and MilestoneComplete with running cost', () => {
    ...
    expect(lines[1]).toBe(`${BOLD}  Eclusa Milestone — 3 phases${RESET}`);
