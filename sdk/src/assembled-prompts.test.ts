...
import type { ParsedPlan, ContextFiles, EclusaEvent } from './types.js';
...
describe('InitRunner assembled output', () => {
  ...
  const stubTools: EclusaTools = {
    ...
    configSet: async () => {},
    commit: async () => {},
  } as unknown as EclusaTools;

  const stubEventStream: EclusaEventStream = {
    emitEvent: (_event: EclusaEvent) => {},
  } as unknown as EclusaEventStream;

  beforeAll(async () => {
    // Create temp directory with .eclusa/ structure for InitRunner file reads
  ...
  describe('buildRoadmapPrompt', () => {
    ...
    it('contains agent definition content', () => {
      // Roadmap prompt loads eclusa-roadmapper.md
