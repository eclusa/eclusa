/**
 * Eclusa Event Stream — maps SDKMessage variants to typed Eclusa events.
 *
 * Extends EventEmitter to provide a typed event bus. Includes:
 * - SDKMessage → EclusaEvent mapping
...
import {
  ...
  type EclusaRateLimitEvent as EclusaRateLimitEventType,
...
export class EclusaEventStream extends EventEmitter {
  ...
  /** Emit a typed Eclusa event to all listeners and transports. */
  emitEvent(event: EclusaEvent): void {
  ...
  /**
   * Map an SDKMessage to a EclusaEvent.
   * Returns null for non-actionable message types (user messages, replays, etc.).
   */
  mapSDKMessage(msg: SDKMessage, context: EventStreamContext = {}): EclusaEvent | null {
    const base = {
   ...
   */
  mapAndEmit(msg: SDKMessage, context: EventStreamContext = {}): EclusaEvent | null {
  ...
  private mapSystemMessage(
    ...
    switch (subtype) {
      ...
      case 'task_notification': {
        ...
        } as EclusaTaskNotificationEvent;
      }
  ...
  private mapToolUseSummaryMessage(
    ...
    } as EclusaToolUseSummaryEvent;
  }

  private mapRateLimitMessage(
    msg: SDKRateLimitEvent,
    base: Omit<EclusaEvent, 'type'>,
  ): EclusaRateLimitEventType {
    return {
      ...base,
