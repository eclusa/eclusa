...
import { EclusaTools, EclusaToolsError, resolveEclusaToolsPath } from './eclusa-tools.js';
import { mkdir, writeFile, rm } from 'node:fs/promises';
import { join } from 'node:path';
import { tmpdir, homedir } from 'node:os';

describe('EclusaTools', () => {
  let tmpDir: string;
  let fixtureDir: string;

  beforeEach(async () => {
    tmpDir = join(tmpdir(), `eclusa-tools-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  describe('exec()', () => {
    ...
    it('throws EclusaToolsError on non-zero exit code', async () => {
      const scriptPath = await createScript(
        'fail.cjs',
      ...
      try {
        ...
        expect(err).toBeInstanceOf(EclusaToolsError);
        const eclusaErr = err as EclusaToolsError;
        expect(eclusaErr.command).toBe('state');
        expect(eclusaErr.args).toEqual(['load']);
        expect(eclusaErr.stderr).toContain('something went wrong');
        expect(eclusaErr.exitCode).toBeGreaterThan(0);
      }
    });

    it('throws EclusaToolsError with context when eclusa-tools.cjs not found', async () => {
      const tools = new EclusaTools({
        projectDir: tmpDir,
        eclusaToolsPath: '/nonexistent/path/eclusa-tools.cjs',
      });

      await expect(tools.exec('state', ['load'])).rejects.toThrow(EclusaToolsError);
    });
    ...
    it('throws parse error when stdout is non-JSON', async () => {
      ...
      try {
        ...
        expect(err).toBeInstanceOf(EclusaToolsError);
        const eclusaErr = err as EclusaToolsError;
        expect(eclusaErr.message).toContain('Failed to parse');
        expect(eclusaErr.message).toContain('Not JSON at all');
      }
    });
    ...
    it('handles timeout by killing child process', async () => {
      ...
      try {
        ...
        expect(err).toBeInstanceOf(EclusaToolsError);
        const eclusaErr = err as EclusaToolsError;
        expect(eclusaErr.message).toContain('timed out');
      }
    }, 10_000);
  });
  ...
  describe('initNewProject()', () => {
    it('calls init new-project and returns typed result', async () => {
      ...
      expect(result.project_path).toBe('.eclusa/PROJECT.md');
    });

    it('propagates errors from eclusa-tools', async () => {
      const scriptPath = await createScript(
        'init-fail.cjs',
      ...
      await expect(tools.initNewProject()).rejects.toThrow(EclusaToolsError);
    });
  });

  // ─── resolveEclusaToolsPath() tests ────────────────────────────────────────

  describe('resolveEclusaToolsPath()', () => {
    it('returns repo-local path when it exists', async () => {
      const localBinDir = join(tmpDir, '.claude', 'eclusa', 'bin');
      await mkdir(localBinDir, { recursive: true });
      await writeFile(join(localBinDir, 'eclusa-tools.cjs'), '// stub');

      const result = resolveEclusaToolsPath(tmpDir);
      expect(result).toBe(join(localBinDir, 'eclusa-tools.cjs'));
    });

    it('falls back to global path when repo-local does not exist', () => {
      const result = resolveEclusaToolsPath(tmpDir);
      expect(result).toBe(
        join(homedir(), '.claude', 'eclusa', 'bin', 'eclusa-tools.cjs'),
      );
    });
    ...
    it('constructor uses repo-local path when available', async () => {
      const localBinDir = join(tmpDir, '.claude', 'eclusa', 'bin');
      await mkdir(localBinDir, { recursive: true });
      const scriptPath = join(localBinDir, 'eclusa-tools.cjs');
      await writeFile(
        scriptPath,
      ...
      // No explicit eclusaToolsPath — should auto-resolve to local
      const tools = new EclusaTools({ projectDir: tmpDir });
