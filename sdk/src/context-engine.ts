/**
 * Context engine — resolves which .eclusa/ state files exist per phase type.
 *
...
export class ContextEngine {
  private readonly eclusaDir: string;
  private readonly logger?: EclusaLogger;

  constructor(projectDir: string, logger?: EclusaLogger) {
    this.eclusaDir = join(projectDir, '.eclusa');
    this.logger = logger;
  }
  ...
  async resolveContextFiles(phaseType: PhaseType): Promise<ContextFiles> {
    ...
    for (const spec of manifest) {
      const filePath = join(this.eclusaDir, spec.filename);
