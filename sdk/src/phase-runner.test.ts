...
import { PhaseStepType, PhaseType, EclusaEventType } from './types.js';
import type { EclusaConfig } from './config.js';
...
function makePhaseOp(overrides: Partial<PhaseOpInfo> = {}): PhaseOpInfo {
  return {
    phase_found: true,
    phase_dir: '/tmp/project/.eclusa/phases/01-auth',
    phase_number: '1',
    ...
    commit_docs: true,
    context_path: '/tmp/project/.eclusa/phases/01-auth/CONTEXT.md',
    research_path: '/tmp/project/.eclusa/phases/01-auth/RESEARCH.md',
    ...overrides,
  };
}
...
function makeConfig(overrides: Partial<EclusaConfig> = {}): EclusaConfig {
  return {
...
describe('PhaseRunner', () => {
  ...
  describe('phase lifecycle events', () => {
    it('emits events in correct order', async () => {
      ...
      expect(eventTypes[0]).toBe(EclusaEventType.PhaseStart);

      // Last event: phase_complete
      expect(eventTypes[eventTypes.length - 1]).toBe(EclusaEventType.PhaseComplete);

      // Each step has start + complete pair
      const stepStarts = events.filter(e => e.type === EclusaEventType.PhaseStepStart);
      const stepCompletes = events.filter(e => e.type === EclusaEventType.PhaseStepComplete);
    ...
    it('phase_start event contains correct phaseNumber and phaseName', async () => {
      ...
      const events = getEmittedEvents(deps);
      const phaseStart = events.find(e => e.type === EclusaEventType.PhaseStart) as any;
    ...
    it('phase_complete event reports success and step count', async () => {
      ...
      const events = getEmittedEvents(deps);
      const phaseComplete = events.find(e => e.type === EclusaEventType.PhaseComplete) as any;
    ...
    it('step_start events include correct step type', async () => {
      ...
      const stepStarts = events
        .filter(e => e.type === EclusaEventType.PhaseStepStart)
        .map(e => (e as any).step);
  ...
  describe('error propagation', () => {
    ...
    it('throws PhaseRunnerError when initPhaseOp fails', async () => {
      ...
      (deps.tools.initPhaseOp as ReturnType<typeof vi.fn>).mockRejectedValue(
        new Error('eclusa-tools crashed'),
      );
  ...
  describe('advance step', () => {
    ...
    it('captures phaseComplete errors without throwing', async () => {
      ...
      (deps.tools.phaseComplete as ReturnType<typeof vi.fn>).mockRejectedValue(
        new Error('eclusa-tools commit failed'),
      );
  ...
  describe('wave-grouped parallel execution', () => {
    ...
    it('emits wave_start and wave_complete events with correct data', async () => {
      ...
      const events = getEmittedEvents(deps);
      const waveStarts = events.filter(e => e.type === EclusaEventType.WaveStart) as any[];
      const waveCompletes = events.filter(e => e.type === EclusaEventType.WaveComplete) as any[];
    ...
    it('no wave events emitted when parallelization is disabled', async () => {
      ...
      const waveEvents = events.filter(
        e => e.type === EclusaEventType.WaveStart || e.type === EclusaEventType.WaveComplete,
      );
  ...
  describe('plan-check step', () => {
    ...
    it('plan-check emits PhaseStepStart and PhaseStepComplete events', async () => {
      ...
      const planCheckStarts = events.filter(
        e => e.type === EclusaEventType.PhaseStepStart && (e as any).step === PhaseStepType.PlanCheck,
      );
      const planCheckCompletes = events.filter(
        e => e.type === EclusaEventType.PhaseStepComplete && (e as any).step === PhaseStepType.PlanCheck,
      );
