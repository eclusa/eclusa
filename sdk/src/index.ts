/**
 * Eclusa SDK — Public API for running Eclusa plans programmatically.
 *
 * The Eclusa class composes plan parsing, config loading, prompt building,
 * and session running into a single `executePlan()` call.
 *
 * @example
 * ```typescript
 * import { Eclusa } from '@eclusa/sdk';
 *
 * const eclusa = new Eclusa({ projectDir: '/path/to/project' });
 * const result = await eclusa.executePlan('.eclusa/phases/01-auth/01-auth-01-PLAN.md');
 *
 * if (result.success) {
...
import type { EclusaOptions, PlanResult, SessionOptions, EclusaEvent, TransportHandler, PhaseRunnerOptions, PhaseRunnerResult, MilestoneRunnerOptions, MilestoneRunnerResult, RoadmapPhaseInfo } from './types.js';
import { EclusaEventType } from './types.js';
import { parsePlan, parsePlanFile } from './plan-parser.js';
import { loadConfig } from './config.js';
import { EclusaTools, resolveEclusaToolsPath } from './eclusa-tools.js';
...
export class Eclusa {
  ...
  private readonly eclusaToolsPath: string;
  private readonly defaultModel?: string;
  private readonly defaultMaxBudgetUsd: number;
  private readonly defaultMaxTurns: number;
  private readonly autoMode: boolean;
  readonly eventStream: EclusaEventStream;

  constructor(options: EclusaOptions) {
    this.projectDir = resolve(options.projectDir);
    this.eclusaToolsPath =
      options.eclusaToolsPath ?? resolveEclusaToolsPath(this.projectDir);
    ...
    this.eventStream = new EclusaEventStream();
  }

  /**
   * Execute a single Eclusa plan file.
   *
  ...
  /**
   * Subscribe a simple handler to receive all Eclusa events.
   */
  onEvent(handler: (event: EclusaEvent) => void): void {
    this.eventStream.on('event', handler);
  }

  /**
   * Subscribe a transport handler to receive all Eclusa events.
  ...
  /**
   * Create a EclusaTools instance for state management operations.
   */
  createTools(): EclusaTools {
    return new EclusaTools({
      projectDir: this.projectDir,
      eclusaToolsPath: this.eclusaToolsPath,
    });
  }
   ...
   *
   * Creates the necessary collaborators (EclusaTools, PromptFactory, ContextEngine),
  ...
  /**
   * Load the eclusa-executor agent definition if available.
   * Falls back gracefully — returns undefined if not found.
   */
  private async loadAgentDefinition(): Promise<string | undefined> {
    const paths = [
      // Repo-local Eclusa installation
      join(this.projectDir, '.claude', 'eclusa', 'agents', 'eclusa-executor.md'),
      // Repo-local agents directory
      join(this.projectDir, '.claude', 'agents', 'eclusa-executor.md'),
      // Global home directory
      join(homedir(), '.claude', 'agents', 'eclusa-executor.md'),
      join(this.projectDir, 'agents', 'eclusa-executor.md'),
    ];
...
export type { EclusaConfig } from './config.js';
export { EclusaTools, EclusaToolsError, resolveEclusaToolsPath } from './eclusa-tools.js';
...
export { EclusaEventStream } from './event-stream.js';
...
export { EclusaLogger } from './logger.js';
export type { LogLevel, LogEntry, EclusaLoggerOptions } from './logger.js';
