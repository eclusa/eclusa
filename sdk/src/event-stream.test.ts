...
import { EclusaEventStream } from './event-stream.js';
import {
  EclusaEventType,
  PhaseType,
  type EclusaEvent,
  type EclusaSessionInitEvent,
  type EclusaSessionCompleteEvent,
  type EclusaSessionErrorEvent,
  type EclusaAssistantTextEvent,
  type EclusaToolCallEvent,
  type EclusaToolProgressEvent,
  type EclusaToolUseSummaryEvent,
  type EclusaTaskStartedEvent,
  type EclusaTaskProgressEvent,
  type EclusaTaskNotificationEvent,
  type EclusaAPIRetryEvent,
  type EclusaRateLimitEvent,
  type EclusaStatusChangeEvent,
  type EclusaCompactBoundaryEvent,
  type EclusaStreamEvent,
  type EclusaCostUpdateEvent,
...
// ─── SDKMessage → EclusaEvent mapping tests ─────────────────────────────────────

describe('EclusaEventStream', () => {
  let stream: EclusaEventStream;

  beforeEach(() => {
    stream = new EclusaEventStream();
  });
  ...
  describe('mapSDKMessage', () => {
    it('maps SDKSystemMessage init → SessionInit', () => {
      ...
      expect(event!.type).toBe(EclusaEventType.SessionInit);

      const init = event as EclusaSessionInitEvent;
    ...
    it('maps assistant text blocks → AssistantText', () => {
      ...
      expect(event).not.toBeNull();
      expect(event!.type).toBe(EclusaEventType.AssistantText);
      expect((event as EclusaAssistantTextEvent).text).toBe('Hello world');
    });
    ...
    it('maps assistant tool_use blocks → ToolCall', () => {
      ...
      const event = stream.mapSDKMessage(msg);
      expect(event).not.toBeNull();
      expect(event!.type).toBe(EclusaEventType.ToolCall);

      const tc = event as EclusaToolCallEvent;
    ...
    it('handles multi-block assistant messages (text + tool_use)', () => {
      ...
      stream.on('event', (e: EclusaEvent) => events.push(e));
      ...
      expect(events[0]!.type).toBe(EclusaEventType.AssistantText);
      expect(events[1]!.type).toBe(EclusaEventType.ToolCall);
    });

    it('maps SDKResultSuccess → SessionComplete', () => {
      const event = stream.mapSDKMessage(makeResultSuccess());
      expect(event).not.toBeNull();
      expect(event!.type).toBe(EclusaEventType.SessionComplete);

      const complete = event as EclusaSessionCompleteEvent;
    ...
    it('maps SDKResultError → SessionError', () => {
      ...
      expect(event).not.toBeNull();
      expect(event!.type).toBe(EclusaEventType.SessionError);

      const err = event as EclusaSessionErrorEvent;
    ...
    it('maps SDKToolProgressMessage → ToolProgress', () => {
      ...
      expect(event).not.toBeNull();
      expect(event!.type).toBe(EclusaEventType.ToolProgress);

      const tp = event as EclusaToolProgressEvent;
    ...
    it('maps SDKToolUseSummaryMessage → ToolUseSummary', () => {
      ...
      expect(event).not.toBeNull();
      expect(event!.type).toBe(EclusaEventType.ToolUseSummary);

      const tus = event as EclusaToolUseSummaryEvent;
    ...
    it('maps SDKTaskStartedMessage → TaskStarted', () => {
      ...
      expect(event).not.toBeNull();
      expect(event!.type).toBe(EclusaEventType.TaskStarted);

      const ts = event as EclusaTaskStartedEvent;
    ...
    it('maps SDKTaskProgressMessage → TaskProgress', () => {
      ...
      expect(event).not.toBeNull();
      expect(event!.type).toBe(EclusaEventType.TaskProgress);

      const tp = event as EclusaTaskProgressEvent;
    ...
    it('maps SDKTaskNotificationMessage → TaskNotification', () => {
      ...
      expect(event).not.toBeNull();
      expect(event!.type).toBe(EclusaEventType.TaskNotification);

      const tn = event as EclusaTaskNotificationEvent;
    ...
    it('maps SDKAPIRetryMessage → APIRetry', () => {
      ...
      expect(event).not.toBeNull();
      expect(event!.type).toBe(EclusaEventType.APIRetry);

      const retry = event as EclusaAPIRetryEvent;
    ...
    it('maps SDKRateLimitEvent → RateLimit', () => {
      ...
      expect(event).not.toBeNull();
      expect(event!.type).toBe(EclusaEventType.RateLimit);

      const rl = event as EclusaRateLimitEvent;
    ...
    it('maps SDKStatusMessage → StatusChange', () => {
      ...
      expect(event).not.toBeNull();
      expect(event!.type).toBe(EclusaEventType.StatusChange);
      expect((event as EclusaStatusChangeEvent).status).toBe('compacting');
    });

    it('maps SDKCompactBoundaryMessage → CompactBoundary', () => {
      const event = stream.mapSDKMessage(makeCompactBoundary());
      expect(event).not.toBeNull();
      expect(event!.type).toBe(EclusaEventType.CompactBoundary);

      const cb = event as EclusaCompactBoundaryEvent;
  ...
  describe('transport management', () => {
    it('delivers events to subscribed transports', () => {
      ...
      expect(received[0]!.type).toBe(EclusaEventType.SessionInit);
    });

    it('delivers events to multiple transports', () => {
      const received1: EclusaEvent[] = [];
      const received2: EclusaEvent[] = [];
    ...
    it('survives transport.onEvent() throwing', () => {
      ...
      const goodReceived: EclusaEvent[] = [];
  ...
  describe('EventEmitter integration', () => {
    it('emits typed events via "event" channel', () => {
      ...
      expect(events).toHaveLength(2);
      expect(events[0]!.type).toBe(EclusaEventType.SessionInit);
      expect(events[1]!.type).toBe(EclusaEventType.SessionComplete);
    });

    it('emits events on per-type channels', () => {
      const initEvents: EclusaEvent[] = [];
      stream.on(EclusaEventType.SessionInit, (e: EclusaEvent) => initEvents.push(e));

      stream.mapAndEmit(makeSystemInit());
      stream.mapAndEmit(makeResultSuccess());

      expect(initEvents).toHaveLength(1);
      expect(initEvents[0]!.type).toBe(EclusaEventType.SessionInit);
    });
  });
  ...
  describe('stream_event mapping', () => {
    it('maps SDKPartialAssistantMessage → StreamEvent', () => {
      ...
      const event = stream.mapSDKMessage(msg);
      expect(event).not.toBeNull();
      expect(event!.type).toBe(EclusaEventType.StreamEvent);
      expect((event as EclusaStreamEvent).event).toEqual({ type: 'content_block_delta' });
    });
  });
