...
import { EclusaLogger } from './logger.js';
...
describe('EclusaLogger', () => {
  ...
  it('outputs valid JSON on each log call', () => {
    const logger = new EclusaLogger({ output, level: 'debug' });
  ...
  it('filters messages below minimum log level', () => {
    const logger = new EclusaLogger({ output, level: 'warn' });
  ...
  it('defaults to info level filtering', () => {
    const logger = new EclusaLogger({ output });
  ...
  it('writes to custom output stream', () => {
    ...
    const logger = new EclusaLogger({ output: customOutput, level: 'debug' });
    logger.info('custom');
