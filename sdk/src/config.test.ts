...
describe('loadConfig', () => {
  ...
  beforeEach(async () => {
    tmpDir = join(tmpdir(), `eclusa-config-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  it('loads valid config and merges with defaults', async () => {
    ...
    await writeFile(
      join(tmpDir, '.eclusa', 'config.json'),
  ...
  it('partial config merges correctly for nested objects', async () => {
    ...
    expect(config.git.phase_branch_template).toBe('eclusa/phase-{phase}-{slug}');
