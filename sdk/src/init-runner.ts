/**
 * InitRunner — orchestrates the Eclusa new-project init workflow.
 *
 ...
 * prompts derived from Eclusa-1 workflow/agent/template files on disk.
 */
...
import { EclusaEventType, PhaseStepType } from './types.js';
import type { EclusaTools } from './eclusa-tools.js';
import type { EclusaEventStream } from './event-stream.js';
...
const ECLUSA_TEMPLATES_DIR = join(homedir(), '.claude', 'eclusa', 'templates');
const ECLUSA_AGENTS_DIR = join(homedir(), '.claude', 'agents');
...
export class InitRunner {
  ...
  private readonly tools: EclusaTools;
  private readonly eventStream: EclusaEventStream;
  ...
  async run(input: string): Promise<InitResult> {
    ...
    this.emitEvent<EclusaInitStartEvent>({
    ...
    try {
      ...
      const setupResult = await this.runStep('setup', async () => {
        ...
        if (info.project_exists) {
          throw new Error('Project already exists (.eclusa/PROJECT.md found). Use a fresh directory or delete .eclusa/ first.');
        }
        return info;
      });
      ...
      const configResult = await this.runStep('config', async () => {
        ...
        // Ensure .eclusa/ directory exists
        const planningDir = join(this.projectDir, '.eclusa');
        await mkdir(planningDir, { recursive: true });

        // Write config.json
        const configPath = join(planningDir, 'config.json');
        await writeFile(configPath, JSON.stringify(AUTO_MODE_CONFIG, null, 2) + '\n', 'utf-8');
        artifacts.push('.eclusa/config.json');

        // Persist auto_advance via eclusa-tools (validates & updates state)
        await this.tools.configSet('workflow.auto_advance', 'true');

        // Commit config
        if (projectInfo.commit_docs) {
          await this.tools.commit('chore: add project config', ['.eclusa/config.json']);
        }
      });
      ...
      const projectResult = await this.runStep('project', async () => {
        ...
        artifacts.push('.eclusa/PROJECT.md');
        if (projectInfo.commit_docs) {
          await this.tools.commit('docs: add PROJECT.md', ['.eclusa/PROJECT.md']);
        }
        return result;
      });
      ...
      const synthResult = await this.runStep('synthesis', async () => {
        ...
        artifacts.push('.eclusa/research/SUMMARY.md');
        if (projectInfo.commit_docs) {
          await this.tools.commit('docs: add research files', ['.eclusa/research/']);
        }
        return result;
      });
      ...
      const reqResult = await this.runStep('requirements', async () => {
        ...
        artifacts.push('.eclusa/REQUIREMENTS.md');
        if (projectInfo.commit_docs) {
          await this.tools.commit('docs: add REQUIREMENTS.md', ['.eclusa/REQUIREMENTS.md']);
        }
        return result;
      });
      ...
      const roadmapResult = await this.runStep('roadmap', async () => {
        ...
        artifacts.push('.eclusa/ROADMAP.md', '.eclusa/STATE.md');
        if (projectInfo.commit_docs) {
  ...
  private async runStep<T>(
    ...
    this.emitEvent<EclusaInitStepStartEvent>({
    ...
    try {
      ...
      this.emitEvent<EclusaInitStepCompleteEvent>({
  ...
  private async runParallelResearch(
    ...
    this.emitEvent<EclusaInitResearchSpawnEvent>({
    ...
    const promises = RESEARCH_TYPES.map(async (researchType) => {
      ...
      if (result.stepResult.success) {
        result.stepResult.artifacts = [`.eclusa/research/${researchType}.md`];
      }
  ...
  private async buildProjectPrompt(input: string): Promise<string> {
    const template = await this.readEclusaFile('templates/project.md');

    return sanitizePrompt([
      'You are creating the PROJECT.md for a new software project.',
      'Write .eclusa/PROJECT.md based on the template structure below and the user\'s project description.',
      '',
      '<project_template>',
      template,
      '</project_template>',
      ...
      '</user_input>',
      '',
      'Write the file to .eclusa/PROJECT.md. Follow the template structure but fill in with real content derived from the user input.',
  ...
  private async buildResearchPrompt(
    ...
    const agentDef = await this.readAgentFile('eclusa-project-researcher.md');
    const template = await this.readEclusaFile(`templates/research-project/${researchType}.md`);

    // Read PROJECT.md if it exists (it should by now)
    let projectContent = '';
    try {
      projectContent = await readFile(
        join(this.projectDir, '.eclusa', 'PROJECT.md'),
        'utf-8',
      );
    } catch {
    ...
    return sanitizePrompt([
      ...
      `Write your findings to .eclusa/research/${researchType}.md`,
      '',
      '<files_to_read>',
      '.eclusa/PROJECT.md',
      '</files_to_read>',
      ...
      '</research_template>',
      '',
      `Write .eclusa/research/${researchType}.md following the template structure.`,
  ...
  private async buildSynthesisPrompt(): Promise<string> {
    const agentDef = await this.readAgentFile('eclusa-research-synthesizer.md');
    const summaryTemplate = await this.readEclusaFile('templates/research-project/SUMMARY.md');
    const researchDir = join(this.projectDir, '.eclusa', 'research');
    ...
    return sanitizePrompt([
      ...
      '</files_to_read>',
      '',
      'Synthesize the research files below into .eclusa/research/SUMMARY.md',
      '',
      ...researchContent,
      '',
      '<summary_template>',
      ...
      '</summary_template>',
      '',
      'Write .eclusa/research/SUMMARY.md synthesizing all research findings.',
      'Also commit all research files: git add .eclusa/research/ && git commit.',
    ].join('\n'));
  }
  ...
  private async buildRequirementsPrompt(): Promise<string> {
    const reqTemplate = await this.readEclusaFile('templates/requirements.md');
    ...
    try {
      featuresContent = await readFile(
        join(this.projectDir, '.eclusa', 'research', 'FEATURES.md'),
        'utf-8',
      );
    } catch {
    ...
    return sanitizePrompt([
      ...
      '</requirements_template>',
      '',
      'Write .eclusa/REQUIREMENTS.md following the template structure.',
  ...
  private async buildRoadmapPrompt(): Promise<string> {
    const agentDef = await this.readAgentFile('eclusa-roadmapper.md');
    const roadmapTemplate = await this.readEclusaFile('templates/roadmap.md');
    const stateTemplate = await this.readEclusaFile('templates/state.md');

    const filesToRead = [
      '.eclusa/PROJECT.md',
      '.eclusa/REQUIREMENTS.md',
      '.eclusa/research/SUMMARY.md',
      '.eclusa/config.json',
    ];
    ...
    return sanitizePrompt([
      ...
      '</state_template>',
      '',
      'Create .eclusa/ROADMAP.md and .eclusa/STATE.md.',
  ...
  /**
   * Read a file from the Eclusa templates directory.
   * Tries sdk/prompts/{relativePath} first (headless versions), then
   * falls back to Eclusa-1 originals (~/.claude/eclusa/).
   */
  private async readEclusaFile(relativePath: string): Promise<string> {
    ...
    try {
      ...
      // Not in sdk/prompts/, fall through to Eclusa-1 originals
    }

    // Fall back to Eclusa-1 originals
    const fullPath = join(ECLUSA_TEMPLATES_DIR, '..', relativePath);
    try {
   ...
   * falls back to Eclusa-1 originals (~/.claude/agents/).
   */
  private async readAgentFile(filename: string): Promise<string> {
    ...
    // Fall back to Eclusa-1 originals
    const fullPath = join(ECLUSA_AGENTS_DIR, filename);
    try {
      return await readFile(fullPath, 'utf-8');
    } catch {
  ...
  private emitEvent<T extends { type: EclusaEventType }>(
    partial: Omit<T, 'timestamp' | 'sessionId'> & { type: EclusaEventType },
  ): void {
    this.eventStream.emitEvent({
      timestamp: new Date().toISOString(),
      sessionId: this.sessionId,
      ...partial,
    } as unknown as import('./types.js').EclusaEvent);
  }
  ...
  private buildResult(
    ...
    this.emitEvent<EclusaInitCompleteEvent>({
