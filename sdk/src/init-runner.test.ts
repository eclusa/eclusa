...
import { EclusaEventType } from './types.js';
...
// Mock fs/promises for template reading (InitRunner reads Eclusa templates)
...
function makeProjectInfo(overrides: Partial<InitNewProjectInfo> = {}): InitNewProjectInfo {
  return {
    ...
    project_path: '.eclusa/PROJECT.md',
    ...overrides,
  };
}
...
function makeEventStream() {
  const events: EclusaEvent[] = [];
  return {
    emitEvent: vi.fn((event: EclusaEvent) => events.push(event)),
    on: vi.fn(),
    emit: vi.fn(),
    addTransport: vi.fn(),
    events,
...
function makeDeps(overrides: Partial<InitRunnerDeps> & { tmpDir: string }): InitRunnerDeps & { events: EclusaEvent[] } {
...
describe('InitRunner', () => {
  ...
  function createRunner(toolsOverrides: Record<string, unknown> = {}, configOverrides?: Partial<InitRunnerDeps['config']>) {
    ...
    return { runner, tools, eventStream, events: eventStream.events as EclusaEvent[] };
  }
  ...
  it('run() writes config.json with auto-mode defaults', async () => {
    ...
    // config.json should be written to .eclusa/config.json in tmpDir
    const configPath = join(tmpDir, '.eclusa', 'config.json');
  ...
  it('run() emits InitStart and InitComplete events', async () => {
    ...
    const startEvents = events.filter(e => e.type === EclusaEventType.InitStart);
    const completeEvents = events.filter(e => e.type === EclusaEventType.InitComplete);
  ...
  it('run() emits InitStepStart/Complete for each step', async () => {
    ...
    const stepStarts = events.filter(e => e.type === EclusaEventType.InitStepStart);
    const stepCompletes = events.filter(e => e.type === EclusaEventType.InitStepComplete);
  ...
  it('run() emits InitResearchSpawn before research sessions', async () => {
    ...
    const spawnEvents = events.filter(e => e.type === EclusaEventType.InitResearchSpawn);
  ...
  it('run() returns all expected artifacts on success', async () => {
    ...
    expect(result.artifacts).toContain('.eclusa/config.json');
    expect(result.artifacts).toContain('.eclusa/PROJECT.md');
    expect(result.artifacts).toContain('.eclusa/research/SUMMARY.md');
    expect(result.artifacts).toContain('.eclusa/REQUIREMENTS.md');
    expect(result.artifacts).toContain('.eclusa/ROADMAP.md');
    expect(result.artifacts).toContain('.eclusa/STATE.md');
  });
  ...
  it('run() includes research artifact paths on success', async () => {
    ...
    expect(result.artifacts).toContain('.eclusa/research/STACK.md');
    expect(result.artifacts).toContain('.eclusa/research/FEATURES.md');
    expect(result.artifacts).toContain('.eclusa/research/ARCHITECTURE.md');
    expect(result.artifacts).toContain('.eclusa/research/PITFALLS.md');
  });
  ...
  describe('sdkPromptsDir preference and sanitizer integration', () => {
    ...
    beforeEach(async () => {
      ...
      await writeFile(
        join(sdkPromptsDir, 'agents', 'eclusa-project-researcher.md'),
        '# Project Researcher Agent\nSDK_HEADLESS_MARKER_RESEARCHER\n',
      );
      await writeFile(
        join(sdkPromptsDir, 'agents', 'eclusa-research-synthesizer.md'),
        '# Research Synthesizer Agent\nSDK_HEADLESS_MARKER_SYNTHESIZER\n',
      );
      await writeFile(
        join(sdkPromptsDir, 'agents', 'eclusa-roadmapper.md'),
    ...
    it('readEclusaFile prefers sdk/prompts/ template over Eclusa-1 path', async () => {
    ...
    it('readAgentFile prefers sdk/prompts/agents/ over Eclusa-1 path', async () => {
      const { runner } = createRunnerWithSdkPrompts();

      await runner.run('build a todo app');

      // Research calls (indices 1-4) use eclusa-project-researcher.md agent def
    ...
    it('readEclusaFile falls back to Eclusa-1 when sdk/prompts/ file does not exist', async () => {
      ...
      // falls through to Eclusa-1 path. If Eclusa-1 also missing, gets placeholder.
      const projectPrompt = mockRunSession.mock.calls[0]![0] as string;
    ...
    it('readAgentFile falls back to Eclusa-1 when sdk/prompts/agents/ file does not exist', async () => {
      ...
      // Research prompt uses agent def — not in empty agents dir, falls to Eclusa-1
      const researchPrompt = mockRunSession.mock.calls[1]![0] as string;
    ...
    it('buildProjectPrompt output passes through sanitizePrompt (no /eclusa: patterns)', async () => {
      // Write a template that contains an interactive pattern
      await writeFile(
        join(sdkPromptsDir, 'templates', 'project.md'),
        '# PROJECT Template\nRun /eclusa:map-codebase to analyze.\nSDK_HEADLESS_MARKER_PROJECT\n',
      );
      ...
      const projectPrompt = mockRunSession.mock.calls[0]![0] as string;
      // sanitizePrompt should have stripped the /eclusa: line
      expect(projectPrompt).not.toMatch(/\/eclusa:\S+/);
      // But the marker should still be there
      expect(projectPrompt).toContain('SDK_HEADLESS_MARKER_PROJECT');
    });

    it('buildResearchPrompt output passes through sanitizePrompt (no /eclusa: patterns)', async () => {
      // Write an agent def that contains interactive patterns
      await writeFile(
        join(sdkPromptsDir, 'agents', 'eclusa-project-researcher.md'),
        '# Researcher Agent\nSpawn /eclusa:something for analysis.\nSDK_HEADLESS_MARKER_RESEARCHER\n',
      );
      ...
      const researchPrompt = mockRunSession.mock.calls[1]![0] as string;
      // sanitizePrompt should have stripped the /eclusa: line
      expect(researchPrompt).not.toMatch(/\/eclusa:\S+/);
      // Marker should still be present
      expect(researchPrompt).toContain('SDK_HEADLESS_MARKER_RESEARCHER');
    });

    it('buildRoadmapPrompt output passes through sanitizePrompt (no /eclusa: patterns)', async () => {
      // Write agent and templates with interactive patterns
      await writeFile(
        join(sdkPromptsDir, 'agents', 'eclusa-roadmapper.md'),
        '# Roadmapper Agent\nUse /eclusa:execute to run.\nSDK_HEADLESS_MARKER_ROADMAPPER\n',
      );
      await writeFile(
        join(sdkPromptsDir, 'templates', 'roadmap.md'),
        '# ROADMAP Template\nRun /eclusa:check-progress.\nSDK_HEADLESS_MARKER_ROADMAP\n',
      );
      await writeFile(
        join(sdkPromptsDir, 'templates', 'state.md'),
        '# STATE Template\nUse /eclusa:add-todo for tracking.\nSDK_HEADLESS_MARKER_STATE\n',
      );
      ...
      // sanitizePrompt should have stripped all /eclusa: patterns
      expect(roadmapPrompt).not.toMatch(/\/eclusa:\S+/);
