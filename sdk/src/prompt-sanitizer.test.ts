  ...
  describe('@file: references', () => {
    ...
    it('strips @file: with various path formats', () => {
      const input = [
        ...
        '@file:~/.claude/eclusa/workflows/execute-plan.md',
      ].join('\n');
      expect(sanitizePrompt(input)).toBe('');
    });
  });

  // ─── /eclusa: slash commands ────────────────────────────────────────────────

  describe('/eclusa: slash commands', () => {
    it('strips lines containing /eclusa: commands', () => {
      const input = 'Before\nRun /eclusa:execute-plan to proceed\nAfter';
      const result = sanitizePrompt(input);
      expect(result).not.toContain('/eclusa:');
      expect(result).toContain('Before');
      expect(result).toContain('After');
    });

    it('strips various /eclusa: command formats', () => {
      const input = [
        'Use /eclusa:research-phase',
        'Then /eclusa:plan-phase --auto',
        'Finally /eclusa:verify-phase',
      ].join('\n');
      expect(sanitizePrompt(input)).toBe('');
    });
  });
  ...
  describe('multiple patterns in one string', () => {
    it('strips all pattern types from a mixed prompt', () => {
      const input = [
        ...
        '',
        'When done, run /eclusa:plan-phase to proceed.',
        '',
