/**
 * Config reader — loads `.eclusa/config.json` and merges with defaults.
 *
 * Mirrors the default structure from `eclusa/bin/lib/config.cjs`
...
export const CONFIG_DEFAULTS: EclusaConfig = {
...
/**
 * Load project config from `.eclusa/config.json`, merging with defaults.
 * Returns full defaults when file is missing or empty.
 * Throws on malformed JSON with a helpful error message.
 */
export async function loadConfig(projectDir: string): Promise<EclusaConfig> {
  const configPath = join(projectDir, '.eclusa', 'config.json');
