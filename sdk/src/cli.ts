#!/usr/bin/env node
/**
 * CLI entry point for eclusa-sdk.
 *
 * Usage: eclusa-sdk run "<prompt>" [--project-dir <dir>] [--ws-port <port>]
...
export const USAGE = `
Usage: eclusa-sdk <command> [args] [options]
...
async function readStdin(): Promise<string> {
  ...
  if (stdin.isTTY) {
    throw new Error(
      ...
      '  eclusa-sdk init @path/to/prd.md\n' +
      '  eclusa-sdk init "build a todo app"\n' +
      '  cat prd.md | eclusa-sdk init'
    );
  }
...
export async function main(argv: string[] = process.argv.slice(2)): Promise<void> {
  ...
  if (args.version) {
    ...
    console.log(`eclusa-sdk v${ver}`);
    return;
  }

  if (args.command !== 'run' && args.command !== 'init' && args.command !== 'auto') {
    console.error('Error: Expected "eclusa-sdk run <prompt>", "eclusa-sdk auto", or "eclusa-sdk init [input]"');
    console.error(USAGE);
    process.exitCode = 1;
    return;
  }
  ...
  if (args.command === 'run' && !args.prompt) {
    console.error('Error: "eclusa-sdk run" requires a prompt');
    console.error(USAGE);
    process.exitCode = 1;
    return;
  }
  ...
  if (args.command === 'init') {
    ...
    // Build Eclusa instance for tools and event stream
    ...
    eclusa.addTransport(cliTransport);
    ...
    if (args.wsPort !== undefined) {
      ...
      eclusa.addTransport(wsTransport);
      console.log(`WebSocket transport listening on port ${args.wsPort}`);
    }

    try {
      const tools = eclusa.createTools();
      const runner = new InitRunner({
        projectDir: args.projectDir,
        tools,
        eventStream: eclusa.eventStream,
        config: {
  ...
  try {
    const result = await eclusa.run(args.prompt!);
