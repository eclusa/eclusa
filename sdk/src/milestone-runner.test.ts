...
// Mock the heavy dependencies that Eclusa constructor + runPhase pull in
...
vi.mock('./event-stream.js', () => {
  return {
    EclusaEventStream: vi.fn().mockImplementation(() => ({
      emitEvent: vi.fn(),
      on: vi.fn(),
      emit: vi.fn(),
      addTransport: vi.fn(),
...
vi.mock('./eclusa-tools.js', () => ({
  EclusaTools: vi.fn().mockImplementation(() => ({
    roadmapAnalyze: vi.fn(),
  })),
  EclusaToolsError: class extends Error {
    name = 'EclusaToolsError';
  },
  resolveEclusaToolsPath: vi.fn().mockReturnValue('/mock/eclusa-tools.cjs'),
}));
...
describe('Eclusa.run()', () => {
  ...
  let events: EclusaEvent[];

  beforeEach(() => {
    vi.clearAllMocks();

    eclusa = new Eclusa({ projectDir: '/tmp/test-project' });
    events = [];

    // Capture emitted events
    (eclusa.eventStream.emitEvent as ReturnType<typeof vi.fn>).mockImplementation(
      (event: EclusaEvent) => events.push(event),
    );

    // Wire mock roadmapAnalyze on the EclusaTools instance
    mockRoadmapAnalyze = vi.fn();
    vi.mocked(EclusaTools).mockImplementation(
      () =>
        ({
  ...
  it('skips phases where roadmap_complete === true', async () => {
    ...
    const result = await eclusa.run('build it');
  ...
  it('emits MilestoneStart and MilestoneComplete events', async () => {
    ...
    await eclusa.run('build it');

    const startEvents = events.filter(e => e.type === EclusaEventType.MilestoneStart);
    const completeEvents = events.filter(e => e.type === EclusaEventType.MilestoneComplete);
  ...
  it('sorts phases numerically, not lexicographically', async () => {
    ...
    vi.spyOn(eclusa, 'runPhase').mockImplementation(async (phaseNumber: string) => {
  ...
  it('passes MilestoneRunnerOptions through to runPhase', async () => {
    ...
    await eclusa.run('build it', opts);
