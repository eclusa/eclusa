<purpose>
Validate `.eclusa/` directory integrity and report actionable issues. Checks for missing files, invalid configurations, inconsistent state, and orphaned plans. Optionally repairs auto-fixable issues.
</purpose>
...
```bash
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" validate health $REPAIR_FLAG
```
...
- [E001] config.json: JSON parse error at line 5
  Fix: Run /eclusa:health --repair to reset to defaults

- [E002] PROJECT.md not found
  Fix: Run /eclusa:new-project to create
```
...
```
---
N issues can be auto-repaired. Run: /eclusa:health --repair
```
</step>
...
```
Would you like to run /eclusa:health --repair to fix N issues automatically?
```
...
```bash
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" validate health
```
...
|------|----------|-------------|------------|
| E001 | error | .eclusa/ directory not found | No |
