...
```bash
CONFIG=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" state load)
```
...
```bash
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" state update "Last Activity" "$(date +%Y-%m-%d)"
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" state update "Status" "Phase ${PHASE_NUMBER} shipped — PR #${PR_NUMBER}"
```

If `commit_docs` is true:
```bash
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" commit "docs(${padded_phase}): ship phase ${PHASE_NUMBER} — PR #${PR_NUMBER}" --files .eclusa/STATE.md
```
</step>
...
- /eclusa:complete-milestone (if last phase in milestone)
- /eclusa:progress (to see what's next)
...
- /eclusa:complete-milestone — if all phases in milestone are done
- /eclusa:progress — see overall project state
- /eclusa:execute-phase {next} — continue to next phase
</offer_next>
