...
Post-mortem investigation for failed or stuck Eclusa workflows. Analyzes git history,
`.eclusa/` artifacts, and file system state to detect anomalies and generate a
...
- `.eclusa/STATE.md` — current milestone, phase, progress, blockers, last session
- `.eclusa/ROADMAP.md` — phase list with status
- `.eclusa/config.json` — workflow configuration
...
Read `.eclusa/reports/SESSION_REPORT.md` if it exists — extract last session outcomes,
...
```bash
mkdir -p .eclusa/forensics
```

Write to `.eclusa/forensics/report-$(date +%Y%m%d-%H%M%S).md`:
...
3. {Recovery command if applicable — e.g., `/eclusa:resume-work`, `/eclusa:execute-phase N`}

---

*Report generated by `/eclusa:forensics`. All paths redacted for portability.*
```
...
> "Report saved to `.eclusa/forensics/report-{timestamp}.md`.
>
...
eclusa-tools.cjs state record-session \
  ...
  --resume-file ".eclusa/forensics/report-{timestamp}.md"
```
