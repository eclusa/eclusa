...
This workflow wires Phase 1 (session pipeline) and Phase 2 (profiling engine) into a cohesive user-facing experience. All heavy lifting is done by existing eclusa-tools.cjs subcommands and the eclusa-user-profiler agent -- this workflow orchestrates the sequence, handles branching, and provides the UX.
</purpose>
...
Key references:
- @$HOME/.claude/eclusa/references/ui-brand.md (display patterns)
- @$HOME/.claude/eclusa/agents/eclusa-user-profiler.md (profiler agent definition)
- @$HOME/.claude/eclusa/references/user-profiling.md (profiling reference doc)
</required_reading>
...
```bash
cp "$HOME/.claude/eclusa/USER-PROFILE.md" "$HOME/.claude/eclusa/USER-PROFILE.backup.md"
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Eclusa > PROFILE YOUR CODING STYLE
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
✓ Stores profile at $HOME/.claude/eclusa/USER-PROFILE.md
...
- options:
  ...
  - "Not now" -- Display "No worries. Run /eclusa:profile-user when ready." and exit
...
Run session scan:
```bash
SCAN_RESULT=$(node $HOME/.claude/eclusa/bin/eclusa-tools.cjs scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(node $HOME/.claude/eclusa/bin/eclusa-tools.cjs profile-sample --json 2>/dev/null)
```
...
**Spawn eclusa-user-profiler agent using Task tool:**

Use the Task tool to spawn the `eclusa-user-profiler` agent. Provide it with:
- The sampled JSONL file path from profile-sample output
- The user-profiling reference doc at `$HOME/.claude/eclusa/references/user-profiling.md`
...
Reference: @$HOME/.claude/eclusa/references/user-profiling.md
...
**Get questions:**
```bash
QUESTIONS=$(node $HOME/.claude/eclusa/bin/eclusa-tools.cjs profile-questionnaire --json 2>/dev/null)
```
...
```bash
ANSWERS_PATH=$(mktemp /tmp/eclusa-profile-answers-XXXXXX.json)
```
...
```bash
ANALYSIS_RESULT=$(node $HOME/.claude/eclusa/bin/eclusa-tools.cjs profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
```bash
ANALYSIS_PATH=$(mktemp /tmp/eclusa-profile-analysis-XXXXXX.json)
```
...
```bash
node $HOME/.claude/eclusa/bin/eclusa-tools.cjs write-profile --input "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Profile written to $HOME/.claude/eclusa/USER-PROFILE.md"
...
- options (ALL pre-selected by default):
  - "/eclusa:dev-preferences command file" -- "Load your preferences in any session"
...
**If no artifacts selected:** Display "No artifacts generated. Your profile is saved at $HOME/.claude/eclusa/USER-PROFILE.md" and jump to step 10.
...
**For /eclusa:dev-preferences (if selected):**

```bash
node $HOME/.claude/eclusa/bin/eclusa-tools.cjs generate-dev-preferences --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Generated /eclusa:dev-preferences at $HOME/.claude/commands/eclusa/dev-preferences.md"

**For CLAUDE.md profile section (if selected):**

```bash
node $HOME/.claude/eclusa/bin/eclusa-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node $HOME/.claude/eclusa/bin/eclusa-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json 2>/dev/null
```

Display: "✓ Added profile section to $HOME/.claude/CLAUDE.md"

**Error handling:** If any eclusa-tools.cjs call fails, display the error message and use AskUserQuestion to offer "Retry" or "Skip this artifact". On retry, re-run the command. On skip, continue to next artifact.
...
```bash
BACKUP_PATH="$HOME/.claude/eclusa/USER-PROFILE.backup.md"
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Eclusa > PROFILE COMPLETE ✓
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Your profile:    $HOME/.claude/eclusa/USER-PROFILE.md
```

Then list paths for each generated artifact:
```
Artifacts:
  ✓ /eclusa:dev-preferences   $HOME/.claude/commands/eclusa/dev-preferences.md
...
- [ ] Artifacts generated sequentially via eclusa-tools.cjs subcommands
