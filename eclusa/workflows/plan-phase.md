<purpose>
Create executable phase prompts (PLAN.md files) for a roadmap phase with integrated research and verification. Default flow: Research (if needed) -> Plan -> Verify -> Done. Orchestrates eclusa-phase-researcher, eclusa-planner, and eclusa-plan-checker agents with a revision loop (max 3 iterations).
</purpose>
...
```bash
INIT=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" init plan-phase "$PHASE")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_RESEARCHER=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" agent-skills eclusa-researcher 2>/dev/null)
AGENT_SKILLS_PLANNER=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" agent-skills eclusa-planner 2>/dev/null)
AGENT_SKILLS_CHECKER=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" agent-skills eclusa-checker 2>/dev/null)
```
...
**If `planning_exists` is false:** Error — run `/eclusa:new-project` first.
...
```bash
mkdir -p ".eclusa/phases/${padded_phase}-${phase_slug}"
```
...
/eclusa:review --phase {N}

Then re-run /eclusa:plan-phase {N} --reviews
```
Exit workflow.
...
```bash
PHASE_INFO=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" roadmap get-phase "${PHASE}")
```
...
5. Commit:
```bash
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
DISCUSS_MODE=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" config-get workflow.discuss_mode 2>/dev/null || echo "discuss")
```
...
If "Run discuss-phase first":
  **IMPORTANT:** Do NOT invoke discuss-phase as a nested Skill/Task call — AskUserQuestion
  ...
  ```
  Run this command first, then re-run /eclusa:plan-phase {X} ${ECLUSA_WS}:

  /eclusa:discuss-phase {X} ${ECLUSA_WS}
  ```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Eclusa ► RESEARCHING PHASE {X}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

◆ Spawning researcher...
```

### Spawn eclusa-phase-researcher

```bash
PHASE_DESC=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" roadmap get-phase "${PHASE}" --pick section)
```
...
<files_to_read>
- {context_path} (USER DECISIONS from /eclusa:discuss-phase)
...
Task(
  ...
  subagent_type="eclusa-phase-researcher",
...
**If found:**
1. Read template: `~/.claude/eclusa/templates/VALIDATION.md`
...
> Skip if `workflow.ui_phase` is explicitly `false` AND `workflow.ui_safety_gate` is explicitly `false` in `.eclusa/config.json`. If keys are absent, treat as enabled.

```bash
UI_PHASE_CFG=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" config-get workflow.ui_phase 2>/dev/null || echo "true")
UI_GATE_CFG=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" config-get workflow.ui_safety_gate 2>/dev/null || echo "true")
```
...
```bash
PHASE_SECTION=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" roadmap get-phase "${PHASE}" 2>/dev/null)
...
1. Generate UI-SPEC first — Run /eclusa:ui-phase {N} then re-run /eclusa:plan-phase {N}
...
- options:
  - "Generate UI-SPEC first" → Display: "Run `/eclusa:ui-phase {N} ${ECLUSA_WS}` then re-run `/eclusa:plan-phase {N} ${ECLUSA_WS}`". Exit workflow.
...
```bash
_eclusa_field() { node -e "const o=JSON.parse(process.argv[1]); const v=o[process.argv[2]]; process.stdout.write(v==null?'':String(v))" "$1" "$2"; }
STATE_PATH=$(_eclusa_field "$INIT" state_path)
ROADMAP_PATH=$(_eclusa_field "$INIT" roadmap_path)
REQUIREMENTS_PATH=$(_eclusa_field "$INIT" requirements_path)
RESEARCH_PATH=$(_eclusa_field "$INIT" research_path)
VERIFICATION_PATH=$(_eclusa_field "$INIT" verification_path)
UAT_PATH=$(_eclusa_field "$INIT" uat_path)
CONTEXT_PATH=$(_eclusa_field "$INIT" context_path)
REVIEWS_PATH=$(_eclusa_field "$INIT" reviews_path)
```
...
1. Re-run: `/eclusa:plan-phase {PHASE} --research ${ECLUSA_WS}`
2. Disable Nyquist with the exact command:
   `node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" config-set workflow.nyquist_validation false`
3. Continue anyway (plans fail Dimension 8)

Proceed to Step 8 only if user selects 2 or 3.

## 8. Spawn eclusa-planner Agent

Display banner:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Eclusa ► PLANNING PHASE {X}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
<downstream_consumer>
Output consumed by /eclusa:execute-phase. Plans need:
...
## 10. Spawn eclusa-plan-checker Agent

Display banner:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Eclusa ► VERIFYING PLANS
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
Task(
  ...
  subagent_type="eclusa-plan-checker",
  model="{checker_model}",
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Eclusa ► AUTO-ADVANCING TO EXECUTE
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```
Skill(skill="eclusa:execute-phase", args="${PHASE} --auto --no-transition ${ECLUSA_WS}")
```
...
- **PHASE COMPLETE** → Display final summary:
  ```
  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   Eclusa ► PHASE ${PHASE} COMPLETE ✓
  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

  Auto-advance pipeline finished.

  Next: /eclusa:discuss-phase ${NEXT_PHASE} --auto ${ECLUSA_WS}
  ```
- **GAPS FOUND / VERIFICATION FAILED** → Display result, stop chain:
  ...
  /eclusa:execute-phase ${PHASE} ${ECLUSA_WS}
  ```
...
/eclusa:execute-phase {X} ${ECLUSA_WS}
...
**Also available:**
- cat .eclusa/phases/{phase-dir}/*-PLAN.md — review plans
- /eclusa:plan-phase {X} --research — re-research first
- /eclusa:review --phase {X} --all — peer review plans with external AIs
- /eclusa:plan-phase {X} --reviews — replan incorporating review feedback
...
5. **Retry:** Restart Claude Code and run `/eclusa:plan-phase` again

If freezes persist, try `--skip-research` to reduce the agent chain from 3 to 2 agents:
```
/eclusa:plan-phase N --skip-research
```
</windows_troubleshooting>

<success_criteria>
- [ ] .eclusa/ directory validated
- [ ] Phase validated against roadmap
- [ ] Phase directory created if needed
- [ ] CONTEXT.md loaded early (step 4) and passed to ALL agents
- [ ] Research completed (unless --skip-research or --gaps or exists)
- [ ] eclusa-phase-researcher spawned with CONTEXT.md
- [ ] Existing plans checked
- [ ] eclusa-planner spawned with CONTEXT.md + RESEARCH.md
- [ ] Plans created (PLANNING COMPLETE or CHECKPOINT handled)
- [ ] eclusa-plan-checker spawned with CONTEXT.md
