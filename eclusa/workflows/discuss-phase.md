...
1. **eclusa-phase-researcher** — Reads CONTEXT.md to know WHAT to research
   - "User wants card-based layout" → researcher investigates card component patterns
   - "Infinite scroll decided" → researcher looks into virtualization libraries

2. **eclusa-planner** — Reads CONTEXT.md to know WHAT decisions are locked
...
Enable text mode:
- Per-session: pass `--text` flag to any command (e.g., `/eclusa:discuss-phase --text`)
- Per-project: `eclusa-tools config-set workflow.text_mode true`
...
**Express path available:** If you already have a PRD or acceptance criteria document, use `/eclusa:plan-phase {phase} --prd path/to/prd.md` to skip this discussion and go straight to planning.

<step name="initialize" priority="first">
Phase number from argument (required).

```bash
INIT=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" init phase-op "${PHASE}")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_ADVISOR=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" agent-skills eclusa-advisor 2>/dev/null)
```
...
Use /eclusa:progress ${ECLUSA_WS} to see available phases.
```
Exit workflow.
...
- options:
  - "Continue and replan after" — Capture context, then run /eclusa:plan-phase {X} ${ECLUSA_WS} to replan
...
```bash
# Core project files
cat .eclusa/PROJECT.md 2>/dev/null || true
cat .eclusa/REQUIREMENTS.md 2>/dev/null || true
cat .eclusa/STATE.md 2>/dev/null || true
```
...
(find .eclusa/phases -name "*-CONTEXT.md" 2>/dev/null || true) | sort
```
...
```bash
TODO_MATCHES=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" todo match-phase "${PHASE_NUMBER}")
```
...
```bash
ls .eclusa/codebase/*.md 2>/dev/null || true
```
...
1. Check for USER-PROFILE.md:
   ```bash
   PROFILE_PATH="$HOME/.claude/eclusa/USER-PROFILE.md"
   ```
...
3. Resolve model for advisor agents:
   ```bash
   ADVISOR_MODEL=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" resolve-model eclusa-advisor-researcher --raw)
   ```
...
2. For EACH user-selected gray area, spawn a Task() in parallel:
   ...
   Task(
     prompt="First, read @~/.claude/agents/eclusa-advisor-researcher.md for your role and instructions.
...
**Research-before-questions mode:** Check if `workflow.research_before_questions` is enabled in config (from init context or `.eclusa/config.json`). When enabled, before presenting questions for each area:
...
```
Created: .eclusa/phases/${PADDED_PHASE}-${SLUG}/${PADDED_PHASE}-CONTEXT.md
...
`/eclusa:plan-phase ${PHASE} ${ECLUSA_WS}`
...
**Also available:**
- `/eclusa:plan-phase ${PHASE} --skip-research ${ECLUSA_WS}` — plan without research
- `/eclusa:ui-phase ${PHASE} ${ECLUSA_WS}` — generate UI design contract before planning (if phase has frontend work)
...
```bash
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
```bash
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" commit "docs(state): record phase ${PHASE} context session" --files .eclusa/STATE.md
```
</step>
...
```
Skill(skill="eclusa:plan-phase", args="${PHASE} --auto ${ECLUSA_WS}")
```
...
- **PLANNING COMPLETE** → Planning done, execution didn't complete:
  ...
  Continue: /eclusa:execute-phase ${PHASE} ${ECLUSA_WS}
  ```
- **PLANNING INCONCLUSIVE / CHECKPOINT** → Stop chain:
  ```
  Auto-advance stopped: Planning needs input.
  Continue: /eclusa:plan-phase ${PHASE} ${ECLUSA_WS}
  ```
- **GAPS FOUND** → Stop chain:
  ```
  Auto-advance stopped: Gaps found during execution.
  Continue: /eclusa:plan-phase ${PHASE} --gaps ${ECLUSA_WS}
  ```
