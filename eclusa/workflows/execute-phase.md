...
- **Claude Code:** Uses `Task(subagent_type="eclusa-executor", ...)` — blocks until complete, returns result
...
These are the valid Eclusa subagent types registered in .claude/agents/ (or equivalent for your runtime).
Always use the exact name from this list — do not fall back to 'general-purpose' or other built-in types:

- eclusa-executor — Executes plan tasks, commits, creates SUMMARY.md
- eclusa-verifier — Verifies phase completion, checks quality gates
- eclusa-planner — Creates detailed plans from phase scope
- eclusa-phase-researcher — Researches technical approaches for a phase
- eclusa-plan-checker — Reviews plan quality before execution
- eclusa-debugger — Diagnoses and fixes issues
- eclusa-codebase-mapper — Maps project structure and dependencies
- eclusa-integration-checker — Checks cross-phase integration
- eclusa-nyquist-auditor — Validates verification coverage
- eclusa-ui-researcher — Researches UI/UX approaches
- eclusa-ui-checker — Reviews UI implementation quality
- eclusa-ui-auditor — Audits UI against design requirements
</available_agent_types>
...
```bash
INIT=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" init execute-phase "${PHASE_ARG}")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" agent-skills eclusa-executor 2>/dev/null)
```
...
**If `state_exists` is false but `.eclusa/` exists:** Offer reconstruct or continue.

When `parallelization` is false, plans within a wave execute sequentially.

**Runtime detection for Copilot:**
Check if the current runtime is Copilot by testing for the `@eclusa-executor` agent pattern
...
if [[ ! "$ARGUMENTS" =~ --auto ]]; then
  node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" config-set workflow._auto_chain_active false 2>/dev/null
fi
```
</step>
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `~/.claude/eclusa/workflows/execute-plan.md` **inline**
...
- Maintains Eclusa's planning/tracking structure
- Best for: small phases, bug fixes, verification gaps, learning Eclusa
...
```bash
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" state begin-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
```bash
PLAN_INDEX=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" phase-plan-index "${PHASE_NUMBER}")
```
...
2. **Spawn executor agents:**
   ...
   Task(
     ...
     prompt="
       ...
       For eclusa-tools commits: add --no-verify flag.
       For direct git commits: use git commit --no-verify -m "..."
       </parallel_execution>

       <execution_context>
       @~/.claude/eclusa/workflows/execute-plan.md
       @~/.claude/eclusa/templates/summary.md
       @~/.claude/eclusa/references/checkpoints.md
       @~/.claude/eclusa/references/tdd.md
       </execution_context>

       <files_to_read>
       Read these files at execution start using the Read tool:
       - {phase_dir}/{plan_file} (Plan)
       - .eclusa/PROJECT.md (Project context — core value, requirements, evolution rules)
       - .eclusa/STATE.md (State)
       - .eclusa/config.json (Config, if exists)
...
5. **Handle failures:**
   ...
   **Known Claude Code bug (classifyHandoffIfNeeded):** If an agent reports "failed" with error containing `classifyHandoffIfNeeded is not defined`, this is a Claude Code runtime bug — not a Eclusa or agent issue. The error fires in the completion handler AFTER all tool calls finish. In this case: run the same spot-checks as step 4 (SUMMARY.md exists, git commits present, no Self-Check: FAILED). If spot-checks PASS → treat as **successful**. If spot-checks FAIL → treat as real failure below.
...
5b. **Pre-wave dependency check (waves 2+ only):**
    ...
    ```bash
    node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" verify key-links {phase_dir}/{plan}-PLAN.md
    ```
...
```bash
AUTO_CHAIN=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" config-get workflow._auto_chain_active 2>/dev/null || echo "false")
AUTO_CFG=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" config-get workflow.auto_advance 2>/dev/null || echo "false")
```
...
```bash
POST_PLAN_INDEX=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" phase-plan-index "${PHASE_NUMBER}")
```
...
/eclusa:execute-phase {phase} ${ECLUSA_WS}                # Continue remaining waves
/eclusa:execute-phase {phase} --wave {next} ${ECLUSA_WS}  # Run the next wave explicitly
```
...
```bash
PARENT_INFO=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" find-phase "${PARENT_PHASE}" --raw)
...
```bash
mkdir -p .eclusa/debug/resolved
mv .eclusa/debug/{slug}.md .eclusa/debug/resolved/
```

**6. Commit updated artifacts:**
```bash
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" commit "docs(phase-${PARENT_PHASE}): resolve UAT gaps and debug sessions after ${PHASE_NUMBER} gap closure" --files .eclusa/phases/*${PARENT_PHASE}*/*-UAT.md .eclusa/debug/resolved/*.md
```
</step>
...
PRIOR_VERIFICATIONS=$(find .eclusa/phases/ -name "*-VERIFICATION.md" ! -path "*${PHASE_NUMBER}*" 2>/dev/null)
```
...
```bash
VERIFIER_SKILLS=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" agent-skills eclusa-verifier 2>/dev/null)
```
...
| `gaps_found` | Present gap summary, offer `/eclusa:plan-phase {phase} --gaps ${ECLUSA_WS}` |
...
Commit the file:
```bash
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" commit "test({phase_num}): persist human verification items as UAT" --files "{phase_dir}/{phase_num}-HUMAN-UAT.md"
```
...
Items saved to `{phase_num}-HUMAN-UAT.md` — they will appear in `/eclusa:progress` and `/eclusa:audit-uat`.

"approved" → continue | Report issues → gap closure
```

**If user says "approved":** Proceed to `update_roadmap`. The HUMAN-UAT.md file persists with `status: partial` and will surface in future progress checks until the user runs `/eclusa:verify-work` on it.
...
`/eclusa:plan-phase {X} --gaps ${ECLUSA_WS}`

<sub>`/clear` first → fresh context window</sub>

Also: `cat {phase_dir}/{phase_num}-VERIFICATION.md` — full report
Also: `/eclusa:verify-work {X} ${ECLUSA_WS}` — manual testing first
```

Gap closure cycle: `/eclusa:plan-phase {X} --gaps ${ECLUSA_WS}` reads VERIFICATION.md → creates gap plans with `gap_closure: true` → user runs `/eclusa:execute-phase {X} --gaps-only ${ECLUSA_WS}` → verifier re-runs.
</step>
...
```bash
COMPLETION=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" phase complete "${PHASE_NUMBER}")
```
...
These items are tracked and will appear in `/eclusa:progress` and `/eclusa:audit-uat`.
```

```bash
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" commit "docs(phase-{X}): complete phase execution" --files .eclusa/ROADMAP.md .eclusa/STATE.md .eclusa/REQUIREMENTS.md {phase_dir}/*-VERIFICATION.md
```
</step>
...
```bash
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" commit "docs(phase-{X}): evolve PROJECT.md after phase completion" --files .eclusa/PROJECT.md
```

**Skip this step if** `.eclusa/PROJECT.md` does not exist.
</step>

<step name="offer_next">

**Exception:** If `gaps_found`, the `verify_phase_goal` step already presents the gap-closure path (`/eclusa:plan-phase {X} --gaps`). No additional routing needed — skip auto-advance.
...
Read and follow `~/.claude/eclusa/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.

**If none of `--auto`, `AUTO_CHAIN`, or `AUTO_CFG` is true:**

**STOP. Do not auto-advance. Do not execute transition. Do not plan next phase. Present options to the user and wait.**

**IMPORTANT: There is NO `/eclusa:transition` command. Never suggest it. The transition workflow is internal only.**

```
## ✓ Phase {X}: {Name} Complete

/eclusa:progress ${ECLUSA_WS} — see updated roadmap
/eclusa:discuss-phase {next} ${ECLUSA_WS} — discuss next phase before planning
/eclusa:plan-phase {next} ${ECLUSA_WS} — plan next phase
/eclusa:execute-phase {next} ${ECLUSA_WS} — execute next phase
```
...
<failure_handling>
- **classifyHandoffIfNeeded false failure:** Agent reports "failed" but error is `classifyHandoffIfNeeded is not defined` → Claude Code bug, not Eclusa. Spot-check (SUMMARY exists, commits present) → if pass, treat as success
...
<resumption>
Re-run `/eclusa:execute-phase {phase}` → discover_plans finds completed SUMMARYs → skips them → resumes from first incomplete plan → continues wave execution.
