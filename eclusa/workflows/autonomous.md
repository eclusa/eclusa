...
```bash
INIT=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" init milestone-op)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
```

Parse JSON for: `milestone_version`, `milestone_name`, `phase_count`, `completed_phases`, `roadmap_exists`, `state_exists`, `commit_docs`.

**If `roadmap_exists` is false:** Error — "No ROADMAP.md found. Run `/eclusa:new-milestone` first."
**If `state_exists` is false:** Error — "No STATE.md found. Run `/eclusa:new-milestone` first."
...
```bash
DETAIL=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" roadmap get-phase ${PHASE_NUM})
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Eclusa ► AUTONOMOUS ▸ Phase {N}/{T}: {Name} [████░░░░] {P}%
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
```bash
PHASE_STATE=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" init phase-op ${PHASE_NUM})
```
...
```bash
SKIP_DISCUSS=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" config-get workflow.skip_discuss 2>/dev/null || echo "false")
```
...
```bash
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" commit "docs(${PADDED_PHASE}): auto-generated context (discuss skipped)" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
PHASE_SECTION=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" roadmap get-phase ${PHASE_NUM} 2>/dev/null)
...
```
Skill(skill="eclusa:ui-phase", args="${PHASE_NUM}")
```
...
```
Skill(skill="eclusa:plan-phase", args="${PHASE_NUM}")
```
...
```
Skill(skill="eclusa:execute-phase", args="${PHASE_NUM} --no-transition")
```
...
```
Skill(skill="eclusa:plan-phase", args="${PHASE_NUM} --gaps")
```
...
```bash
UI_REVIEW_CFG=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" config-get workflow.ui_review 2>/dev/null || echo "true")
```
...
```
Skill(skill="eclusa:ui-review", args="${PHASE_NUM}")
```
...
> **Note:** Smart discuss is an autonomous-optimized variant of the `eclusa:discuss-phase` skill. It produces identical CONTEXT.md output but uses batch table proposals instead of sequential questioning. The original `discuss-phase` skill remains unchanged (per CTRL-03). Future milestones may extract this to a separate skill file.
...
```bash
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" commit "docs(${PADDED_PHASE}): smart discuss context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```
Skill(skill="eclusa:audit-milestone")
```

After audit completes, detect the result:

```bash
AUDIT_FILE=".eclusa/v${milestone_version}-MILESTONE-AUDIT.md"
...
On **"Stop"**: Go to handle_blocker with "User stopped — audit gaps remain. Run /eclusa:audit-milestone to review, then /eclusa:complete-milestone when ready."
...
On **"Stop"**: Go to handle_blocker with "User stopped — tech debt to address. Run /eclusa:audit-milestone to review details."

**5b. Complete Milestone**

```
Skill(skill="eclusa:complete-milestone", args="${milestone_version}")
```

After complete-milestone returns, verify it produced output:

```bash
ls .eclusa/milestones/v${milestone_version}-ROADMAP.md 2>/dev/null || true
```
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 Resume with: /eclusa:autonomous --from {next_phase}
```
...
- [ ] Audit invoked via Skill(skill="eclusa:audit-milestone")
