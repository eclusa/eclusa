<purpose>
Create all phases necessary to close gaps identified by `/eclusa:audit-milestone`. Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase. One command creates all fix phases — no manual `/eclusa:add-phase` per gap.
</purpose>
...
(ls -t .eclusa/v*-MILESTONE-AUDIT.md 2>/dev/null || true) | head -1
```
...
```
No audit gaps found. Run `/eclusa:audit-milestone` first.
```
...
HIGHEST=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" phases list --pick directories[-1])
```
...
grep -c "Pending" .eclusa/REQUIREMENTS.md
```

## 8. Create Phase Directories

```bash
mkdir -p ".eclusa/phases/{NN}-{name}"
```

## 9. Commit Roadmap and Requirements Update

```bash
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" commit "docs(roadmap): add gap closure phases {N}-{M}" --files .eclusa/ROADMAP.md .eclusa/REQUIREMENTS.md
```
...
**Also available:**
- `/eclusa:execute-phase {N}` — if plans already exist
- `cat .eclusa/ROADMAP.md` — see updated roadmap

---

**After all gap phases complete:**

`/eclusa:audit-milestone` — re-audit to verify gaps closed
`/eclusa:complete-milestone {version}` — archive when audit passes
```
...
- [ ] User knows to run `/eclusa:plan-phase` next
</success_criteria>
