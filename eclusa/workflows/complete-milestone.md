...
1. Extract full milestone details to `.eclusa/milestones/v[X.Y]-ROADMAP.md`
2. Archive requirements to `.eclusa/milestones/v[X.Y]-REQUIREMENTS.md`
...
8. Clean up `.eclusa/ui-reviews/` screenshot files (binary assets, never archived)
...
2. **Run audit first** — `/eclusa:audit-milestone` to assess gap severity
...
for summary in .eclusa/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
  node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" summary-extract "$summary" --fields one_liner --pick one_liner
done
```
...
**Note:** MILESTONES.md entry is now created automatically by `eclusa-tools milestone complete` in the archive_milestone step. The entry includes version, date, phase/plan/task counts, and accomplishments extracted from SUMMARY.md files.
...
Update `.eclusa/ROADMAP.md` — group completed milestone phases:
...
**Delegate archival to eclusa-tools:**

```bash
ARCHIVE=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" milestone complete "v[X.Y]" --name "[Milestone Name]")
```

The CLI handles:
- Creating `.eclusa/milestones/` directory
...
Verify: `✅ Milestone archived to .eclusa/milestones/`
...
```bash
mkdir -p .eclusa/milestones/v[X.Y]-phases
# For each phase directory in .eclusa/phases/:
mv .eclusa/phases/{phase-dir} .eclusa/milestones/v[X.Y]-phases/
```
Verify: `✅ Phase directories archived to .eclusa/milestones/v[X.Y]-phases/`

If "Skip": Phase directories remain in `.eclusa/phases/` as raw execution history. Use `/eclusa:cleanup` later to archive retroactively.
...
```bash
rm .eclusa/ROADMAP.md
rm .eclusa/REQUIREMENTS.md
```
...
```bash
ls .eclusa/RETROSPECTIVE.md 2>/dev/null || true
```

**If exists:** Read the file, append new milestone section before the "## Cross-Milestone Trends" section.

**If doesn't exist:** Create from template at `~/.claude/eclusa/templates/retrospective.md`.
...
**Commit:**
```bash
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" commit "docs: update retrospective for v${VERSION}" --files .eclusa/RETROSPECTIVE.md
```
...
```bash
INIT=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" init execute-phase "1")
...
if [ "$BRANCHING_STRATEGY" = "phase" ]; then
  for branch in $PHASE_BRANCHES; do
    ...
    # Strip .eclusa/ from staging if commit_docs is false
    if [ "$COMMIT_DOCS" = "false" ]; then
      git reset HEAD .eclusa/ 2>/dev/null || true
    fi
...
See .eclusa/MILESTONES.md for full details."
```
...
```bash
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" commit "chore: complete v[X.Y] milestone" --files .eclusa/milestones/v[X.Y]-ROADMAP.md .eclusa/milestones/v[X.Y]-REQUIREMENTS.md .eclusa/milestones/v[X.Y]-MILESTONE-AUDIT.md .eclusa/MILESTONES.md .eclusa/PROJECT.md .eclusa/STATE.md
```
```
...
- [ ] User knows next step (/eclusa:new-milestone)
