...
1. Check `.eclusa/STATE.md` for current milestone version
2. Check `.eclusa/milestones/` for the latest archived version
3. If neither found, check if `.eclusa/ROADMAP.md` exists (project may be mid-milestone)
4. If nothing found: error "No milestone found. Run /eclusa:new-project or /eclusa:new-milestone first."
...
**Archived milestone** (`.eclusa/milestones/v{VERSION}-ROADMAP.md` exists):
```
ROADMAP_PATH=".eclusa/milestones/v${VERSION}-ROADMAP.md"
REQUIREMENTS_PATH=".eclusa/milestones/v${VERSION}-REQUIREMENTS.md"
AUDIT_PATH=".eclusa/milestones/v${VERSION}-MILESTONE-AUDIT.md"
```

**Current/in-progress milestone** (no archive yet):
```
ROADMAP_PATH=".eclusa/ROADMAP.md"
REQUIREMENTS_PATH=".eclusa/REQUIREMENTS.md"
AUDIT_PATH=".eclusa/v${VERSION}-MILESTONE-AUDIT.md"
```

Note: The audit file moves to `.eclusa/milestones/` on archive (per `complete-milestone` workflow). Check both locations as a fallback.

**Always available:**
```
PROJECT_PATH=".eclusa/PROJECT.md"
RETRO_PATH=".eclusa/RETROSPECTIVE.md"
STATE_PATH=".eclusa/STATE.md"
```
...
```bash
eclusa-tools.cjs init progress
```
...
Find the earliest `.eclusa/phases/` commit:
```bash
git log --oneline --diff-filter=A -- ".eclusa/phases/" | tail -1
```
...
Write to `.eclusa/reports/MILESTONE_SUMMARY-v${VERSION}.md`:
...
**Overwrite guard:** If `.eclusa/reports/MILESTONE_SUMMARY-v${VERSION}.md` already exists, ask the user:
...
```bash
mkdir -p .eclusa/reports
```

Write the summary, then commit:
```bash
eclusa-tools.cjs commit "docs(v${VERSION}): generate milestone summary for onboarding" \
  --files ".eclusa/reports/MILESTONE_SUMMARY-v${VERSION}.md"
```
...
> "Summary written to `.eclusa/reports/MILESTONE_SUMMARY-v{VERSION}.md`.
>
...
If the user is done:
- Suggest next steps: `/eclusa:new-milestone`, `/eclusa:progress`, or sharing the summary with the team

## Step 9: Update STATE.md

```bash
eclusa-tools.cjs state record-session \
  --stopped-at "Milestone v${VERSION} summary generated" \
  --resume-file ".eclusa/reports/MILESTONE_SUMMARY-v${VERSION}.md"
```
