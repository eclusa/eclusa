...
The PR branch contains only code changes — reviewers don't see Eclusa artifacts
...
For each commit, check if it ONLY touches .eclusa/ files:

```bash
# For each commit hash
FILES=$(git diff-tree --no-commit-id --name-only -r $HASH)
ALL_PLANNING=$(echo "$FILES" | grep -v "^\.eclusa/" | wc -l)
```

Classify:
- **Code commits**: Touch at least one non-.eclusa/ file → INCLUDE
- **Planning-only commits**: Touch only .eclusa/ files → EXCLUDE
...
for HASH in $CODE_COMMITS; do
  ...
  # Remove any .eclusa/ files that came along in mixed commits
  git rm -r --cached .eclusa/ 2>/dev/null || true
  git commit -C "$HASH"
done
```
...
<step name="verify">
```bash
# Verify no .eclusa/ files in PR branch
PLANNING_FILES=$(git diff --name-only "$TARGET".."$PR_BRANCH" | grep "^\.eclusa/" | wc -l)
...
Or use /eclusa:ship to create the PR automatically.
```
</step>
...
- [ ] No .eclusa/ files in PR branch diff
