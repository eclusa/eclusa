...
AGENT_SKILLS_CHECKER=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" agent-skills eclusa-integration-checker 2>/dev/null)
```
...
```bash
integration_checker_model=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" resolve-model eclusa-integration-checker --raw)
```
...
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" phases list
```
...
PHASE_INFO=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" find-phase 01 --raw)
...
${AGENT_SKILLS_CHECKER}",
  subagent_type="eclusa-integration-checker",
...
for summary in .eclusa/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
  node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" summary-extract "$summary" --fields requirements_completed --pick requirements_completed
done
```
...
```bash
NYQUIST_CONFIG=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" config-get workflow.nyquist_validation --raw 2>/dev/null)
```
...
Discovery only — never auto-calls `/eclusa:validate-phase`.

## 6. Aggregate into v{version}-MILESTONE-AUDIT.md

Create `.eclusa/v{version}-v{version}-MILESTONE-AUDIT.md` with:
...
**Report:** .eclusa/v{version}-MILESTONE-AUDIT.md
...
|-------|---------------|-----------|--------|
| {phase} | exists/missing | true/false/partial | `/eclusa:validate-phase {N}` |

Phases needing validation: run `/eclusa:validate-phase {N}` for each flagged phase.
...
**Also available:**
- cat .eclusa/v{version}-MILESTONE-AUDIT.md — see full report
- /eclusa:complete-milestone {version} — proceed anyway (accept tech debt)
