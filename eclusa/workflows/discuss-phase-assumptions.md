...
- eclusa-assumptions-analyzer — Analyzes codebase to surface implementation assumptions
</available_agent_types>
...
AGENT_SKILLS_ANALYZER=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" agent-skills eclusa-assumptions-analyzer 2>/dev/null)
```
...
Use /eclusa:progress to see available phases.
```
Exit workflow.
...
Spawn a `eclusa-assumptions-analyzer` agent to deeply analyze the codebase for this phase. This
...
```
Task(subagent_type="eclusa-assumptions-analyzer", prompt="""
...
```bash
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" commit "docs(${padded_phase}): capture phase context (assumptions mode)" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
**Also available:**
- `/eclusa:plan-phase ${PHASE} --skip-research` — plan without research
- `/eclusa:ui-phase ${PHASE}` — generate UI design contract (if frontend work)
...
Launch: `Skill(skill="eclusa:plan-phase", args="${PHASE} --auto")`
