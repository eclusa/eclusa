...
- Check for MILESTONE-CONTEXT.md (from /eclusa:discuss-milestone)
...
```bash
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" commit "docs: start milestone v[X.Y] [Name]" --files .eclusa/PROJECT.md .eclusa/STATE.md
```

## 7. Load Context and Resolve Models

```bash
INIT=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" init new-milestone)
...
find .eclusa/phases -mindepth 1 -maxdepth 1 -type d -exec mv {} "${phase_archive_path}/" \;
```

Then verify `.eclusa/phases/` no longer contains old milestone directories before continuing.
...
- Tell the user to complete/archive the previous milestone first, then rerun `/eclusa:new-milestone --reset-phase-numbers ${ECLUSA_WS}`.
...
**IMPORTANT:** Do NOT persist this choice to config.json. The `workflow.research` setting is a persistent user preference that controls plan-phase behavior across the project. Changing it here would silently alter future `/eclusa:plan-phase` behavior. To change the default, use `/eclusa:settings`.
...
Spawn 4 parallel eclusa-project-researcher agents. Each uses this template with dimension-specific fields:
...
<output>
Write to: .eclusa/research/{FILE}
Use template: ~/.claude/eclusa/templates/research-project/{FILE}
</output>
", subagent_type="eclusa-project-researcher", model="{researcher_model}", description="{DIMENSION} research")
```
...
**Commit requirements:**
```bash
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" commit "docs: define milestone v[X.Y] requirements" --files .eclusa/REQUIREMENTS.md
```
...
- .eclusa/research/SUMMARY.md (if exists)
- .eclusa/config.json
- .eclusa/MILESTONES.md
</files_to_read>
...
```bash
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" commit "docs: create milestone v[X.Y] roadmap ([N] phases)" --files .eclusa/ROADMAP.md .eclusa/STATE.md .eclusa/REQUIREMENTS.md
```
...
`/eclusa:discuss-phase [N] ${ECLUSA_WS}` — gather context and clarify approach

<sub>`/clear` first → fresh context window</sub>

Also: `/eclusa:plan-phase [N] ${ECLUSA_WS}` — skip discussion, plan directly
```
...
- [ ] eclusa-roadmapper spawned with phase numbering context
- [ ] Roadmap files written immediately (not draft)
- [ ] User feedback incorporated (if any)
- [ ] Phase numbering mode respected (continued or reset)
- [ ] All commits made (if planning docs committed)
- [ ] User knows next step: `/eclusa:discuss-phase [N] ${ECLUSA_WS}`
