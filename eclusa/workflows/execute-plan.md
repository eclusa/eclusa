...
@~/.claude/eclusa/references/git-integration.md
</required_reading>
...
```bash
INIT=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" init execute-phase "${PHASE}")
...
If `.eclusa/` missing: error.
</step>

<step name="identify_plan">
```bash
# Use plans/summaries from INIT JSON, or list files
(ls .eclusa/phases/XX-name/*-PLAN.md 2>/dev/null || true) | sort
(ls .eclusa/phases/XX-name/*-SUMMARY.md 2>/dev/null || true) | sort
```
...
# config settings can be fetched via eclusa-tools config-get if needed
```
...
```bash
grep -n "type=\"checkpoint" .eclusa/phases/XX-name/{phase}-{plan}-PLAN.md
```
...
**Pattern A:** init_agent_tracking → spawn Task(subagent_type="eclusa-executor", model=executor_model, isolation="worktree") with prompt: execute plan at [path], autonomous, all tasks + SUMMARY + commit, follow deviation/auth rules, report: plan name, tasks, SUMMARY path, commit hash → track agent_id → wait → update tracking → report.
...
```bash
if [ ! -f .eclusa/agent-history.json ]; then
  echo '{"version":"1.0","max_entries":50,"entries":[]}' > .eclusa/agent-history.json
fi
rm -f .eclusa/current-agent-id.txt
if [ -f .eclusa/current-agent-id.txt ]; then
  INTERRUPTED_ID=$(cat .eclusa/current-agent-id.txt)
...
2. Per segment:
   - Subagent route: spawn eclusa-executor for assigned tasks only. Prompt: task range, plan path, read full plan for context, execute assigned tasks, track deviations, NO SUMMARY/commit. Track via agent protocol.
...
```bash
cat .eclusa/phases/XX-name/{phase}-{plan}-PLAN.md
```
...
```bash
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" phases list --type summaries --raw
...
See `~/.claude/eclusa/references/tdd.md` for structure.
</tdd_plan_execution>
...
```bash
node ~/.claude/eclusa/bin/eclusa-tools.cjs commit-to-subrepo "{type}({phase}-{plan}): {description}" --files file1 file2 ...
```
...
See ~/.claude/eclusa/references/checkpoints.md for details.
</step>
...
```bash
NODE_REPAIR=$(node "./.claude/eclusa/bin/eclusa-tools.cjs" config-get workflow.node_repair 2>/dev/null || echo "true")
```

If `NODE_REPAIR` is `true`: invoke `@./.claude/eclusa/workflows/node-repair.md` with:
...
```bash
grep -A 50 "^user_setup:" .eclusa/phases/XX-name/{phase}-{plan}-PLAN.md | head -50
```

If user_setup exists: create `{phase}-USER-SETUP.md` using template `~/.claude/eclusa/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>

<step name="create_summary">
Create `{phase}-{plan}-SUMMARY.md` at `.eclusa/phases/XX-name/`. Use `~/.claude/eclusa/templates/summary.md`.
...
Update STATE.md using eclusa-tools:

```bash
# Advance plan counter (handles last-plan edge case)
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" state advance-plan

# Recalculate progress bar from disk state
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" state update-progress

# Record execution metrics
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" state record-metric \
...
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" state add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>

<step name="update_session_continuity">
Update session info using eclusa-tools:

```bash
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" state record-session \
...
```bash
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" roadmap update-plan-progress "${PHASE}"
```
...
```bash
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" requirements mark-complete ${REQ_IDS}
```
...
```bash
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" commit "docs({phase}-{plan}): complete [plan-name] plan" --files .eclusa/phases/XX-name/{phase}-{plan}-SUMMARY.md .eclusa/STATE.md .eclusa/ROADMAP.md .eclusa/REQUIREMENTS.md
```
</step>

<step name="update_codebase_map">
If .eclusa/codebase/ doesn't exist: skip.
...
```bash
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" commit "" --files .eclusa/codebase/*.md --amend
```
</step>
...
```bash
(ls -1 .eclusa/phases/[current-phase-dir]/*-PLAN.md 2>/dev/null || true) | wc -l
(ls -1 .eclusa/phases/[current-phase-dir]/*-SUMMARY.md 2>/dev/null || true) | wc -l
```

| Condition | Route | Action |
|-----------|-------|--------|
| summaries < plans | **A: More plans** | Find next PLAN without SUMMARY. Yolo: auto-continue. Interactive: show next plan, suggest `/eclusa:execute-phase {phase}` + `/eclusa:verify-work`. STOP here. |
| summaries = plans, current < highest phase | **B: Phase done** | Show completion, suggest `/eclusa:plan-phase {Z+1}` + `/eclusa:verify-work {Z}` + `/eclusa:discuss-phase {Z+1}` |
| summaries = plans, current = highest phase | **C: Milestone done** | Show banner, suggest `/eclusa:complete-milestone` + `/eclusa:verify-work` + `/eclusa:add-phase` |
