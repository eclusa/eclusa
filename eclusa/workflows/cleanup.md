...
Archive accumulated phase directories from completed milestones into `.eclusa/milestones/v{X.Y}-phases/`. Identifies which phases belong to each completed milestone, shows a dry-run summary, and moves directories on confirmation.

</purpose>

<required_reading>

1. `.eclusa/MILESTONES.md`
2. `.eclusa/milestones/` directory listing
3. `.eclusa/phases/` directory listing
...
Read `.eclusa/MILESTONES.md` to identify completed milestones and their versions.

```bash
cat .eclusa/MILESTONES.md
```
...
```bash
ls -d .eclusa/milestones/v*-phases 2>/dev/null || true
```
...
```bash
cat .eclusa/milestones/v{X.Y}-ROADMAP.md
```

Extract phase numbers and names from the archived roadmap (e.g., Phase 1: Foundation, Phase 2: Auth).

Check which of those phase directories still exist in `.eclusa/phases/`:

```bash
ls -d .eclusa/phases/*/ 2>/dev/null || true
```

Match phase directories to milestone membership. Only include directories that still exist in `.eclusa/phases/`.
...
Destination: .eclusa/milestones/v{X.Y}-phases/
...
Destination: .eclusa/milestones/v{X.Z}-phases/
```
...
```bash
mv .eclusa/phases/{dir} .eclusa/milestones/v{X.Y}-phases/
```
...
```bash
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" commit "chore: archive phase directories from completed milestones" --files .eclusa/milestones/ .eclusa/phases/
```
...
Archived:
{For each milestone}
- v{X.Y}: {N} phase directories → .eclusa/milestones/v{X.Y}-phases/

.eclusa/phases/ cleaned up.
```
...
- [ ] Phase directories moved to `.eclusa/milestones/v{X.Y}-phases/`
- [ ] Changes committed
