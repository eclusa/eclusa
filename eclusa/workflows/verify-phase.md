...
<required_reading>
@~/.claude/eclusa/references/verification-patterns.md
@~/.claude/eclusa/templates/verification-report.md
</required_reading>
...
grep -E "^| ${phase_number}" .eclusa/REQUIREMENTS.md 2>/dev/null || true
...
Use eclusa-tools to extract must_haves from each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" frontmatter get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" roadmap get-phase "${phase_number}" --raw)
```
...
Use eclusa-tools for artifact verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" verify artifacts "$plan")
...
Use eclusa-tools for key link verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" verify key-links "$plan")
...
```bash
grep -E "Phase ${PHASE_NUM}" .eclusa/REQUIREMENTS.md 2>/dev/null || true
```
...
See ~/.claude/eclusa/templates/verification-report.md for complete template.
</step>
