<purpose>
Generate a post-session summary document capturing work performed, outcomes achieved, and estimated resource usage. Writes SESSION_REPORT.md to .eclusa/reports/ for human review and stakeholder sharing.
</purpose>
...
Read `.eclusa/STATE.md` to get:
- Current milestone and phase
- Progress percentage
- Active blockers
- Recent decisions

Read `.eclusa/ROADMAP.md` to get milestone name and goals.

Check for existing reports:
```bash
ls -la .eclusa/reports/SESSION_REPORT*.md 2>/dev/null || echo "No previous reports"
```
</step>
...
Write `.eclusa/reports/SESSION_REPORT.md` (or `.eclusa/reports/YYYYMMDD-session-report.md` if previous reports exist):
...
*Generated by `/eclusa:session-report`*
```
</step>
...
```
💡 Run `/eclusa:session-report` at the end of each session to build a history of project activity.
```
</step>
...
- [ ] Report written to .eclusa/reports/
