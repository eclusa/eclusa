...
Debug files use the `.eclusa/debug/` path (hidden directory with leading dot).
</paths>
...
```bash
AGENT_SKILLS_DEBUGGER=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" agent-skills eclusa-debugger 2>/dev/null)
```
...
- truth: "Comment appears immediately after submission"
  ...
  debug_session: .eclusa/debug/comment-not-refreshing.md
```
...
```bash
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" commit "docs({phase_num}): add root causes from diagnosis" --files ".eclusa/phases/XX-name/{phase_num}-UAT.md"
```
</step>
