<purpose>
Retroactive 6-pillar visual audit of implemented frontend code. Standalone command that works on any project — Eclusa-managed or not. Produces scored UI-REVIEW.md with actionable findings.
</purpose>
...
AGENT_SKILLS_UI_REVIEWER=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" agent-skills eclusa-ui-reviewer 2>/dev/null)
```

Parse: `phase_dir`, `phase_number`, `phase_name`, `phase_slug`, `padded_phase`, `commit_docs`.

```bash
UI_AUDITOR_MODEL=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" resolve-model eclusa-ui-auditor --raw)
```

Display banner:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Eclusa ► UI AUDIT — PHASE {N}: {name}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
**If `SUMMARY_FILES` empty:** Exit — "Phase {N} not executed. Run /eclusa:execute-phase {N} first."
...
```markdown
Read ~/.claude/agents/eclusa-ui-auditor.md for instructions.
...
Task(
  ...
  subagent_type="eclusa-ui-auditor",
...
- `/eclusa:verify-work {N}` — UAT testing
- `/eclusa:plan-phase {N+1}` — plan next phase
...
```bash
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" commit "docs(${padded_phase}): UI audit review" --files "${PHASE_DIR}/${PADDED_PHASE}-UI-REVIEW.md"
```
...
- [ ] eclusa-ui-auditor spawned with correct context
