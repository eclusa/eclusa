<purpose>
Remove a Eclusa workspace, cleaning up git worktrees and deleting the workspace directory.
</purpose>
...
```bash
INIT=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" init remove-workspace "$WORKSPACE_NAME")
...
First run `/eclusa:list-workspaces` to show available workspaces, then ask:
