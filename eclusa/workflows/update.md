<purpose>
Check for Eclusa updates via npm, display changelog for versions between installed and latest, obtain user confirmation, and execute clean installation with cache clearing.
</purpose>
...
Detect whether Eclusa is installed locally or globally by checking both locations and validating install integrity.
...
Use `PREFERRED_RUNTIME` as the first runtime checked so `/eclusa:update` targets the runtime that invoked it.
...
for entry in "${ORDERED_RUNTIME_DIRS[@]}"; do
  runtime="${entry%%:*}"
  dir="${entry#*:}"
  if [ -f "./$dir/eclusa/VERSION" ] || [ -f "./$dir/eclusa/workflows/update.md" ]; then
    LOCAL_RUNTIME="$runtime"
    LOCAL_VERSION_FILE="./$dir/eclusa/VERSION"
    LOCAL_MARKER_FILE="./$dir/eclusa/workflows/update.md"
...
for entry in "${ORDERED_RUNTIME_DIRS[@]}"; do
  runtime="${entry%%:*}"
  dir="${entry#*:}"
  if [ -f "$HOME/$dir/eclusa/VERSION" ] || [ -f "$HOME/$dir/eclusa/workflows/update.md" ]; then
    GLOBAL_RUNTIME="$runtime"
    GLOBAL_VERSION_FILE="$HOME/$dir/eclusa/VERSION"
    GLOBAL_MARKER_FILE="$HOME/$dir/eclusa/workflows/update.md"
...
```bash
npm view eclusa version 2>/dev/null
```
...
To update manually: `npx eclusa --global`
```
...
⚠️  **Note:** The installer performs a clean install of Eclusa folders:
- `commands/eclusa/` will be wiped and replaced
- `eclusa/` will be wiped and replaced
- `agents/eclusa-*` files will be replaced
...
- Custom commands not in `commands/eclusa/` ✓
- Custom agents not prefixed with `eclusa-` ✓
- Custom hooks ✓
- Your CLAUDE.md files ✓

If you've modified any Eclusa files directly, they'll be automatically backed up to `eclusa-local-patches/` and can be reapplied with `/eclusa:reapply-patches` after the update.
```
...
**If LOCAL install:**
```bash
npx -y eclusa@latest "$RUNTIME_FLAG" --local
```

**If GLOBAL install:**
```bash
npx -y eclusa@latest "$RUNTIME_FLAG" --global
```

**If UNKNOWN install:**
```bash
npx -y eclusa@latest --claude --global
```
...
for dir in .claude .config/opencode .opencode .gemini .codex; do
  rm -f "./$dir/cache/eclusa-update-check.json"
  rm -f "$HOME/$dir/cache/eclusa-update-check.json"
done
```

The SessionStart hook (`eclusa-check-update.js`) writes to the detected runtime's cache directory, so all paths must be cleared to prevent stale update indicators.
</step>
...
```
╔═══════════════════════════════════════════════════════════╗
║  Eclusa Updated: v1.5.10 → v1.5.15                           ║
╚═══════════════════════════════════════════════════════════╝

⚠️  Restart your runtime to pick up the new commands.

[View full changelog](https://github.com/eclusa/eclusa/blob/main/CHANGELOG.md)
```
</step>
...
Check for eclusa-local-patches/backup-meta.json in the config directory.

**If patches found:**

```
Local patches were backed up before the update.
Run /eclusa:reapply-patches to merge your modifications into the new version.
```
