...
```bash
INIT=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" init manager)
...
All {phase_count} phases done. Ready for final steps:
  → /eclusa:verify-work — run acceptance testing
  → /eclusa:complete-milestone — archive and wrap up
```
...
Handle responses:
- "Verify work": `Skill(skill="eclusa:verify-work")`  then loop to dashboard.
- "Complete milestone": `Skill(skill="eclusa:complete-milestone")` then exit.
...
**Auto-refresh:** If background agents are running (`is_active` is true for any phase), set a 60-second auto-refresh cycle. After presenting the action menu, if no user input is received within 60 seconds, automatically refresh the dashboard. This interval is configurable via `manager_refresh_interval` in Eclusa config (default: 60 seconds, set to 0 to disable).
...
```
Skill(skill="eclusa:discuss-phase", args="{PHASE_NUM}")
```
...
Task(
  ...
  prompt="You are running the Eclusa plan-phase workflow for phase {N} of the project.
...
Steps:
1. Read the plan-phase workflow: cat ~/.claude/eclusa/workflows/plan-phase.md
2. Run: node \"$HOME/.claude/eclusa/bin/eclusa-tools.cjs\" init plan-phase {N}
...
5. Spawn a eclusa-planner subagent via Task() to create the plans.
6. If plan-checker is enabled, spawn a eclusa-plan-checker subagent to verify.
...
Task(
  ...
  run_in_background=true,
  prompt="You are running the Eclusa execute-phase workflow for phase {N} of the project.
...
Steps:
1. Read the execute-phase workflow: cat ~/.claude/eclusa/workflows/execute-phase.md
2. Run: node \"$HOME/.claude/eclusa/bin/eclusa-tools.cjs\" init execute-phase {N}
3. Follow the workflow steps: discover plans, analyze dependencies, group into waves.
4. For each wave, spawn eclusa-executor subagents via Task() to execute plans in parallel.
5. After all waves complete, spawn a eclusa-verifier subagent if verifier is enabled.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 Resume anytime: /eclusa:manager
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

**Note:** Any background agents still running will continue to completion. Their results will be visible on next `/eclusa:manager` or `/eclusa:progress` invocation.
