<purpose>
Create structured `.eclusa/HANDOFF.json` and `.continue-here.md` handoff files to preserve complete work state across sessions. The JSON provides machine-readable state for `/eclusa:resume-work`; the markdown provides human-readable context.
</purpose>
...
(ls -lt .eclusa/phases/*/PLAN.md 2>/dev/null || true) | head -1 | grep -oP 'phases/\K[^/]+' || true
```
...
grep -l "To be filled\|placeholder\|TBD" .eclusa/phases/*/*.md 2>/dev/null || true
```
Report any summaries with placeholder content as incomplete items.
</step>

<step name="write_structured">
**Write structured handoff to `.eclusa/HANDOFF.json`:**

```bash
timestamp=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" current-timestamp full --raw)
```
...
<step name="write">
**Write handoff to `.eclusa/phases/XX-name/.continue-here.md`:**
...
<step name="commit">
```bash
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" commit "wip: [phase-name] paused at task [X]/[Y]" --files .eclusa/phases/*/.continue-here.md .eclusa/HANDOFF.json
```
</step>
...
```
✓ Handoff created:
  - .eclusa/HANDOFF.json (structured, machine-readable)
  - .eclusa/phases/[XX-name]/.continue-here.md (human-readable)
