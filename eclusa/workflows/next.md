<purpose>
Detect current project state and automatically advance to the next logical Eclusa workflow step.
...
```bash
# Get state snapshot
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" state json 2>/dev/null || echo "{}"
```

Also read:
- `.eclusa/STATE.md` — current phase, progress, plan counts
- `.eclusa/ROADMAP.md` — milestone structure and phase list
...
If no `.eclusa/` directory exists:
```
No Eclusa project detected. Run `/eclusa:new-project` to get started.
```
Exit.
</step>
...
→ Next action: `/eclusa:discuss-phase <first-phase>`
...
▶ **Next step:** `/eclusa:[command] [args]`
  [One-line explanation of why this is the next step]
```

Then immediately invoke the determined command via SlashCommand.
Do not ask for confirmation — the whole point of `/eclusa:next` is zero-friction advancement.
</step>
