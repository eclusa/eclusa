...
Seeds auto-surface during /eclusa:new-milestone when trigger conditions match the
...
EXISTING=$( (ls .eclusa/seeds/SEED-*.md 2>/dev/null || true) | wc -l )
NEXT=$((EXISTING + 1))
...
Write `.eclusa/seeds/SEED-{PADDED}-{slug}.md`:
...
This seed should be presented during `/eclusa:new-milestone` when the milestone
...
```bash
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" commit "docs: plant seed — {$IDEA}" --files .eclusa/seeds/SEED-{PADDED}-{slug}.md
```
</step>
...
Trigger: {$TRIGGER}
Scope: {$SCOPE}
File: .eclusa/seeds/SEED-{PADDED}-{slug}.md

This seed will surface automatically when you run /eclusa:new-milestone
...
<success_criteria>
- [ ] Seed file created in .eclusa/seeds/
