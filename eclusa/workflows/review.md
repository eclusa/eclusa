...
Then read:
1. `.eclusa/PROJECT.md` (first 80 lines — project context)
2. Phase section from `.eclusa/ROADMAP.md`
3. All `*-PLAN.md` files in the phase directory
4. `*-CONTEXT.md` if present (user decisions)
5. `*-RESEARCH.md` if present (domain research)
6. `.eclusa/REQUIREMENTS.md` (requirements this phase addresses)
</step>
...
Write to a temp file: `/tmp/eclusa-review-prompt-{phase}.md`
</step>
...
**Gemini:**
```bash
gemini -p "$(cat /tmp/eclusa-review-prompt-{phase}.md)" 2>/dev/null > /tmp/eclusa-review-gemini-{phase}.md
```

**Claude (separate session):**
```bash
claude -p "$(cat /tmp/eclusa-review-prompt-{phase}.md)" --no-input 2>/dev/null > /tmp/eclusa-review-claude-{phase}.md
```

**Codex:**
```bash
codex exec --skip-git-repo-check "$(cat /tmp/eclusa-review-prompt-{phase}.md)" 2>/dev/null > /tmp/eclusa-review-codex-{phase}.md
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Eclusa ► CROSS-AI REVIEW — Phase {N}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
Commit:
```bash
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" commit "docs: cross-AI review for phase {N}" --files {phase_dir}/{padded_phase}-REVIEWS.md
```
</step>
...
- [ ] User knows how to use feedback (/eclusa:plan-phase --reviews)
</success_criteria>
