<purpose>
Create an isolated workspace directory with git repo copies (worktrees or clones) and an independent `.eclusa/` directory. Supports multi-repo orchestration and single-repo feature branch isolation.
</purpose>
...
```bash
INIT=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" init new-workspace)
...
Run this command from a directory containing git repos, or specify repos explicitly:
  /eclusa:new-workspace --name my-workspace --repos /path/to/repo1,/path/to/repo2
```
Exit.
...
Usage:
  /eclusa:new-workspace --name my-workspace --repos repo1,repo2 --auto
```
Exit.
...
Next steps:
  cd $TARGET_PATH
  /eclusa:new-project    # Initialize Eclusa in the workspace
```
...
**Offer to initialize Eclusa (if not `--auto`):**

Use AskUserQuestion:
- header: "Initialize Eclusa"
- question: "Would you like to initialize a Eclusa project in the new workspace?"
- options:
  - "Yes — run /eclusa:new-project" → tell user to `cd $TARGET_PATH` first, then run `/eclusa:new-project`
...
- [ ] `.eclusa/` directory initialized at workspace root
