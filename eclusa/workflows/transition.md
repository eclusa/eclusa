...
There is no `/eclusa:transition` command. This workflow is invoked automatically by
`execute-phase` during auto-advance, or inline by the orchestrator after phase
verification. Users should never be told to run `/eclusa:transition`.

**Valid user commands for phase progression:**
- `/eclusa:discuss-phase {N}` — discuss a phase before planning
- `/eclusa:plan-phase {N}` — plan a phase
- `/eclusa:execute-phase {N}` — execute a phase
- `/eclusa:progress` — see roadmap progress
...
```bash
(ls .eclusa/phases/XX-current/*-PLAN.md 2>/dev/null || true) | sort
(ls .eclusa/phases/XX-current/*-SUMMARY.md 2>/dev/null || true) | sort
```
...
```bash
cat .eclusa/config.json 2>/dev/null || true
```
...
OUTSTANDING=""
for f in .eclusa/phases/XX-current/*-UAT.md .eclusa/phases/XX-current/*-VERIFICATION.md; do
  [ -f "$f" ] || continue
...
These will carry forward as debt. Review: `/eclusa:audit-uat`
```
...
```bash
ls .eclusa/phases/XX-current/.continue-here*.md 2>/dev/null || true
```
...
**Delegate ROADMAP.md and STATE.md updates to eclusa-tools:**

```bash
TRANSITION=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" phase complete "${current_phase}")
```
...
```bash
cat .eclusa/phases/XX-current/*-SUMMARY.md
```
...
**Note:** Basic position updates (Current Phase, Status, Current Plan, Last Activity) were already handled by `eclusa-tools phase complete` in the update_roadmap_and_state step.

Verify the updates are correct by reading STATE.md. If the progress bar needs updating, use:

```bash
PROGRESS=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" progress bar --raw)
```
...
See: .eclusa/PROJECT.md (updated [today])
...
**Use the transition result from `eclusa-tools phase complete`:**
...
**Skip this check if NOT in workstream mode** (i.e., `ECLUSA_WORKSTREAM` is not set / flat mode).
In flat mode, go directly to **Route B**.

```bash
# Only check if we're in workstream mode
if [ -n "$ECLUSA_WORKSTREAM" ]; then
  WS_LIST=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" workstream list --raw)
fi
```
...
Filter out the current workstream (`$ECLUSA_WORKSTREAM`) and any workstreams with
...
```bash
ls .eclusa/phases/*[X+1]*/*-CONTEXT.md 2>/dev/null || true
```
...
Exit skill and invoke SlashCommand("/eclusa:plan-phase [X+1] --auto ${ECLUSA_WS}")
...
Exit skill and invoke SlashCommand("/eclusa:discuss-phase [X+1] --auto ${ECLUSA_WS}")
...
`/eclusa:discuss-phase [X+1] ${ECLUSA_WS}` — gather context and clarify approach
...
**Also available:**
- `/eclusa:plan-phase [X+1] ${ECLUSA_WS}` — skip discussion, plan directly
- `/eclusa:research-phase [X+1] ${ECLUSA_WS}` — investigate unknowns
...
`/eclusa:plan-phase [X+1] ${ECLUSA_WS}`
...
**Also available:**
- `/eclusa:discuss-phase [X+1] ${ECLUSA_WS}` — revisit context
- `/eclusa:research-phase [X+1] ${ECLUSA_WS}` — investigate unknowns
...
```bash
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" config-set workflow._auto_chain_active false
```
...
`/eclusa:workstreams complete {current_ws_name} ${ECLUSA_WS}`

See overall milestone progress:

`/eclusa:workstreams progress ${ECLUSA_WS}`
...
Do NOT suggest `/eclusa:complete-milestone` or `/eclusa:new-milestone`.
...
Exit skill and invoke SlashCommand("/eclusa:complete-milestone {version} ${ECLUSA_WS}")
...
`/eclusa:complete-milestone {version} ${ECLUSA_WS}`
