<purpose>
Interactive configuration of Eclusa workflow agents (research, plan_check, verifier) and model profile selection via multi-question prompt. Updates .eclusa/config.json with user preferences. Optionally saves settings as global defaults (~/.eclusa/defaults.json) for future projects.
</purpose>
...
```bash
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" config-ensure-section
INIT=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" state load)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
```

Creates `.eclusa/config.json` with defaults if missing and loads current config values.
</step>

<step name="read_current">
```bash
cat .eclusa/config.json
```
...
- `workflow.ui_safety_gate` — prompt to run /eclusa:ui-phase before planning frontend phases (default: true if absent)
...
AskUserQuestion([
  ...
  {
    question: "Enable UI Safety Gate? (prompts to run /eclusa:ui-phase before planning frontend phases)",
    header: "UI Gate",
    multiSelect: false,
    options: [
      { label: "Yes (Recommended)", description: "plan-phase asks to run /eclusa:ui-phase first when frontend indicators detected." },
  ...
  },
  {
    ...
    options: [
      ...
      { label: "Per Phase", description: "Create branch for each phase (eclusa/phase-{N}-{name})" },
      { label: "Per Milestone", description: "Create branch for entire milestone (eclusa/{version}-{name})" }
    ]
  },
  {
  ...
  },
  {
    ...
    options: [
      ...
      { label: "Yes", description: "Skip discuss in /eclusa:autonomous — chain directly to plan. Best for backend/pipeline work where phase descriptions are the spec." }
    ]
  }
])
```
...
Write updated config to `.eclusa/config.json`.
</step>
...
AskUserQuestion([
  {
    ...
    multiSelect: false,
    options: [
      { label: "Yes", description: "New projects start with these settings (saved to ~/.eclusa/defaults.json)" },
...
If "Yes": write the same config object (minus project-specific fields like `brave_search`) to `~/.eclusa/defaults.json`:

```bash
mkdir -p ~/.eclusa
```

Write `~/.eclusa/defaults.json` with:
```json
{
  "mode": <current>,
  "granularity": <current>,
...
These settings apply to future /eclusa:plan-phase and /eclusa:execute-phase runs.

Quick commands:
- /eclusa:set-profile <profile> — switch model profile
- /eclusa:plan-phase --research — force research
- /eclusa:plan-phase --skip-research — skip research
- /eclusa:plan-phase --skip-verify — skip plan check
```
</step>
...
- [ ] User offered to save as global defaults (~/.eclusa/defaults.json)
