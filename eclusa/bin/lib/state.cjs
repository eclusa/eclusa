...
function buildStateFrontmatter(bodyContent, cwd) {
  ...
  const fm = { eclusa_state_version: '1.0' };
...
/**
 * Write a WAITING.json signal file when Eclusa hits a decision point.
 * External watchers (fswatch, polling, orchestrators) can detect this.
 * File is written to .eclusa/WAITING.json (or .eclusa/WAITING.json if .eclusa exists).
 * Fixes #1034.
 */
function cmdSignalWaiting(cwd, type, question, options, phase, raw) {
  const eclusaDir = fs.existsSync(path.join(cwd, '.eclusa')) ? path.join(cwd, '.eclusa') : planningDir(cwd);
  const waitingPath = path.join(eclusaDir, 'WAITING.json');
...
function cmdSignalResume(cwd, raw) {
  const paths = [
    path.join(cwd, '.eclusa', 'WAITING.json'),
