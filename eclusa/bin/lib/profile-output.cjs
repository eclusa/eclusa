...
const CLAUDE_MD_FALLBACKS = {
  project: 'Project not yet initialized. Run /eclusa:new-project to set up.',
...
function extractSectionContent(fileContent, sectionName) {
  const startMarker = `<!-- Eclusa:${sectionName}-start`;
  const endMarker = `<!-- Eclusa:${sectionName}-end -->`;
...
function buildSection(sectionName, sourceFile, content) {
  return [
    `<!-- Eclusa:${sectionName}-start source:${sourceFile} -->`,
    content,
    `<!-- Eclusa:${sectionName}-end -->`,
  ].join('\n');
}
...
function generateProjectSection(cwd) {
  const projectPath = path.join(cwd, '.eclusa', 'PROJECT.md');
...
function generateStackSection(cwd) {
  const codebasePath = path.join(cwd, '.eclusa', 'codebase', 'STACK.md');
  const researchPath = path.join(cwd, '.eclusa', 'research', 'STACK.md');
...
function generateConventionsSection(cwd) {
  const conventionsPath = path.join(cwd, '.eclusa', 'codebase', 'CONVENTIONS.md');
...
function generateArchitectureSection(cwd) {
  const architecturePath = path.join(cwd, '.eclusa', 'codebase', 'ARCHITECTURE.md');
...
function cmdWriteProfile(cwd, options, raw) {
  ...
  if (!outputPath) {
    outputPath = path.join(os.homedir(), '.claude', 'eclusa', 'USER-PROFILE.md');
...
function cmdGenerateDevPreferences(cwd, options, raw) {
  ...
  if (analysis.data_source === 'questionnaire') {
    stackBlock = 'Stack preferences not available (questionnaire-only profile). Run `/eclusa:profile-user --refresh` with session data to populate.';
  ...
  let outputPath = options.output;
  if (!outputPath) {
    outputPath = path.join(os.homedir(), '.claude', 'commands', 'eclusa', 'dev-preferences.md');
  } else if (!path.isAbsolute(outputPath)) {
    outputPath = path.join(cwd, outputPath);
  }
  ...
  const result = {
    ...
    command_name: '/eclusa:dev-preferences',
...
function cmdGenerateClaudeProfile(cwd, options, raw) {
  ...
  const sectionLines = [
    ...
    '## Developer Profile',
    '',
    `> Generated by Eclusa from ${dataSource}. Run \`/eclusa:profile-user --refresh\` to update.`,
    '',
  ...
  if (fs.existsSync(targetPath)) {
    ...
    const startMarker = '<!-- Eclusa:profile-start -->';
    const endMarker = '<!-- Eclusa:profile-end -->';
...
function cmdGenerateClaudeMd(cwd, options, raw) {
  ...
  const sectionHeadings = {
    ...
    workflow: '## Eclusa Workflow Enforcement',
  };
  ...
  if (existingContent === null) {
    ...
    for (const name of MANAGED_SECTIONS) {
      ...
      const hasMarkers = fileContent.indexOf(`<!-- Eclusa:${name}-start`) !== -1;
    ...
    if (!options.auto && fileContent.indexOf('<!-- Eclusa:profile-start') === -1) {
  ...
  let profileStatus;
  if (finalContent && finalContent.indexOf('<!-- Eclusa:profile-start') !== -1) {
    if (action === 'created' || existingContent.indexOf('<!-- Eclusa:profile-start') === -1) {
  ...
  if (profileStatus === 'placeholder_added') message += ' Run /eclusa:profile-user to unlock Developer Profile.';
