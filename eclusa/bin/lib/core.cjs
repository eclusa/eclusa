...
/**
 * Walk up from `startDir` to find the project root that owns `.eclusa/`.
 *
 * In multi-repo workspaces, Claude may open inside a sub-repo (e.g. `backend/`)
 * instead of the project root. This function prevents `.eclusa/` from being
 ...
 * 1. Parent has `.eclusa/config.json` with `sub_repos` listing this directory
 * 2. Parent has `.eclusa/config.json` with `multiRepo: true` (legacy format)
 * 3. Parent has `.eclusa/` and current dir has its own `.git` (heuristic)
 *
 * Returns `startDir` unchanged when no ancestor `.eclusa/` is found (first-run
...
function findProjectRoot(startDir) {
  ...
  // If startDir already contains .eclusa/, it IS the project root.
  // Do not walk up to a parent workspace that also has .eclusa/ (#1362).
  const ownPlanning = path.join(resolved, '.eclusa');
  ...
  // as well as the common case where .git lives at the same level as .eclusa/.
  ...
  while (dir !== root) {
    ...
    const parentPlanning = path.join(parent, '.eclusa');
    if (fs.existsSync(parentPlanning) && fs.statSync(parentPlanning).isDirectory()) {
      ...
      // Heuristic: parent has .eclusa/ and we're inside a git repo
...
/**
 * Remove stale eclusa-* temp files/dirs older than maxAgeMs (default: 5 minutes).
 * Runs opportunistically before each new temp file write to prevent unbounded accumulation.
 * @param {string} prefix - filename prefix to match (e.g., 'eclusa-')
 * @param {object} opts
 ...
 */
function reapStaleTempFiles(prefix = 'eclusa-', { maxAgeMs = 5 * 60 * 1000, dirsOnly = false } = {}) {
  try {
...
function output(result, raw, rawValue) {
  ...
  if (raw && rawValue !== undefined) {
    ...
    if (json.length > 50000) {
      reapStaleTempFiles();
      const tmpPath = path.join(require('os').tmpdir(), `eclusa-${Date.now()}.json`);
  ...
  try {
    ...
    return {
      ...
      commit_docs: (() => {
        ...
        // Auto-detection: when no explicit value and .eclusa/ is gitignored,
        // default to false instead of true
        if (isGitIgnored(cwd, '.eclusa/')) return false;
...
function isGitIgnored(cwd, targetPath) {
  try {
    ...
    // .gitignore explicitly lists them — a common source of confusion when .eclusa/
 ...
 * Applied at write points so Eclusa-generated .eclusa/ files are IDE-friendly.
 *
 * Rules enforced:
 ...
 * In a linked worktree, .eclusa/ lives in the main worktree, not in the linked one.
 * Returns the main worktree path, or cwd if not in a worktree.
 */
function resolveWorktreeRoot(cwd) {
  // If the current directory already has its own .eclusa/, respect it.
...
/**
 * Acquire a file-based lock for .eclusa/ writes.
...
/**
 * Get the .eclusa directory path, workstream-aware.
 * When a workstream is active (via explicit ws arg or ECLUSA_WORKSTREAM env var),
 * returns `.eclusa/workstreams/{ws}/`. Otherwise returns `.eclusa/`.
 *
 * @param {string} cwd - project root
 * @param {string} [ws] - explicit workstream name; if omitted, checks ECLUSA_WORKSTREAM env var
 */
function planningDir(cwd, ws) {
  if (ws === undefined) ws = process.env.ECLUSA_WORKSTREAM || null;
  if (!ws) return path.join(cwd, '.eclusa');
  return path.join(cwd, '.eclusa', 'workstreams', ws);
}

/** Always returns the root .eclusa/ path, ignoring workstreams. For shared resources. */
function planningRoot(cwd) {
  return path.join(cwd, '.eclusa');
}

/**
 * Get common .eclusa file paths, workstream-aware.
 * Scoped paths (state, roadmap, phases, requirements) resolve to the active workstream.
 * Shared paths (project, config) always resolve to the root .eclusa/.
 */
function planningPaths(cwd, ws) {
  const base = planningDir(cwd, ws);
  const root = path.join(cwd, '.eclusa');
  return {
    planning: base,
...
/**
 * Get the active workstream name from .eclusa/active-workstream file.
...
function findPhaseInternal(cwd, phase) {
  ...
  const milestonesDir = path.join(cwd, '.eclusa', 'milestones');
  ...
  try {
    ...
    for (const archiveName of archiveDirs) {
      ...
      const relBase = '.eclusa/milestones/' + archiveName;
    ...
    for (const archiveName of phaseDirs) {
      ...
      for (const dir of dirs) {
        results.push({
          name: dir,
          milestone: version,
          basePath: path.join('.eclusa', 'milestones', archiveName),
...
/**
 * Resolve the agents directory from the Eclusa install location.
 * eclusa-tools.cjs lives at <configDir>/eclusa/bin/eclusa-tools.cjs,
 * so agents/ is at <configDir>/agents/.
 *
 * @returns {string} Absolute path to the agents directory
 */
function getAgentsDir() {
  // __dirname is eclusa/bin/lib/ → go up 3 levels to configDir
  return path.join(__dirname, '..', '..', '..', 'agents');
}

/**
 * Check which Eclusa agents are installed on disk.
