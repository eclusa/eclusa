...
function cmdMilestoneComplete(cwd, version, options, raw) {
  ...
  const milestonesPath = path.join(cwd, '.eclusa', 'MILESTONES.md');
  const archiveDir = path.join(cwd, '.eclusa', 'milestones');
  ...
  const auditFile = path.join(cwd, '.eclusa', `${version}-MILESTONE-AUDIT.md`);
