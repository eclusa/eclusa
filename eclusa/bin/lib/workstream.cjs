 ...
 * REQUIREMENTS.md, and phases/ into .eclusa/workstreams/{name}/ directories.
 *
 * When no workstreams/ directory exists, Eclusa operates in "flat mode" with
 * everything at .eclusa/ — backward compatible with pre-workstream installs.
 */
...
/**
 * Migrate flat .eclusa/ layout to workstream mode.
 * Moves per-workstream files (ROADMAP.md, STATE.md, REQUIREMENTS.md, phases/)
 * into .eclusa/workstreams/{name}/. Shared files (PROJECT.md, config.json,
...
function migrateToWorkstreams(cwd, workstreamName) {
  ...
  if (fs.existsSync(path.join(baseDir, 'workstreams'))) {
    throw new Error('Already in workstream mode — .eclusa/workstreams/ exists');
  }
...
function cmdWorkstreamCreate(cwd, name, options, raw) {
  ...
  if (!fs.existsSync(baseDir)) {
    error('.eclusa/ directory not found — run /eclusa:new-project first');
  }
