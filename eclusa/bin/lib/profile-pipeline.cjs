...
async function cmdExtractMessages(projectArg, options, raw, overridePath) {
  ...
  reapStaleTempFiles('eclusa-pipeline-', { dirsOnly: true });
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'eclusa-pipeline-'));
...
async function cmdProfileSample(overridePath, options, raw) {
  ...
  reapStaleTempFiles('eclusa-profile-', { dirsOnly: true });
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'eclusa-profile-'));
