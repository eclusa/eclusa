...
function cmdValidateHealth(cwd, options, raw) {
  ...
  if (resolved === os.homedir()) {
    output({
      status: 'error',
      errors: [{ code: 'E010', message: `CWD is home directory (${resolved}) — health check would read the wrong .eclusa/ directory. Run from your project root instead.`, fix: 'cd into your project directory and retry' }],
      warnings: [],
  ...
  // ─── Check 1: .eclusa/ exists ───────────────────────────────────────────
  if (!fs.existsSync(planBase)) {
    addIssue('error', 'E001', '.eclusa/ directory not found', 'Run /eclusa:new-project to initialize');
    output({
      status: 'broken',
      errors,
      warnings,
  ...
  if (!fs.existsSync(projectPath)) {
    addIssue('error', 'E002', 'PROJECT.md not found', 'Run /eclusa:new-project to create');
  } else {
  ...
  if (!fs.existsSync(roadmapPath)) {
    addIssue('error', 'E003', 'ROADMAP.md not found', 'Run /eclusa:new-milestone to create roadmap');
  }

  // ─── Check 4: STATE.md exists and references valid phases ─────────────────
  if (!fs.existsSync(statePath)) {
    addIssue('error', 'E004', 'STATE.md not found', 'Run /eclusa:health --repair to regenerate', true);
      ...
      if (!diskPhases.has(ref) && !diskPhases.has(normalizedRef) && !diskPhases.has(String(parseInt(ref, 10)))) {
        ...
        if (diskPhases.size > 0) {
          addIssue(
            ...
            'Review STATE.md manually before changing it; /eclusa:health --repair will not overwrite an existing STATE.md for phase mismatches'
          );
        }
      }
    }
  ...
  if (!fs.existsSync(configPath)) {
    addIssue('warning', 'W003', 'config.json not found', 'Run /eclusa:health --repair to create with defaults', true);
    ...
    try {
      ...
      addIssue('error', 'E005', `config.json: JSON parse error - ${err.message}`, 'Run /eclusa:health --repair to reset to defaults', true);
  ...
  if (fs.existsSync(configPath)) {
    try {
      ...
      if (configParsed.workflow && configParsed.workflow.nyquist_validation === undefined) {
        addIssue('warning', 'W008', 'config.json: workflow.nyquist_validation absent (defaults to enabled but agents may skip)', 'Run /eclusa:health --repair to add key', true);
    ...
    for (const e of phaseEntries) {
      ...
      if (hasResearch && !hasValidation) {
        ...
        if (researchContent.includes('## Validation Architecture')) {
          addIssue('warning', 'W009', `Phase ${e.name}: has Validation Architecture in RESEARCH.md but no VALIDATION.md`, 'Re-run /eclusa:plan-phase with --research to regenerate');
        }
      }
    }
  } catch { /* intentionally empty */ }
  ...
  // Verify Eclusa agents are installed. Missing agents cause Task(subagent_type=...)
    ...
    if (!agentStatus.agents_installed) {
      if (agentStatus.installed_agents.length === 0) {
        addIssue('warning', 'W010',
          `No Eclusa agents found in ${agentStatus.agents_dir} — Task(subagent_type="eclusa-*") will fall back to general-purpose`,
          'Run the Eclusa installer: npx eclusa@latest');
      } else {
        addIssue('warning', 'W010',
          `Missing ${agentStatus.missing_agents.length} Eclusa agents: ${agentStatus.missing_agents.join(', ')} — affected workflows will fall back to general-purpose`,
          'Run the Eclusa installer: npx eclusa@latest');
      }
    }
      ...
      try {
        switch (repair) {
          ...
          case 'regenerateState': {
            ...
            stateContent += `See: .eclusa/PROJECT.md\n\n`;
            stateContent += `## Position\n\n`;
            stateContent += `**Milestone:** ${milestone.version} ${milestone.name}\n`;
            stateContent += `**Current phase:** (determining...)\n`;
            stateContent += `**Status:** Resuming\n\n`;
            stateContent += `## Session Log\n\n`;
            stateContent += `- ${new Date().toISOString().split('T')[0]}: STATE.md regenerated by /eclusa:health --repair\n`;
