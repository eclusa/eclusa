 ...
 * Workflows use this to prefix `.eclusa/` paths correctly when Claude's CWD
...
function cmdInitExecutePhase(cwd, phase, raw) {
  ...
  const result = {
    // Models
    executor_model: resolveModelInternal(cwd, 'eclusa-executor'),
    verifier_model: resolveModelInternal(cwd, 'eclusa-verifier'),
...
function cmdInitPlanPhase(cwd, phase, raw) {
  ...
  const result = {
    // Models
    researcher_model: resolveModelInternal(cwd, 'eclusa-phase-researcher'),
    planner_model: resolveModelInternal(cwd, 'eclusa-planner'),
    checker_model: resolveModelInternal(cwd, 'eclusa-plan-checker'),
...
function cmdInitNewProject(cwd, raw) {
  ...
  const firecrawlKeyFile = path.join(homedir, '.eclusa', 'firecrawl_api_key');
  const hasFirecrawl = !!(process.env.FIRECRAWL_API_KEY || fs.existsSync(firecrawlKeyFile));

  // Detect Exa API key availability
  const exaKeyFile = path.join(homedir, '.eclusa', 'exa_api_key');
  ...
  try {
    ...
    const skipDirs = new Set(['node_modules', '.git', '.eclusa', '.claude', '__pycache__', 'target', 'dist', 'build']);
  ...
  const result = {
    // Models
    researcher_model: resolveModelInternal(cwd, 'eclusa-project-researcher'),
    synthesizer_model: resolveModelInternal(cwd, 'eclusa-research-synthesizer'),
    roadmapper_model: resolveModelInternal(cwd, 'eclusa-roadmapper'),

    // Config
    commit_docs: config.commit_docs,

    // Existing state
    project_exists: pathExistsInternal(cwd, '.eclusa/PROJECT.md'),
    has_codebase_map: pathExistsInternal(cwd, '.eclusa/codebase'),
    planning_exists: pathExistsInternal(cwd, '.eclusa'),

    // Brownfield detection
    has_existing_code: hasCode,
    has_package_file: hasPackageFile,
    is_brownfield: hasCode || hasPackageFile,
    needs_codebase_map: (hasCode || hasPackageFile) && !pathExistsInternal(cwd, '.eclusa/codebase'),
...
function cmdInitQuick(cwd, description, raw) {
  ...
  const result = {
    ...
    // Paths
    quick_dir: '.eclusa/quick',
    task_dir: slug ? `.eclusa/quick/${quickId}-${slug}` : null,
...
function cmdInitMapCodebase(cwd, raw) {
  ...
  const result = {
    // Models
    mapper_model: resolveModelInternal(cwd, 'eclusa-codebase-mapper'),
    ...
    // File existence
    planning_exists: pathExistsInternal(cwd, '.eclusa'),
    codebase_dir_exists: pathExistsInternal(cwd, '.eclusa/codebase'),
  };
...
function cmdInitManager(cwd, raw) {
  ...
  if (!fs.existsSync(paths.roadmap)) {
    error('No ROADMAP.md found. Run /eclusa:new-milestone first.');
  }
  if (!fs.existsSync(paths.state)) {
    error('No STATE.md found. Run /eclusa:new-milestone first.');
  }
  ...
  try {
    const waitingPath = path.join(cwd, '.eclusa', 'WAITING.json');
  ...
  for (const phase of phases) {
    ...
    if (phase.disk_status === 'planned' && phase.deps_satisfied) {
      recommendedActions.push({
        ...
        command: `/eclusa:execute-phase ${phase.number}`,
      });
      ...
      recommendedActions.push({
        ...
        command: `/eclusa:plan-phase ${phase.number}`,
      });
      ...
      recommendedActions.push({
        ...
        command: `/eclusa:discuss-phase ${phase.number}`,
      });
    }
  }
...
function cmdInitNewWorkspace(cwd, raw) {
  ...
  const defaultBase = path.join(homedir, 'eclusa-workspaces');
...
function cmdInitListWorkspaces(cwd, raw) {
  ...
  if (fs.existsSync(defaultBase)) {
    ...
    for (const entry of entries) {
      ...
      hasProject = fs.existsSync(path.join(wsPath, '.eclusa', 'PROJECT.md'));
 ...
 * @param {string} agentType - The agent type (e.g., 'eclusa-executor', 'eclusa-planner')
 ...
 * Used by workflows: SKILLS=$(node "$TOOLS" agent-skills eclusa-executor 2>/dev/null)
 */
