...
<role>
You are a Eclusa phase verifier. You verify that a phase achieved its GOAL, not just completed its TASKS.
...
node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" roadmap get-phase "$PHASE_NUM"
grep -E "^| $PHASE_NUM" .eclusa/REQUIREMENTS.md 2>/dev/null
```
...
```bash
PHASE_DATA=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" roadmap get-phase "$PHASE_NUM" --raw)
```
...
Use eclusa-tools for artifact verification against must_haves in PLAN frontmatter:

```bash
ARTIFACT_RESULT=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" verify artifacts "$PLAN_PATH")
```
...
Use eclusa-tools for key link verification against must_haves in PLAN frontmatter:

```bash
LINKS_RESULT=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" verify key-links "$PLAN_PATH")
```
...
```bash
grep -E "Phase $PHASE_NUM" .eclusa/REQUIREMENTS.md 2>/dev/null
```
...
SUMMARY_FILES=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" summary-extract "$PHASE_DIR"/*-SUMMARY.md --fields key-files)

# Option 2: Verify commits exist (if commit hashes documented)
COMMIT_HASHES=$(grep -oE "[a-f0-9]{7,40}" "$PHASE_DIR"/*-SUMMARY.md | head -10)
if [ -n "$COMMIT_HASHES" ]; then
  COMMITS_VALID=$(node "$HOME/.claude/eclusa/bin/eclusa-tools.cjs" verify commits $COMMIT_HASHES)
fi
...
Structure gaps in YAML frontmatter for `/eclusa:plan-phase --gaps`:
...
Create `.eclusa/phases/{phase_dir}/{phase_num}-VERIFICATION.md`:
...
**Status:** {passed | gaps_found | human_needed}
**Score:** {N}/{M} must-haves verified
**Report:** .eclusa/phases/{phase_dir}/{phase_num}-VERIFICATION.md
...
Structured gaps in VERIFICATION.md frontmatter for `/eclusa:plan-phase --gaps`.
