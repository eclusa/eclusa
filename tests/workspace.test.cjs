/**
 * Eclusa Workspace Tests
 *
 * Tests for /eclusa:new-workspace, /eclusa:list-workspaces, /eclusa:remove-workspace
 * init functions and integration with eclusa-tools routing.
 */
...
const { detectChildRepos } = require('../eclusa/bin/lib/init.cjs');
...
describe('detectChildRepos', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('eclusa-ws-test-');
  });
...
// ─── cmdInitNewWorkspace via eclusa-tools ──────────────────────────────────────
...
describe('init new-workspace', () => {
  ...
  test('returns expected JSON fields', () => {
    const result = runEclusaTools('init new-workspace', tmpDir);
...
// ─── cmdInitListWorkspaces via eclusa-tools ────────────────────────────────────
...
describe('init list-workspaces', () => {
  ...
  test('returns empty list when no workspaces exist', () => {
    const result = runEclusaTools('init list-workspaces', tmpDir, { HOME: tmpDir });
    assert.ok(result.success, `init failed: ${result.error}`);
    const data = JSON.parse(result.output);
  ...
  test('finds workspaces with WORKSPACE.md', () => {
    const wsBase = path.join(tmpDir, 'eclusa-workspaces');
    const ws1 = path.join(wsBase, 'feature-a');
    fs.mkdirSync(path.join(ws1, '.eclusa'), { recursive: true });
...
// ─── cmdInitRemoveWorkspace via eclusa-tools ───────────────────────────────────
...
describe('init remove-workspace', () => {
  ...
  test('errors when no name provided', () => {
    const result = runEclusaTools('init remove-workspace', tmpDir);
    assert.strictEqual(result.success, false);
    assert.ok(result.error.includes('workspace name required'));
  });

  test('errors when workspace not found', () => {
    const result = runEclusaTools('init remove-workspace nonexistent', tmpDir, { HOME: tmpDir });
    assert.strictEqual(result.success, false);
  ...
  test('returns workspace info for existing workspace', () => {
    ...
    const result = runEclusaTools('init remove-workspace test-ws', tmpDir, { HOME: tmpDir });
    assert.ok(result.success, `init failed: ${result.error}`);
    const data = JSON.parse(result.output);
...
describe('workspace worktree integration', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('eclusa-ws-integ-');
  ...
  test('creates workspace with git worktree', () => {
    ...
    fs.mkdirSync(wsPath);
    fs.mkdirSync(path.join(wsPath, '.eclusa'));
...
describe('workspace command files', () => {
  ...
  test('new-workspace command exists with correct frontmatter', () => {
    const content = fs.readFileSync(path.join(baseDir, 'commands/eclusa/new-workspace.md'), 'utf8');
    assert.ok(content.includes('name: eclusa:new-workspace'));
  ...
  test('list-workspaces command exists with correct frontmatter', () => {
    const content = fs.readFileSync(path.join(baseDir, 'commands/eclusa/list-workspaces.md'), 'utf8');
    assert.ok(content.includes('name: eclusa:list-workspaces'));
    assert.ok(content.includes('workflows/list-workspaces.md'));
  });

  test('remove-workspace command exists with correct frontmatter', () => {
    const content = fs.readFileSync(path.join(baseDir, 'commands/eclusa/remove-workspace.md'), 'utf8');
    assert.ok(content.includes('name: eclusa:remove-workspace'));
    assert.ok(content.includes('workflows/remove-workspace.md'));
  });

  test('new-workspace workflow exists', () => {
    const content = fs.readFileSync(path.join(baseDir, 'eclusa/workflows/new-workspace.md'), 'utf8');
  ...
  test('list-workspaces workflow exists', () => {
    const content = fs.readFileSync(path.join(baseDir, 'eclusa/workflows/list-workspaces.md'), 'utf8');
    assert.ok(content.includes('init list-workspaces'));
  });

  test('remove-workspace workflow exists', () => {
    const content = fs.readFileSync(path.join(baseDir, 'eclusa/workflows/remove-workspace.md'), 'utf8');
    assert.ok(content.includes('init remove-workspace'));
    assert.ok(content.includes('git worktree remove'));
  });
});

// ─── Routing in eclusa-tools ───────────────────────────────────────────────────

describe('workspace routing in eclusa-tools', () => {
  test('init new-workspace is routed correctly', () => {
    const toolsContent = fs.readFileSync(
      path.join(__dirname, '..', 'eclusa', 'bin', 'eclusa-tools.cjs'),
      'utf8'
    );
