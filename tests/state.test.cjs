...
describe('state-snapshot command', () => {
  ...
  test('missing STATE.md returns error', () => {
    const result = runEclusaTools('state-snapshot', tmpDir);
...
**Resume File:** .eclusa/phases/03-api/03-02-PLAN.md
`
    );
    ...
    assert.strictEqual(output.session.resume_file, '.eclusa/phases/03-api/03-02-PLAN.md', 'resume file extracted');
  });
...
`
  ...
  describe('--cwd override', () => {
    ...
    beforeEach(() => {
      outsideDir = fs.mkdtempSync(path.join(require('os').tmpdir(), 'eclusa-test-outside-'));
    });
...
`
      ...
      const result = runEclusaTools(`state-snapshot --cwd "${tmpDir}"`, outsideDir);
      assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('returns error for invalid --cwd path', () => {
    ...
    const result = runEclusaTools(`state-snapshot --cwd "${invalid}"`, tmpDir);
...
`
    ...
    const result = runEclusaTools(['state', 'add-blocker', '--text', 'Waiting on vendor quote $1.00 before approval'], tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
`
    ...
    const result = runEclusaTools(`state add-blocker --text-file "${blockerPath}"`, tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('state json command', () => {
  ...
  test('missing STATE.md returns error', () => {
    const result = runEclusaTools('state json', tmpDir);
    assert.ok(result.success, `Command should succeed: ${result.error}`);
...
`
    ...
    const output = JSON.parse(result.output);
    assert.strictEqual(output.eclusa_state_version, '1.0', 'should have version 1.0');
...
`
    ...
    const output = JSON.parse(result.output);
    assert.strictEqual(output.eclusa_state_version, '1.0', 'version from frontmatter');
...
`
    ...
    const result = runEclusaTools('state update Status "Executing Plan 1"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);

    const content = fs.readFileSync(path.join(tmpDir, '.eclusa', 'STATE.md'), 'utf-8');
    assert.ok(content.startsWith('---\n'), 'should start with frontmatter delimiter');
    assert.ok(content.includes('eclusa_state_version: 1.0'), 'should have version field');
...
`
    ...
    const result = runEclusaTools('state patch --Status "In progress" --"Current Plan" 04-02', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
`
    ...
    runEclusaTools('state update Status "In progress"', tmpDir);
    runEclusaTools('state update Status "Paused"', tmpDir);
...
`
    ...
    runEclusaTools('state update "Current Plan" "03-03"', tmpDir);
...
`
    ...
    runEclusaTools('state update Status "Executing Plan 5"', tmpDir);
...
const { stateExtractField, stateReplaceField, stateReplaceFieldWithFallback } = require('../eclusa/bin/lib/state.cjs');
...
describe('cmdStateLoad (state load)', () => {
  ...
  test('returns config and state when STATE.md exists', () => {
    ...
    const result = runEclusaTools('state load', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('returns raw key=value format with --raw flag', () => {
    ...
    const result = runEclusaTools('state load --raw', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('cmdStateGet (state get)', () => {
  ...
  test('returns full content when no section specified', () => {
    ...
    fs.writeFileSync(path.join(tmpDir, '.eclusa', 'STATE.md'), stateContent);

    const result = runEclusaTools('state get', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('extracts bold field value', () => {
    ...
    const result = runEclusaTools('state get Status', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('extracts markdown section content', () => {
    ...
    const result = runEclusaTools('state get Blockers', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('returns error for nonexistent field', () => {
    ...
    const result = runEclusaTools('state get Missing', tmpDir);
...
describe('cmdStatePatch and cmdStateUpdate (state patch, state update)', () => {
  ...
  test('state patch updates multiple fields at once', () => {
    fs.writeFileSync(path.join(tmpDir, '.eclusa', 'STATE.md'), stateMd);

    const result = runEclusaTools('state patch --Status Complete --"Current Phase" 04', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);

    const updated = fs.readFileSync(path.join(tmpDir, '.eclusa', 'STATE.md'), 'utf-8');
  ...
  test('state patch reports failed fields that do not exist', () => {
    ...
    const result = runEclusaTools('state patch --Status Done --Missing value', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('state update changes a single field', () => {
    ...
    const result = runEclusaTools('state update Status "Phase complete"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('state update reports field not found', () => {
    ...
    const result = runEclusaTools('state update Missing value', tmpDir);
  ...
  test('state update returns error when STATE.md missing', () => {
    const result = runEclusaTools('state update Status value', tmpDir);
...
describe('cmdStateAdvancePlan (state advance-plan)', () => {
  ...
  test('advances plan counter when not on last plan', () => {
    fs.writeFileSync(path.join(tmpDir, '.eclusa', 'STATE.md'), advanceFixture);

    const before = new Date().toISOString().split('T')[0];
    const result = runEclusaTools('state advance-plan', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('marks phase complete on last plan', () => {
    ...
    fs.writeFileSync(path.join(tmpDir, '.eclusa', 'STATE.md'), lastPlanFixture);
...
describe('cmdStateRecordMetric (state record-metric)', () => {
  ...
  test('appends metric row to existing table', () => {
    fs.writeFileSync(path.join(tmpDir, '.eclusa', 'STATE.md'), metricsFixture);

    const result = runEclusaTools('state record-metric --phase 2 --plan 1 --duration 5min --tasks 3 --files 4', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('replaces None yet placeholder with first metric', () => {
    ...
    fs.writeFileSync(path.join(tmpDir, '.eclusa', 'STATE.md'), noneYetFixture);

    const result = runEclusaTools('state record-metric --phase 1 --plan 1 --duration 2min --tasks 1 --files 2', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('returns error when required fields missing', () => {
    ...
    const result = runEclusaTools('state record-metric --phase 1', tmpDir);
    assert.ok(result.success, `Command should exit 0: ${result.error}`);
  ...
  test('returns error when STATE.md missing', () => {
    const result = runEclusaTools('state record-metric --phase 1 --plan 1 --duration 2min', tmpDir);
    assert.ok(result.success, `Command should exit 0: ${result.error}`);
...
describe('cmdStateUpdateProgress (state update-progress)', () => {
  ...
  test('calculates progress from plan/summary counts', () => {
    ...
    const phase01Dir = path.join(tmpDir, '.eclusa', 'phases', '01');
    ...
    const phase02Dir = path.join(tmpDir, '.eclusa', 'phases', '02');
    fs.mkdirSync(phase02Dir, { recursive: true });
    fs.writeFileSync(path.join(phase02Dir, '02-01-PLAN.md'), '# Plan\n');

    const result = runEclusaTools('state update-progress', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('cmdStateResolveBlocker (state resolve-blocker)', () => {
  ...
  test('removes matching blocker line (case-insensitive substring match)', () => {
    fs.writeFileSync(path.join(tmpDir, '.eclusa', 'STATE.md'), blockerFixture);

    const result = runEclusaTools('state resolve-blocker --text "api credentials"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('adds None placeholder when last blocker resolved', () => {
    ...
    fs.writeFileSync(path.join(tmpDir, '.eclusa', 'STATE.md'), singleBlockerFixture);

    const result = runEclusaTools('state resolve-blocker --text "single blocker"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('returns error when text not provided', () => {
    ...
    const result = runEclusaTools('state resolve-blocker', tmpDir);
    assert.ok(result.success, `Command should exit 0: ${result.error}`);
  ...
  test('returns error when STATE.md missing', () => {
    const result = runEclusaTools('state resolve-blocker --text "anything"', tmpDir);
    assert.ok(result.success, `Command should exit 0: ${result.error}`);
  ...
  test('returns resolved true even if no line matches', () => {
    ...
    const result = runEclusaTools('state resolve-blocker --text "nonexistent blocker text"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('cmdStateRecordSession (state record-session)', () => {
  ...
  test('updates session fields with stopped-at and resume-file', () => {
    fs.writeFileSync(path.join(tmpDir, '.eclusa', 'STATE.md'), sessionFixture);

    const result = runEclusaTools(
      'state record-session --stopped-at "Phase 3, Plan 2" --resume-file ".eclusa/phases/03/03-02-PLAN.md"',
      tmpDir
    );
    assert.ok(result.success, `Command failed: ${result.error}`);
    ...
    assert.ok(updated.includes('.eclusa/phases/03/03-02-PLAN.md'), 'Resume file should be updated');
  ...
  test('updates Last session timestamp even with no other options', () => {
    ...
    const result = runEclusaTools('state record-session', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('sets Resume file to None when not specified', () => {
    ...
    const result = runEclusaTools('state record-session --stopped-at "Phase 1 complete"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('milestone-scoped phase counting in frontmatter', () => {
  ...
  test('total_phases counts only current milestone phases', () => {
    ...
    const result = runEclusaTools('state update Status "Executing"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);

    // Read the state json to check frontmatter
    const jsonResult = runEclusaTools('state json', tmpDir);
  ...
  test('without ROADMAP counts all phases (pass-all filter)', () => {
    ...
    const result = runEclusaTools('state update Status "In progress"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
`;
    ...
    const content = fs.readFileSync(
      path.join(tmpDir, '.eclusa', 'STATE.md'), 'utf-8'
    );
...
`;
    ...
    // Step 1: begin-phase
    const beginResult = runEclusaTools(
    ...
    const adv1 = runEclusaTools(['state', 'advance-plan'], tmpDir);
    ...
    const adv2 = runEclusaTools(['state', 'advance-plan'], tmpDir);
