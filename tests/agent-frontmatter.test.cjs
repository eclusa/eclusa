/**
 * Eclusa Agent Frontmatter Tests
 *
...
const WORKFLOWS_DIR = path.join(__dirname, '..', 'eclusa', 'workflows');
const COMMANDS_DIR = path.join(__dirname, '..', 'commands', 'eclusa');
...
describe('SPAWN: spawn type consistency', () => {
  test('no "First, read agent .md" workaround pattern remains', () => {
    ...
    for (const dir of dirs) {
      ...
      for (const file of files) {
        ...
        const hasWorkaround = content.includes('First, read ~/.claude/agents/eclusa-');
        assert.ok(
          !hasWorkaround,
  ...
  test('diagnose-issues uses eclusa-debugger (not general-purpose)', () => {
    const content = fs.readFileSync(
      path.join(WORKFLOWS_DIR, 'diagnose-issues.md'), 'utf-8'
    );
    assert.ok(
      content.includes('subagent_type="eclusa-debugger"'),
      'diagnose-issues should spawn eclusa-debugger, not general-purpose'
    );
  });
  ...
  test('workflows spawning named agents have <available_agent_types> listing (#1357)', () => {
    ...
    // workflows that spawn named Eclusa agents.
    const dirs = [WORKFLOWS_DIR, COMMANDS_DIR];
    for (const dir of dirs) {
      if (!fs.existsSync(dir)) continue;
      const files = fs.readdirSync(dir).filter(f => f.endsWith('.md'));
...
describe('CLAUDEMD: CLAUDE.md compliance enforcement', () => {
  test('eclusa-plan-checker has Dimension 10: CLAUDE.md Compliance', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'eclusa-plan-checker.md'), 'utf-8');
    assert.ok(
      content.includes('Dimension 10: CLAUDE.md Compliance'),
      'eclusa-plan-checker must have Dimension 10 for CLAUDE.md compliance checking'
    );
    assert.ok(
      content.includes('claude_md_compliance'),
      'eclusa-plan-checker must use claude_md_compliance as dimension identifier'
    );
  });

  test('eclusa-phase-researcher has CLAUDE.md enforcement directive', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'eclusa-phase-researcher.md'), 'utf-8');
    assert.ok(
      content.includes('CLAUDE.md enforcement'),
      'eclusa-phase-researcher must enforce CLAUDE.md directives during research'
    );
    assert.ok(
      content.includes('Project Constraints (from CLAUDE.md)'),
      'eclusa-phase-researcher must output a Project Constraints section from CLAUDE.md'
    );
  });

  test('eclusa-executor has CLAUDE.md enforcement directive', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'eclusa-executor.md'), 'utf-8');
    assert.ok(
      content.includes('CLAUDE.md enforcement'),
      'eclusa-executor must enforce CLAUDE.md directives during execution'
    );
    assert.ok(
      content.includes('CLAUDE.md rule — it takes precedence over plan instructions'),
      'eclusa-executor must specify CLAUDE.md precedence over plan instructions'
    );
  });

  test('all three agents read CLAUDE.md in project_context', () => {
    const agents = ['eclusa-plan-checker', 'eclusa-phase-researcher', 'eclusa-executor'];
...
describe('VERIFY: data-flow trace, environment audit, and behavioral spot-checks', () => {
  test('eclusa-verifier has Step 4b: Data-Flow Trace', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'eclusa-verifier.md'), 'utf-8');
    assert.ok(
      content.includes('Step 4b: Data-Flow Trace'),
      'eclusa-verifier must have Step 4b for data-flow tracing'
    );
    assert.ok(
      content.includes('HOLLOW'),
      'eclusa-verifier must define HOLLOW status for wired-but-disconnected artifacts'
    );
    assert.ok(
      content.includes('DISCONNECTED'),
      'eclusa-verifier must define DISCONNECTED status for missing data sources'
    );
  });

  test('eclusa-verifier has Step 7b: Behavioral Spot-Checks', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'eclusa-verifier.md'), 'utf-8');
    assert.ok(
      content.includes('Step 7b: Behavioral Spot-Checks'),
      'eclusa-verifier must have Step 7b for behavioral spot-checks'
    );
    assert.ok(
      content.includes('SKIP'),
      'eclusa-verifier spot-checks must support SKIP status for untestable items'
    );
  });

  test('eclusa-verifier VERIFICATION.md template includes data-flow and spot-check sections', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'eclusa-verifier.md'), 'utf-8');
    assert.ok(
  ...
  test('eclusa-verifier success criteria include data-flow and spot-checks', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'eclusa-verifier.md'), 'utf-8');
    assert.ok(
      content.includes('Data-flow trace (Level 4)'),
  ...
  test('eclusa-phase-researcher has Step 2.6: Environment Availability Audit', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'eclusa-phase-researcher.md'), 'utf-8');
    assert.ok(
      content.includes('Step 2.6: Environment Availability Audit'),
      'eclusa-phase-researcher must have Step 2.6 for environment availability auditing'
    );
    assert.ok(
      content.includes('Environment Availability'),
      'eclusa-phase-researcher must include Environment Availability section in RESEARCH.md template'
    );
  });

  test('eclusa-phase-researcher success criteria include environment audit', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'eclusa-phase-researcher.md'), 'utf-8');
    assert.ok(
...
describe('DISCUSS: discussion log generation', () => {
  ...
  test('discussion-log template exists', () => {
    const templatePath = path.join(__dirname, '..', 'eclusa', 'templates', 'discussion-log.md');
    assert.ok(
...
describe('PERM: worktree agents have permissionMode: acceptEdits', () => {
  ...
  const WORKTREE_AGENTS = ['eclusa-executor', 'eclusa-debugger'];
  ...
  test('worktree-spawned agents are covered', () => {
    ...
    for (const dir of dirs) {
      ...
      for (const file of files) {
        const content = fs.readFileSync(path.join(dir, file), 'utf-8');
        // Find patterns like: subagent_type="eclusa-executor" ... isolation="worktree"
