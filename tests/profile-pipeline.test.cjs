...
const os = require('os');
const { runEclusaTools, createTempDir, createTempProject, cleanup } = require('./helpers.cjs');
...
describe('scan-sessions command', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('eclusa-profile-test-');
  });
  ...
  test('returns empty array for empty sessions directory', () => {
    ...
    const result = runEclusaTools(`scan-sessions --path ${sessionsDir} --raw`, tmpDir);
...
describe('extract-messages command', () => {
  ...
  test('extracts user messages from synthetic session', () => {
    ...
    const result = runEclusaTools(`extract-messages my-project --path ${sessionsDir} --raw`, tmpDir);
    assert.ok(result.success, `Failed: ${result.error}`);
    const out = JSON.parse(result.output);
  ...
  test('filters out meta and internal messages', () => {
    ...
    const result = runEclusaTools(`extract-messages filter-test --path ${sessionsDir} --raw`, tmpDir);
    assert.ok(result.success, `Failed: ${result.error}`);
    const out = JSON.parse(result.output);
...
describe('profile-questionnaire command', () => {
  ...
  test('returns questionnaire structure', () => {
    const result = runEclusaTools('profile-questionnaire --raw', tmpDir);
    assert.ok(result.success, `Failed: ${result.error}`);
    const out = JSON.parse(result.output);
