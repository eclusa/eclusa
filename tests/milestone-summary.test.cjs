/**
 * Eclusa Milestone Summary Tests
 *
...
const commandPath = path.join(repoRoot, 'commands', 'eclusa', 'milestone-summary.md');
const workflowPath = path.join(repoRoot, 'eclusa', 'workflows', 'milestone-summary.md');

describe('milestone-summary command', () => {
  test('command file exists', () => {
    assert.ok(fs.existsSync(commandPath), 'commands/eclusa/milestone-summary.md should exist');
  });

  test('command has correct frontmatter name', () => {
    const content = fs.readFileSync(commandPath, 'utf-8');
    assert.ok(content.includes('name: eclusa:milestone-summary'), 'should have correct command name');
  });
...
describe('milestone-summary workflow', () => {
  ...
  test('workflow writes to reports directory', () => {
    const content = fs.readFileSync(workflowPath, 'utf-8');
    assert.ok(
      content.includes('.eclusa/reports/MILESTONE_SUMMARY'),
      'should write summary to .eclusa/reports/'
    );
  });
  ...
  test('workflow updates STATE.md', () => {
    ...
    assert.ok(
      ...
      'should update STATE.md via eclusa-tools'
    );
  });
  ...
  test('workflow checks both audit file locations for archived milestones', () => {
    const content = fs.readFileSync(workflowPath, 'utf-8');
    assert.ok(
      content.includes('.eclusa/milestones/v${VERSION}-MILESTONE-AUDIT.md'),
...
describe('milestone-summary artifact path resolution', () => {
  ...
  test('archived milestone paths point to milestones/ directory', () => {
    ...
    assert.ok(
      content.includes('.eclusa/milestones/v${VERSION}-ROADMAP.md'),
      'archived ROADMAP path should be under .eclusa/milestones/'
    );
    assert.ok(
      content.includes('.eclusa/milestones/v${VERSION}-REQUIREMENTS.md'),
      'archived REQUIREMENTS path should be under .eclusa/milestones/'
    );
    assert.ok(
      content.includes('.eclusa/milestones/v${VERSION}-MILESTONE-AUDIT.md'),
      'archived AUDIT path should be under .eclusa/milestones/'
    );
  });

  test('current milestone paths point to .eclusa/ root', () => {
    const content = fs.readFileSync(workflowPath, 'utf-8');
    // Current milestone should read from .eclusa/ root
    ...
    assert.ok(
      currentSection.includes('ROADMAP_PATH=".eclusa/ROADMAP.md"'),
      'current ROADMAP path should be at .eclusa/ root'
    );
  });
});
...
describe('milestone-summary fixture-based artifact discovery', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'eclusa-ms-test-'));
  });
  ...
  test('discovers artifacts in archived milestone structure', () => {
    ...
    const milestonesDir = path.join(tmpDir, '.eclusa', 'milestones');
  ...
  test('discovers phase artifacts across multiple phases', () => {
    ...
    const phase2 = path.join(tmpDir, '.eclusa', 'phases', '02-core');
    ...
    // Verify discovery
    const phasesDir = path.join(tmpDir, '.eclusa', 'phases');
  ...
  test('handles empty .eclusa directory without error', () => {
    const planningDir = path.join(tmpDir, '.eclusa');
    fs.mkdirSync(planningDir, { recursive: true });

    // No milestones, no phases — just empty .eclusa/
    const contents = fs.readdirSync(planningDir);
    assert.strictEqual(contents.length, 0, 'empty .eclusa/ should have no contents');
