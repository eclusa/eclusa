/**
 * Eclusa Tools Tests - Antigravity Install Plumbing
 *
...
describe('convertClaudeToAntigravityContent', () => {
  ...
  describe('local install path replacements', () => {
    test('replaces ~/.claude/ with .agent/ for local installs', () => {
      const input = 'See ~/.claude/eclusa/';
      const result = convertClaudeToAntigravityContent(input, false);
      assert.ok(result.includes('.agent/eclusa/'), result);
      assert.ok(!result.includes('~/.claude/'), result);
    });

    test('replaces ./.claude/ with ./.agent/', () => {
      const input = 'path ./.claude/hooks/eclusa-check-update.js';
      const result = convertClaudeToAntigravityContent(input, false);
    ...
    test('replaces .claude/ with .agent/', () => {
      const input = 'node .claude/hooks/eclusa-statusline.js';
      const result = convertClaudeToAntigravityContent(input, false);
      assert.ok(result.includes('.agent/hooks/eclusa-statusline.js'), result);
      assert.ok(!result.includes('.claude/'), result);
    });
  });

  describe('command name conversion', () => {
    test('converts /eclusa:command to /eclusa-command', () => {
      const input = 'Run /eclusa:new-project to start';
      const result = convertClaudeToAntigravityContent(input, true);
      assert.ok(result.includes('/eclusa-new-project'), result);
      assert.ok(!result.includes('eclusa:'), result);
    });

    test('converts all eclusa: references', () => {
      const input = '/eclusa:plan-phase and /eclusa:execute-phase';
      const result = convertClaudeToAntigravityContent(input, false);
      assert.ok(result.includes('/eclusa-plan-phase'), result);
      assert.ok(result.includes('/eclusa-execute-phase'), result);
    });
  });
...
description: Initialize a new Eclusa project with requirements and roadmap
...
Initialize new project at ~/.claude/eclusa/workflows/new-project.md
`;

  test('produces name and description only in frontmatter', () => {
    const result = convertClaudeCommandToAntigravitySkill(claudeCommand, 'eclusa-new-project', false);
    assert.ok(result.startsWith('---\n'), result);
    assert.ok(result.includes('name: eclusa-new-project'), result);
    assert.ok(result.includes('description: Initialize a new Eclusa project'), result);
  ...
  test('uses provided skillName for name field', () => {
    const result = convertClaudeCommandToAntigravitySkill(claudeCommand, 'eclusa-custom-name', false);
    assert.ok(result.includes('name: eclusa-custom-name'), result);
  });

  test('converts eclusa: command references in body', () => {
    const content = `---
name: test
description: test skill
---
Run /eclusa:new-project to get started.
`;
    const result = convertClaudeCommandToAntigravitySkill(content, 'eclusa-test', false);
    assert.ok(result.includes('/eclusa-new-project'), result);
    assert.ok(!result.includes('eclusa:'), result);
  });
  ...
  test('returns unchanged content when no frontmatter', () => {
    ...
    const result = convertClaudeCommandToAntigravitySkill(noFm, 'eclusa-test', false);
...
Execute plans from ~/.claude/eclusa/workflows/execute-phase.md
`;

  test('preserves name and description', () => {
    const result = convertClaudeAgentToAntigravityAgent(claudeAgent, false);
    assert.ok(result.includes('name: eclusa-executor'), result);
    assert.ok(result.includes('description: Executes Eclusa plans'), result);
  });
  ...
  test('uses global path for global installs', () => {
    ...
    assert.ok(result.includes('~/.gemini/antigravity/eclusa/'), result);
  });
...
describe('copyCommandsAsAntigravitySkills', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('eclusa-ag-test-');
    srcDir = path.join(tmpDir, 'commands', 'eclusa');
...
`);
  ...
  test('creates skills directory', () => {
    copyCommandsAsAntigravitySkills(srcDir, skillsDir, 'eclusa', false);
    assert.ok(fs.existsSync(skillsDir));
  });

  test('creates one skill directory per command with SKILL.md', () => {
    copyCommandsAsAntigravitySkills(srcDir, skillsDir, 'eclusa', false);
    const skillDir = path.join(skillsDir, 'eclusa-new-project');
  ...
  test('handles subdirectory commands with prefixed names', () => {
    copyCommandsAsAntigravitySkills(srcDir, skillsDir, 'eclusa', false);
    const subSkillDir = path.join(skillsDir, 'eclusa-subdir-sub-command');
  ...
  test('SKILL.md has minimal frontmatter (name + description only)', () => {
    copyCommandsAsAntigravitySkills(srcDir, skillsDir, 'eclusa', false);
    const content = fs.readFileSync(path.join(skillsDir, 'eclusa-new-project', 'SKILL.md'), 'utf8');
    assert.ok(content.includes('name: eclusa-new-project'), content);
  ...
  test('SKILL.md body has paths converted for local install', () => {
    copyCommandsAsAntigravitySkills(srcDir, skillsDir, 'eclusa', false);
    const content = fs.readFileSync(path.join(skillsDir, 'eclusa-new-project', 'SKILL.md'), 'utf8');
    // eclusa: → eclusa- conversion
    assert.ok(!content.includes('eclusa:'), content);
  });

  test('removes old eclusa-* skill dirs before reinstalling', () => {
    // Create a stale skill dir
    const staleDir = path.join(skillsDir, 'eclusa-old-skill');
  ...
  test('does not remove non-eclusa skill dirs', () => {
    // Create a non-Eclusa skill dir
    const otherDir = path.join(skillsDir, 'my-custom-skill');
    fs.mkdirSync(otherDir, { recursive: true });

    copyCommandsAsAntigravitySkills(srcDir, skillsDir, 'eclusa', false);

    assert.ok(fs.existsSync(otherDir), 'non-Eclusa skill dir should be preserved');
  });
});
...
describe('writeManifest (Antigravity)', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('eclusa-manifest-ag-');
    // Create minimal structure
    const skillsDir = path.join(tmpDir, 'skills', 'eclusa-help');
    ...
    fs.writeFileSync(path.join(eclusaDir, 'VERSION'), '1.0.0');
    const agentsDir = path.join(tmpDir, 'agents');
    fs.mkdirSync(agentsDir, { recursive: true });
    fs.writeFileSync(path.join(agentsDir, 'eclusa-executor.md'), '---\nname: eclusa-executor\n---\n');
  });
  ...
  test('manifest includes skills in skills/ directory', () => {
    writeManifest(tmpDir, 'antigravity');
    const manifest = JSON.parse(fs.readFileSync(path.join(tmpDir, 'eclusa-file-manifest.json'), 'utf8'));
