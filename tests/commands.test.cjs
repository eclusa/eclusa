...
describe('history-digest command', () => {
  ...
  test('empty phases directory returns valid schema', () => {
    const result = runEclusaTools('history-digest', tmpDir);
  ...
  test('nested frontmatter fields extracted correctly', () => {
    ...
    const phaseDir = path.join(tmpDir, '.eclusa', 'phases', '01-foundation');
...
`;
  ...
  test('multiple phases merged into single digest', () => {
    // Create phase 01
    const phase01Dir = path.join(tmpDir, '.eclusa', 'phases', '01-foundation');
...
`
    ...
    // Create phase 02
    const phase02Dir = path.join(tmpDir, '.eclusa', 'phases', '02-api');
...
describe('summary-extract command', () => {
  ...
  test('missing file returns error', () => {
    const result = runEclusaTools('summary-extract .eclusa/phases/01-test/01-01-SUMMARY.md', tmpDir);
...
`
    ...
    const result = runEclusaTools('summary-extract .eclusa/phases/01-foundation/01-01-SUMMARY.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);

    const output = JSON.parse(result.output);
    assert.strictEqual(output.path, '.eclusa/phases/01-foundation/01-01-SUMMARY.md', 'path correct');
...
`
    ...
    const result = runEclusaTools('summary-extract .eclusa/phases/01-foundation/01-01-SUMMARY.md --fields one_liner,key_files,requirements_completed', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('progress command', () => {
  ...
  test('renders JSON progress', () => {
    ...
    const result = runEclusaTools('progress json', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('renders bar format', () => {
    ...
    const result = runEclusaTools('progress bar --raw', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('renders table format', () => {
    ...
    const result = runEclusaTools('progress table --raw', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('does not crash when summaries exceed plans (orphaned SUMMARY.md)', () => {
    ...
    const barResult = runEclusaTools('progress bar --raw', tmpDir);
    ...
    const tableResult = runEclusaTools('progress table --raw', tmpDir);
    assert.ok(tableResult.success, `Table format crashed: ${tableResult.error}`);

    // json format - percent should be clamped
    const jsonResult = runEclusaTools('progress json', tmpDir);
...
describe('todo complete command', () => {
  ...
  test('moves todo from pending to completed', () => {
    const pendingDir = path.join(tmpDir, '.eclusa', 'todos', 'pending');
    ...
    const result = runEclusaTools('todo complete add-dark-mode.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
    ...
    // Verify moved
    assert.ok(
      !fs.existsSync(path.join(tmpDir, '.eclusa', 'todos', 'pending', 'add-dark-mode.md')),
      'should be removed from pending'
    );
    assert.ok(
      fs.existsSync(path.join(tmpDir, '.eclusa', 'todos', 'completed', 'add-dark-mode.md')),
      'should be in completed'
    );

    // Verify completion timestamp added
    const content = fs.readFileSync(
      path.join(tmpDir, '.eclusa', 'todos', 'completed', 'add-dark-mode.md'),
      'utf-8'
    );
  ...
  test('fails for nonexistent todo', () => {
    const result = runEclusaTools('todo complete nonexistent.md', tmpDir);
...
describe('todo match-phase command', () => {
  ...
  test('returns empty matches when no todos exist', () => {
    const result = runEclusaTools('todo match-phase 01', tmpDir);
  ...
  test('matches todo by keyword overlap with phase name', () => {
    ...
    fs.writeFileSync(path.join(tmpDir, '.eclusa', 'ROADMAP.md'),
...
describe('scaffold command', () => {
  ...
  test('scaffolds context file', () => {
    ...
    const result = runEclusaTools('scaffold context --phase 3', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
    ...
    // Verify file content
    const content = fs.readFileSync(
      path.join(tmpDir, '.eclusa', 'phases', '03-api', '03-CONTEXT.md'),
      'utf-8'
    );
  ...
  test('scaffolds UAT file', () => {
    ...
    const result = runEclusaTools('scaffold uat --phase 3', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
    ...
    const content = fs.readFileSync(
      path.join(tmpDir, '.eclusa', 'phases', '03-api', '03-UAT.md'),
      'utf-8'
    );
  ...
  test('scaffolds verification file', () => {
    ...
    const result = runEclusaTools('scaffold verification --phase 3', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
    ...
    const content = fs.readFileSync(
      path.join(tmpDir, '.eclusa', 'phases', '03-api', '03-VERIFICATION.md'),
      'utf-8'
    );
  ...
  test('scaffolds phase directory', () => {
    const result = runEclusaTools('scaffold phase-dir --phase 5 --name User Dashboard', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);

    const output = JSON.parse(result.output);
    assert.strictEqual(output.created, true);
    assert.ok(
      fs.existsSync(path.join(tmpDir, '.eclusa', 'phases', '05-user-dashboard')),
...
describe('generate-slug command', () => {
  ...
  test('converts normal text to slug', () => {
    const result = runEclusaTools('generate-slug "Hello World"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('strips special characters', () => {
    const result = runEclusaTools('generate-slug "Test@#$%^Special!!!"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('preserves numbers', () => {
    const result = runEclusaTools('generate-slug "Phase 3 Plan"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('strips leading and trailing hyphens', () => {
    const result = runEclusaTools('generate-slug "---leading-trailing---"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('fails when no text provided', () => {
    const result = runEclusaTools('generate-slug', tmpDir);
...
describe('current-timestamp command', () => {
  ...
  test('date format returns YYYY-MM-DD', () => {
    const result = runEclusaTools('current-timestamp date', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('filename format returns ISO without colons or fractional seconds', () => {
    const result = runEclusaTools('current-timestamp filename', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('full format returns full ISO string', () => {
    const result = runEclusaTools('current-timestamp full', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('default (no format) returns full ISO string', () => {
    const result = runEclusaTools('current-timestamp', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('list-todos command', () => {
  ...
  test('empty directory returns zero count', () => {
    const result = runEclusaTools('list-todos', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('area filter returns only matching todos', () => {
    ...
    const result = runEclusaTools('list-todos ui', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('area filter miss returns zero count', () => {
    ...
    const result = runEclusaTools('list-todos nonexistent-area', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('verify-path-exists command', () => {
  ...
  test('existing file returns exists=true with type=file', () => {
    ...
    const result = runEclusaTools('verify-path-exists test-file.txt', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('existing directory returns exists=true with type=directory', () => {
    ...
    const result = runEclusaTools('verify-path-exists test-dir', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('missing path returns exists=false', () => {
    const result = runEclusaTools('verify-path-exists nonexistent/path', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('absolute path resolves correctly', () => {
    ...
    const result = runEclusaTools(`verify-path-exists ${absFile}`, tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('fails when no path provided', () => {
    const result = runEclusaTools('verify-path-exists', tmpDir);
...
describe('resolve-model command', () => {
  ...
  test('known agent returns model and profile without unknown_agent', () => {
    const result = runEclusaTools('resolve-model eclusa-planner', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('unknown agent returns unknown_agent=true', () => {
    const result = runEclusaTools('resolve-model fake-nonexistent-agent', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('default profile fallback when no config exists', () => {
    ...
    const result = runEclusaTools('resolve-model eclusa-executor', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('fails when no agent-type provided', () => {
    const result = runEclusaTools('resolve-model', tmpDir);
...
describe('commit command', () => {
  ...
  test('skips when commit_docs is false', () => {
    ...
    const result = runEclusaTools('commit "test message"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('skips when .eclusa is gitignored', () => {
    // Add .eclusa/ to .gitignore and commit it so git recognizes the ignore
  ...
  test('creates real commit with correct hash', () => {
    // Create a new file in .eclusa/
    fs.writeFileSync(path.join(tmpDir, '.eclusa', 'test-file.md'), '# Test\n');

    const result = runEclusaTools('commit "test: add test file" --files .eclusa/test-file.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('amend mode works without crashing', () => {
    ...
    fs.writeFileSync(path.join(tmpDir, '.eclusa', 'amend-file.md'), '# Initial\n');
    execSync('git add .eclusa/amend-file.md', { cwd: tmpDir, stdio: 'pipe' });
    execSync('git commit -m "initial file"', { cwd: tmpDir, stdio: 'pipe' });

    // Modify the file and amend
    fs.writeFileSync(path.join(tmpDir, '.eclusa', 'amend-file.md'), '# Amended\n');

    const result = runEclusaTools('commit "ignored" --files .eclusa/amend-file.md --amend', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('creates strategy branch before first commit when branching_strategy is milestone', () => {
    ...
    fs.writeFileSync(path.join(tmpDir, '.eclusa', 'test-context.md'), '# Context\n');

    const result = runEclusaTools('commit "docs: add context" --files .eclusa/test-context.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
    ...
    assert.strictEqual(branch, 'eclusa/v1.0-initial-release', 'should be on milestone branch');
  });
  ...
  test('creates strategy branch before first commit when branching_strategy is phase', () => {
    ...
    fs.mkdirSync(path.join(tmpDir, '.eclusa', 'phases', '01-setup'), { recursive: true });
    fs.writeFileSync(
      path.join(tmpDir, '.eclusa', 'ROADMAP.md'),
      '# Roadmap\n\n## Phase 1: Setup\nGoal: Initial setup\n'
    );
    ...
    fs.writeFileSync(path.join(tmpDir, '.eclusa', 'phases', '01-setup', '01-CONTEXT.md'), '# Context\n');

    const result = runEclusaTools(
      'commit "docs(01): add context" --files .eclusa/phases/01-setup/01-CONTEXT.md',
      tmpDir
    );
    assert.ok(result.success, `Command failed: ${result.error}`);
    ...
    const { execFileSync } = require('child_process');
    const branch = execFileSync('git', ['rev-parse', '--abbrev-ref', 'HEAD'], { cwd: tmpDir, encoding: 'utf-8' }).trim();
    assert.strictEqual(branch, 'eclusa/phase-01-setup', 'should be on phase branch');
  });
});
...
describe('websearch command', () => {
  const { cmdWebsearch } = require('../eclusa/bin/lib/commands.cjs');
  let origFetch;
  let origApiKey;
  let origWriteSync;
  let captured;
...
describe('stats command', () => {
  ...
  test('returns valid JSON with empty project', () => {
    const result = runEclusaTools('stats', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('counts phases, plans, and summaries correctly', () => {
    ...
    const p2 = path.join(tmpDir, '.eclusa', 'phases', '02-api');
    fs.mkdirSync(p1, { recursive: true });
...
`
  ...
  test('includes roadmap-only phases in totals and preserves hyphenated names', () => {
    const p1 = path.join(tmpDir, '.eclusa', 'phases', '14-auth-hardening');
    const p2 = path.join(tmpDir, '.eclusa', 'phases', '15-proof-generation');
    fs.mkdirSync(p1, { recursive: true });
    fs.mkdirSync(p2, { recursive: true });
...
`
  ...
  test('reports git commit count and first commit date from repository history', () => {
    ...
    fs.writeFileSync(path.join(tmpDir, '.eclusa', 'PROJECT.md'), '# Project\n');
  ...
  test('table format renders readable output', () => {
    ...
    const result = runEclusaTools('stats table', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
