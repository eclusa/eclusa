...
const commandPath = path.join(repoRoot, 'commands', 'eclusa', 'forensics.md');
const workflowPath = path.join(repoRoot, 'eclusa', 'workflows', 'forensics.md');

describe('forensics command', () => {
  test('command file exists', () => {
    assert.ok(fs.existsSync(commandPath), 'commands/eclusa/forensics.md should exist');
  });

  test('command has correct frontmatter', () => {
    const content = fs.readFileSync(commandPath, 'utf-8');
    assert.ok(content.includes('name: eclusa:forensics'), 'should have correct command name');
...
describe('forensics workflow', () => {
  ...
  test('workflow writes report to forensics directory', () => {
    const content = fs.readFileSync(workflowPath, 'utf-8');
    assert.ok(
      content.includes('.eclusa/forensics/report-'),
      'should write to .eclusa/forensics/'
    );
  });
...
describe('forensics fixture-based tests', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'eclusa-forensics-test-'));
  });
  ...
  test('forensics report directory can be created', () => {
    const forensicsDir = path.join(tmpDir, '.eclusa', 'forensics');
  ...
  test('handles project with no .eclusa directory', () => {
    // No .eclusa/ at all
    const planningExists = fs.existsSync(path.join(tmpDir, '.eclusa'));
    assert.strictEqual(planningExists, false, 'no .eclusa/ should exist');
