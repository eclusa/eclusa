...
const {
  PROFILING_QUESTIONS,
  CLAUDE_INSTRUCTIONS,
} = require('../eclusa/bin/lib/profile-output.cjs');
...
describe('write-profile command', () => {
  ...
  test('writes USER-PROFILE.md from analysis JSON', () => {
    ...
    const result = runEclusaTools(['write-profile', '--input', analysisPath, '--raw'], tmpDir);
  ...
  test('errors when --input is missing', () => {
    const result = runEclusaTools('write-profile --raw', tmpDir);
...
describe('generate-claude-md command', () => {
  ...
  test('generates CLAUDE.md with --auto flag', () => {
    ...
    const result = runEclusaTools(['generate-claude-md', '--output', outputPath, '--auto', '--raw'], tmpDir);
    assert.ok(result.success, `Failed: ${result.error}`);
...
describe('generate-dev-preferences command', () => {
  ...
  test('errors when --analysis is missing', () => {
    const result = runEclusaTools('generate-dev-preferences --raw', tmpDir);
  ...
  test('generates preferences from analysis file', () => {
    ...
    const result = runEclusaTools(['generate-dev-preferences', '--analysis', analysisPath, '--raw'], tmpDir);
    assert.ok(result.success, `Failed: ${result.error}`);
    const out = JSON.parse(result.output);
