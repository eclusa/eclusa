...
describe('init commands', () => {
  ...
  test('init execute-phase returns file paths', () => {
    ...
    const result = runEclusaTools('init execute-phase 03', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);

    const output = JSON.parse(result.output);
    assert.strictEqual(output.state_path, '.eclusa/STATE.md');
    assert.strictEqual(output.roadmap_path, '.eclusa/ROADMAP.md');
    assert.strictEqual(output.config_path, '.eclusa/config.json');
  });
  ...
  test('init plan-phase returns file paths', () => {
    ...
    const result = runEclusaTools('init plan-phase 03', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);

    const output = JSON.parse(result.output);
    assert.strictEqual(output.state_path, '.eclusa/STATE.md');
    assert.strictEqual(output.roadmap_path, '.eclusa/ROADMAP.md');
    assert.strictEqual(output.requirements_path, '.eclusa/REQUIREMENTS.md');
    assert.strictEqual(output.context_path, '.eclusa/phases/03-api/03-CONTEXT.md');
    assert.strictEqual(output.research_path, '.eclusa/phases/03-api/03-RESEARCH.md');
    assert.strictEqual(output.verification_path, '.eclusa/phases/03-api/03-VERIFICATION.md');
    assert.strictEqual(output.uat_path, '.eclusa/phases/03-api/03-UAT.md');
  });
  ...
  test('init plan-phase exposes text_mode true when set in config', () => {
    const configPath = path.join(tmpDir, '.eclusa', 'config.json');
  ...
  test('init progress returns file paths', () => {
    const result = runEclusaTools('init progress', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);

    const output = JSON.parse(result.output);
    assert.strictEqual(output.state_path, '.eclusa/STATE.md');
    assert.strictEqual(output.roadmap_path, '.eclusa/ROADMAP.md');
    assert.strictEqual(output.project_path, '.eclusa/PROJECT.md');
    assert.strictEqual(output.config_path, '.eclusa/config.json');
  });
  ...
  test('init phase-op returns core and optional phase file paths', () => {
    ...
    const result = runEclusaTools('init phase-op 03', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('init plan-phase detects has_reviews and reviews_path when REVIEWS.md exists', () => {
    ...
    assert.strictEqual(output.reviews_path, '.eclusa/phases/03-api/03-REVIEWS.md');
  });
  ...
  test('init plan-phase extracts phase_req_ids from ROADMAP', () => {
    ...
    const result = runEclusaTools('init plan-phase 3', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('init execute-phase extracts phase_req_ids from ROADMAP', () => {
    ...
    const result = runEclusaTools('init execute-phase 3', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('init commands ROADMAP fallback when phase directory does not exist (#1238)', () => {
  ...
  test('init plan-phase falls back to ROADMAP when no phase directory exists', () => {
    const result = runEclusaTools('init plan-phase 1', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('init execute-phase falls back to ROADMAP when no phase directory exists', () => {
    const result = runEclusaTools('init execute-phase 1', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('init verify-work falls back to ROADMAP when no phase directory exists', () => {
    const result = runEclusaTools('init verify-work 1', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('init plan-phase returns phase_found false when neither directory nor ROADMAP entry exists', () => {
    const result = runEclusaTools('init plan-phase 99', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('init plan-phase prefers disk directory over ROADMAP fallback', () => {
    const phaseDir = path.join(tmpDir, '.eclusa', 'phases', '01-foundation-setup');
    fs.mkdirSync(phaseDir, { recursive: true });
...
describe('cmdInitTodos', () => {
  ...
  test('empty pending dir returns zero count', () => {
    fs.mkdirSync(path.join(tmpDir, '.eclusa', 'todos', 'pending'), { recursive: true });

    const result = runEclusaTools('init todos', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('multiple todos with fields are read correctly', () => {
    ...
    assert.strictEqual(task1.path, '.eclusa/todos/pending/task-1.md');
  });
  ...
  test('area filter returns only matching todos', () => {
    ...
    const result = runEclusaTools('init todos backend', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('area filter miss returns zero count', () => {
    ...
    const result = runEclusaTools('init todos nonexistent', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('cmdInitMilestoneOp', () => {
  ...
  test('no phase directories returns zero counts', () => {
    const result = runEclusaTools('init milestone-op', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('multiple phases with no summaries', () => {
    const phase1 = path.join(tmpDir, '.eclusa', 'phases', '01-setup');
    const phase2 = path.join(tmpDir, '.eclusa', 'phases', '02-api');
  ...
  test('archive directory scanning', () => {
    fs.mkdirSync(path.join(tmpDir, '.eclusa', 'archive', 'v1.0'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.eclusa', 'archive', 'v0.9'), { recursive: true });
...
describe('cmdInitPhaseOp fallback', () => {
  ...
  test('normal path with existing directory', () => {
    ...
    const result = runEclusaTools('init phase-op 3', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('fallback to ROADMAP when no directory exists', () => {
    ...
    const result = runEclusaTools('init phase-op 5', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
`
    ...
    const result = runEclusaTools('init phase-op 2', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('neither directory nor roadmap entry returns not found', () => {
    ...
    const result = runEclusaTools('init phase-op 99', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('cmdInitProgress', () => {
  ...
  test('multiple phases with mixed statuses', () => {
    ...
    const phase3 = path.join(tmpDir, '.eclusa', 'phases', '03-ui');
...
describe('cmdInitQuick', () => {
  ...
  test('with description generates slug and task_dir with YYMMDD-xxx format', () => {
    const result = runEclusaTools('init quick "Fix login bug"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
    ...
    assert.ok(output.task_dir.startsWith('.eclusa/quick/'),
      `task_dir should start with .eclusa/quick/, got: "${output.task_dir}"`);
    assert.ok(output.task_dir.endsWith('-fix-login-bug'),
      `task_dir should end with -fix-login-bug, got: "${output.task_dir}"`);
    assert.ok(/^\.eclusa\/quick\/\d{6}-[0-9a-z]{3}-fix-login-bug$/.test(output.task_dir),
  ...
  test('without description returns null slug and task_dir', () => {
    const result = runEclusaTools('init quick', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('two rapid calls produce different quick_ids (no collision within 2s window)', () => {
    ...
    const r1 = runEclusaTools('init quick "Task one"', tmpDir);
    const r2 = runEclusaTools('init quick "Task two"', tmpDir);
  ...
  test('long description truncates slug to 40 chars', () => {
    const result = runEclusaTools('init quick "This is a very long description that should get truncated to forty characters maximum"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('returns quick branch name when quick_branch_template is configured', () => {
    ...
    assert.ok(output.branch_name.startsWith('eclusa/quick-'));
  ...
  test('uses fallback slug in quick branch name when description is omitted', () => {
    fs.writeFileSync(
      ...
      JSON.stringify({
        git: {
          quick_branch_template: 'eclusa/quick-{quick}-{slug}',
        },
      }, null, 2)
    );
...
describe('cmdInitMapCodebase', () => {
  ...
  test('no codebase dir returns empty', () => {
    const result = runEclusaTools('init map-codebase', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('with existing maps lists md files only', () => {
    const codebaseDir = path.join(tmpDir, '.eclusa', 'codebase');
  ...
  test('map-codebase workflow does not list OpenCode under runtimes without Task tool (#1316)', () => {
    const workflow = fs.readFileSync(
      path.join(__dirname, '..', 'eclusa', 'workflows', 'map-codebase.md'), 'utf8'
    );
...
describe('cmdInitNewProject', () => {
  ...
  test('greenfield project with no code', () => {
    const result = runEclusaTools('init new-project', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('brownfield with codebase map does not need map', () => {
    fs.writeFileSync(path.join(tmpDir, 'package.json'), '{"name":"test"}');
    fs.mkdirSync(path.join(tmpDir, '.eclusa', 'codebase'), { recursive: true });
...
describe('cmdInitNewMilestone', () => {
  ...
  test('returns expected fields', () => {
    const result = runEclusaTools('init new-milestone', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('file existence flags reflect actual state', () => {
    ...
    const result1 = runEclusaTools('init new-milestone', tmpDir);
  ...
  test('reports latest completed milestone and archive target for reset flow', () => {
    fs.writeFileSync(
      path.join(tmpDir, '.eclusa', 'MILESTONES.md'),
      '# Milestones\n\n## v1.2 Search Refresh (Shipped: 2026-02-18)\n\n---\n'
    );
    fs.mkdirSync(path.join(tmpDir, '.eclusa', 'phases', '06-refine-search'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.eclusa', 'phases', '07-polish'), { recursive: true });
    ...
    assert.strictEqual(output.phase_archive_path, '.eclusa/milestones/v1.2-phases');
  });
...
// ─────────────────────────────────────────────────────────────────────────────
// findProjectRoot integration — eclusa-tools resolves project root from sub-repo
// ─────────────────────────────────────────────────────────────────────────────
...
describe('findProjectRoot integration via --cwd', () => {
  ...
  beforeEach(() => {
    ...
    fs.writeFileSync(
      path.join(projectRoot, '.eclusa', 'ROADMAP.md'),
      '# Roadmap\n\n## Phase 1: Foundation\n**Goal:** Setup\n'
    );
  ...
  test('init quick from sub-repo CWD returns project_root pointing to parent', () => {
    ...
    const result = runEclusaTools(['init', 'quick', 'test task', '--cwd', backendDir]);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('init quick from project root returns project_root as-is', () => {
    const result = runEclusaTools(['init', 'quick', 'test task', '--cwd', projectRoot]);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('state load from sub-repo CWD reads project root config', () => {
    ...
    fs.writeFileSync(
      path.join(projectRoot, '.eclusa', 'STATE.md'),
      '---\ncurrent_phase: 1\nphase_name: Foundation\n---\n# State\n'
    );

    const backendDir = path.join(projectRoot, 'backend');
    const result = runEclusaTools(['state', '--cwd', backendDir]);
    assert.ok(result.success, `Command failed: ${result.error}`);
