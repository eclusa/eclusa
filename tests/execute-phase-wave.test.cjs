 ...
 *
 * Validates the /eclusa:execute-phase --wave feature contract:
...
const COMMAND_PATH = path.join(__dirname, '..', 'commands', 'eclusa', 'execute-phase.md');
const WORKFLOW_PATH = path.join(__dirname, '..', 'eclusa', 'workflows', 'execute-phase.md');
const COMMANDS_DOC_PATH = path.join(__dirname, '..', 'docs', 'COMMANDS.md');
const HELP_PATH = path.join(__dirname, '..', 'eclusa', 'workflows', 'help.md');

describe('execute-phase command: --wave flag', () => {
  test('command file exists', () => {
    assert.ok(fs.existsSync(COMMAND_PATH), 'commands/eclusa/execute-phase.md should exist');
  });
...
describe('execute-phase docs: user-facing wave flag', () => {
  test('COMMANDS.md documents --wave usage', () => {
    ...
    assert.ok(
      content.includes('/eclusa:execute-phase 1 --wave 2'),
  ...
  test('help workflow documents --wave behavior', () => {
    ...
    assert.ok(
      content.includes('Usage: `/eclusa:execute-phase 5 --wave 2`'),
