...
`
    ...
    const result = runEclusaTools('roadmap get-phase 1', tmpDir);
...
`
    ...
    const result = runEclusaTools('roadmap get-phase 5', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
`
    ...
    const result = runEclusaTools('roadmap get-phase 2.1', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
`
    ...
    const result1 = runEclusaTools('roadmap get-phase 1', tmpDir);
    ...
    const result2 = runEclusaTools('roadmap get-phase 2', tmpDir);
...
describe('roadmap analyze command', () => {
  ...
  test('missing ROADMAP.md returns error', () => {
    const result = runEclusaTools('roadmap analyze', tmpDir);
...
`
    ...
    const p2 = path.join(tmpDir, '.eclusa', 'phases', '02-authentication');
...
`
    ...
    const p1 = path.join(tmpDir, '.eclusa', 'phases', '01-exploration');
    fs.mkdirSync(p1, { recursive: true });
...
`
    ...
    const p1 = path.join(tmpDir, '.eclusa', 'phases', '01-discussion');
    fs.mkdirSync(p1, { recursive: true });
...
`
    ...
    const p1 = path.join(tmpDir, '.eclusa', 'phases', '01-empty');
    fs.mkdirSync(p1, { recursive: true });
...
describe('roadmap update-plan-progress command', () => {
  ...
  test('missing phase number returns error', () => {
    const result = runEclusaTools('roadmap update-plan-progress', tmpDir);
...
`
    ...
    const result = runEclusaTools('roadmap update-plan-progress 99', tmpDir);
...
`
    ...
    const roadmapContent = fs.readFileSync(path.join(tmpDir, '.eclusa', 'ROADMAP.md'), 'utf-8');
...
`;
    fs.writeFileSync(path.join(tmpDir, '.eclusa', 'ROADMAP.md'), roadmapContent);

    const p50 = path.join(tmpDir, '.eclusa', 'phases', '50-build');
    ...
    const result = runEclusaTools('roadmap update-plan-progress 50', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
