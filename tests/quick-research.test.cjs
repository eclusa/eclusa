/**
 * Eclusa Quick Research Flag Tests
 *
 * Validates the --research flag for /eclusa:quick:
 ...
 * - Workflow spawns eclusa-phase-researcher for research
 */
...
describe('quick command: --research in frontmatter', () => {
  ...
  test('quick.md exists', () => {
    assert.ok(fs.existsSync(commandPath), 'commands/eclusa/quick.md should exist');
  });
...
describe('quick workflow: research step', () => {
  ...
  test('research step spawns eclusa-phase-researcher', () => {
    content = fs.readFileSync(workflowPath, 'utf-8');
    ...
    assert.ok(
      researchSection.includes('subagent_type="eclusa-phase-researcher"'),
      'research step should spawn eclusa-phase-researcher agent'
    );
  });
...
describe('quick task: research file in task directory', () => {
  ...
  test('init quick returns valid task_dir for research file placement', () => {
    const result = runEclusaTools('init quick "Add caching layer"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);

    const output = JSON.parse(result.output);
    assert.ok(output.task_dir, 'task_dir should be non-null');
    assert.ok(
      output.task_dir.startsWith('.eclusa/quick/'),
      'task_dir should be under .eclusa/quick/'
    );
  ...
  test('verify-path-exists detects RESEARCH.md in quick task directory', () => {
    const quickTaskDir = path.join(tmpDir, '.eclusa', 'quick', '1-test-task');
    ...
    const result = runEclusaTools(
      'verify-path-exists .eclusa/quick/1-test-task/1-RESEARCH.md',
      tmpDir
    );
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('quick task directory supports all research workflow artifacts', () => {
    const quickTaskDir = path.join(tmpDir, '.eclusa', 'quick', '1-add-caching');
    fs.mkdirSync(quickTaskDir, { recursive: true });
    ...
    for (const artifact of artifacts) {
      const result = runEclusaTools(
        `verify-path-exists .eclusa/quick/1-add-caching/${artifact}`,
        tmpDir
      );
