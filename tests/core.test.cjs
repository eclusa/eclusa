...
const {
  ...
  findProjectRoot,
  detectSubRepos,
} = require('../eclusa/bin/lib/core.cjs');
...
describe('loadConfig', () => {
  ...
  function writeConfig(obj) {
    fs.writeFileSync(
      path.join(tmpDir, '.eclusa', 'config.json'),
  ...
  test('returns model_overrides when present (REG-01)', () => {
    writeConfig({ model_overrides: { 'eclusa-executor': 'opus' } });
    const config = loadConfig(tmpDir);
    assert.deepStrictEqual(config.model_overrides, { 'eclusa-executor': 'opus' });
  });
...
describe('loadConfig commit_docs gitignore auto-detection (#1250)', () => {
  ...
  test('commit_docs defaults to false when .eclusa/ is gitignored and no explicit config', () => {
    fs.writeFileSync(path.join(tmpDir, '.gitignore'), '.eclusa/\n');
    // No commit_docs in config — should auto-detect
    writeConfig({ model_profile: 'balanced' });
    const config = loadConfig(tmpDir);
    assert.strictEqual(config.commit_docs, false,
      'commit_docs should be false when .eclusa/ is gitignored and not explicitly set');
  });

  test('commit_docs defaults to true when .eclusa/ is NOT gitignored and no explicit config', () => {
    // No .gitignore, no commit_docs in config
    writeConfig({ model_profile: 'balanced' });
    const config = loadConfig(tmpDir);
    assert.strictEqual(config.commit_docs, true,
      'commit_docs should default to true when .eclusa/ is not gitignored');
  });

  test('explicit commit_docs: false is respected even when .eclusa/ is not gitignored', () => {
    writeConfig({ commit_docs: false });
    const config = loadConfig(tmpDir);
    assert.strictEqual(config.commit_docs, false);
  });

  test('explicit commit_docs: true is respected even when .eclusa/ is gitignored', () => {
    fs.writeFileSync(path.join(tmpDir, '.gitignore'), '.eclusa/\n');
  ...
  test('commit_docs auto-detect works with no config.json', () => {
    ...
    try { fs.unlinkSync(path.join(tmpDir, '.eclusa', 'config.json')); } catch {}
    fs.writeFileSync(path.join(tmpDir, '.gitignore'), '.eclusa/\n');
    const config = loadConfig(tmpDir);
    ...
    // a project without config.json hasn't been initialized by Eclusa yet.
...
describe('resolveModelInternal', () => {
  ...
  describe('model profile structural validation', () => {
    test('all known agents resolve to a valid string for each profile', () => {
      const knownAgents = ['eclusa-planner', 'eclusa-executor', 'eclusa-phase-researcher', 'eclusa-codebase-mapper'];
  ...
  describe('override precedence', () => {
    test('per-agent override takes precedence over profile', () => {
      writeConfig({
        ...
        model_overrides: { 'eclusa-executor': 'haiku' },
      });
      assert.strictEqual(resolveModelInternal(tmpDir, 'eclusa-executor'), 'haiku');
    });

    test('opus override resolves to opus', () => {
      writeConfig({
        model_overrides: { 'eclusa-executor': 'opus' },
      });
      assert.strictEqual(resolveModelInternal(tmpDir, 'eclusa-executor'), 'opus');
    });
    ...
    test('agents not in override fall back to profile', () => {
      ...
      // eclusa-planner not overridden, should use quality profile -> opus
      assert.strictEqual(resolveModelInternal(tmpDir, 'eclusa-planner'), 'opus');
    });
  });
  ...
  describe('edge cases', () => {
    test('returns sonnet for unknown agent type', () => {
      writeConfig({ model_profile: 'balanced' });
      assert.strictEqual(resolveModelInternal(tmpDir, 'eclusa-nonexistent'), 'sonnet');
    });
    ...
    test('defaults to balanced profile when model_profile missing', () => {
      writeConfig({});
      // balanced profile, eclusa-planner -> opus
      assert.strictEqual(resolveModelInternal(tmpDir, 'eclusa-planner'), 'opus');
    });
  });
  ...
  describe('resolve_model_ids: "omit"', () => {
    test('returns empty string for known agents', () => {
      ...
      assert.strictEqual(resolveModelInternal(tmpDir, 'eclusa-planner'), '');
    });

    test('returns empty string for unknown agents', () => {
      writeConfig({ resolve_model_ids: 'omit' });
      assert.strictEqual(resolveModelInternal(tmpDir, 'eclusa-nonexistent'), '');
    });

    test('still respects model_overrides even when omit', () => {
      writeConfig({
        resolve_model_ids: 'omit',
        model_overrides: { 'eclusa-planner': 'openai/gpt-5.4' },
      });
      assert.strictEqual(resolveModelInternal(tmpDir, 'eclusa-planner'), 'openai/gpt-5.4');
    });
...
describe('safeReadFile', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'eclusa-core-test-'));
  });
...
describe('pathExistsInternal', () => {
  ...
  test('returns true for existing path', () => {
    assert.strictEqual(pathExistsInternal(tmpDir, '.eclusa'), true);
  });
...
describe('getMilestoneInfo', () => {
  ...
  test('extracts version and name from roadmap', () => {
    fs.writeFileSync(
      path.join(tmpDir, '.eclusa', 'ROADMAP.md'),
  ...
  test('returns active milestone when shipped milestone is collapsed in details block', () => {
    ...
    fs.writeFileSync(path.join(tmpDir, '.eclusa', 'ROADMAP.md'), roadmap);
    const info = getMilestoneInfo(tmpDir);
...
describe('searchPhaseInDir', () => {
  ...
  test('finds phase directory by normalized prefix', () => {
    ...
    const result = searchPhaseInDir(phasesDir, '.eclusa/phases', '01');
  ...
  test('returns null when phase not found', () => {
    fs.mkdirSync(path.join(phasesDir, '01-foundation'));
    const result = searchPhaseInDir(phasesDir, '.eclusa/phases', '99');
...
describe('findPhaseInternal', () => {
  ...
  test('finds phase in current phases directory', () => {
    fs.mkdirSync(path.join(tmpDir, '.eclusa', 'phases', '01-foundation'));
  ...
  test('searches archived milestones when not in current', () => {
    ...
    const archiveDir = path.join(tmpDir, '.eclusa', 'milestones', 'v1.0-phases', '01-foundation');
...
describe('getMilestonePhaseFilter', () => {
  ...
  test('filters directories to only current milestone phases', () => {
    ...
    for (let i = 1; i <= 7; i++) {
      ...
      fs.mkdirSync(path.join(tmpDir, '.eclusa', 'phases', `${padded}-phase-${i}`));
    }
...
describe('stale hook filter', () => {
  test('filter should only match eclusa-prefixed .js files', () => {
    const files = [
      'eclusa-check-update.js',
      'eclusa-context-monitor.js',
      'eclusa-prompt-guard.js',
      'eclusa-statusline.js',
      'eclusa-workflow-guard.js',
      'guard-edits-outside-project.js',  // user hook
      'my-custom-hook.js',               // user hook
      'eclusa-check-update.js.bak',         // backup file
      'README.md',                       // non-js file
    ];

    const eclusaFilter = f => f.startsWith('eclusa-') && f.endsWith('.js');
    const filtered = files.filter(eclusaFilter);
    ...
    assert.deepStrictEqual(filtered, [
      ...
      'eclusa-statusline.js',
      'eclusa-workflow-guard.js',
    ], 'should only include eclusa-prefixed .js files');

    assert.ok(!filtered.includes('guard-edits-outside-project.js'), 'must not include user hooks');
    assert.ok(!filtered.includes('my-custom-hook.js'), 'must not include non-eclusa hooks');
  });
});
...
describe('stale hook path', () => {
  test('eclusa-check-update.js checks eclusa/hooks/ not configDir/hooks/', () => {
    const content = fs.readFileSync(
      path.join(__dirname, '..', 'hooks', 'eclusa-check-update.js'), 'utf-8'
    );
    assert.ok(
      content.includes("path.join(configDir, 'eclusa', 'hooks')"),
      'stale hook check must look in configDir/eclusa/hooks/, not configDir/hooks/'
    );
    assert.ok(
      !content.includes("path.join(configDir, 'hooks')") ||
      content.indexOf("path.join(configDir, 'eclusa', 'hooks')") <
...
describe('resolveWorktreeRoot', () => {
  const { resolveWorktreeRoot } = require('../eclusa/bin/lib/core.cjs');
  let tmpDir;
...
// ─── resolveWorktreeRoot — linked worktree with .eclusa/ (#1315) ───────────

describe('resolveWorktreeRoot with linked worktree .eclusa/', () => {
  const { resolveWorktreeRoot } = require('../eclusa/bin/lib/core.cjs');
  const { execSync: execSyncLocal } = require('child_process');
  ...
  function initBareGitRepo() {
    const dir = normalizePath(fs.mkdtempSync(path.join(os.tmpdir(), 'eclusa-wt-main-')));
  ...
  test('returns linked worktree cwd when it has its own .eclusa/', () => {
    // Add .eclusa/ to main repo
    fs.mkdirSync(path.join(mainDir, '.eclusa'), { recursive: true });

    // Create a linked worktree
    worktreeDir = normalizePath(fs.mkdtempSync(path.join(os.tmpdir(), 'eclusa-wt-linked-')));
    fs.rmSync(worktreeDir, { recursive: true, force: true });
    execSyncLocal(`git worktree add "${worktreeDir}" -b test-linked`, { cwd: mainDir, stdio: 'pipe' });

    // Give the linked worktree its own .eclusa/
    fs.mkdirSync(path.join(worktreeDir, '.eclusa'), { recursive: true });
    ...
    assert.strictEqual(result, worktreeDir,
      'linked worktree with .eclusa/ should resolve to itself, not the main repo');
  });

  test('returns main repo root when linked worktree has no .eclusa/', () => {
    // Create a linked worktree (no .eclusa/ in main or worktree)
    worktreeDir = normalizePath(fs.mkdtempSync(path.join(os.tmpdir(), 'eclusa-wt-linked-')));
    fs.rmSync(worktreeDir, { recursive: true, force: true });
    ...
    assert.strictEqual(result, expected,
      'linked worktree without .eclusa/ should resolve to main repo root');
  });
});
...
describe('monorepo worktree CWD preservation', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'eclusa-monorepo-wt-'));
  });
  ...
  test('CWD with .eclusa/ skips worktree resolution (monorepo subdirectory)', () => {
    const subDir = path.join(tmpDir, 'service-alpha');
    fs.mkdirSync(path.join(subDir, '.eclusa'), { recursive: true });
    let cwd = subDir;
    if (!fs.existsSync(path.join(cwd, '.eclusa'))) {
      const worktreeRoot = resolveWorktreeRoot(cwd);
      if (worktreeRoot !== cwd) cwd = worktreeRoot;
    }
    assert.strictEqual(cwd, subDir, 'CWD with .eclusa/ must not be overridden by worktree resolution');
  });

  test('CWD without .eclusa/ still goes through worktree resolution', () => {
    let cwd = tmpDir;
    ...
    assert.ok(worktreeResolutionCalled, 'worktree resolution must be called when .eclusa/ is absent');
  });
});
...
describe('withPlanningLock', () => {
  const { withPlanningLock, planningDir } = require('../eclusa/bin/lib/core.cjs');
  let tmpDir;
  ...
  test('recovers from stale lock (>30s old)', () => {
    const lockPath = path.join(tmpDir, '.eclusa', '.lock');
    // Create a stale lock
...
describe('detectSubRepos', () => {
  ...
  beforeEach(() => {
    projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'eclusa-detect-test-'));
  });
...
describe('loadConfig sub_repos auto-sync', () => {
  ...
  beforeEach(() => {
    projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'eclusa-sync-test-'));
    fs.mkdirSync(path.join(projectRoot, '.eclusa'), { recursive: true });
  });
  ...
  test('migrates multiRepo: true to sub_repos array', () => {
    ...
    fs.writeFileSync(
      path.join(projectRoot, '.eclusa', 'config.json'),
    ...
    const saved = JSON.parse(fs.readFileSync(path.join(projectRoot, '.eclusa', 'config.json'), 'utf-8'));
...
describe('findProjectRoot', () => {
  ...
  beforeEach(() => {
    projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'eclusa-root-test-'));
  });
  ...
  test('returns startDir when no .eclusa/ exists anywhere', () => {
    const subDir = path.join(projectRoot, 'backend');
    fs.mkdirSync(subDir);
    assert.strictEqual(findProjectRoot(subDir), subDir);
  });

  test('returns startDir when .eclusa/ is in startDir itself', () => {
    fs.mkdirSync(path.join(projectRoot, '.eclusa'), { recursive: true });
    assert.strictEqual(findProjectRoot(projectRoot), projectRoot);
  });

  test('walks up to parent with .eclusa/ and sub_repos config listing this dir', () => {
    fs.mkdirSync(path.join(projectRoot, '.eclusa'), { recursive: true });
    fs.writeFileSync(
      path.join(projectRoot, '.eclusa', 'config.json'),
  ...
  test('walks up from subdirectory when .git is at same level as .eclusa/ (single-repo)', () => {
    // Common single-repo layout: .git and .eclusa are siblings at project root
    fs.mkdirSync(path.join(projectRoot, '.eclusa'), { recursive: true });
    ...
    // Should detect that parent has .eclusa/ and .git is at that same level
    assert.strictEqual(findProjectRoot(srcDir), projectRoot);
  });

  test('walks up from deep subdirectory when .git is at same level as .eclusa/', () => {
    // Single-repo: .git and .eclusa at root, cwd deep inside
    fs.mkdirSync(path.join(projectRoot, '.eclusa'), { recursive: true });
    fs.mkdirSync(path.join(projectRoot, '.git'), { recursive: true });
  ...
  test('returns startDir when .eclusa exists at same level (cwd is project root)', () => {
  ...
  test('does not walk past child with own .eclusa/ to workspace parent (#1362)', () => {
    // Workspace layout: parent has .eclusa/, child git repo also has .eclusa/
    // findProjectRoot should return the child (startDir), not the parent
    fs.mkdirSync(path.join(projectRoot, '.eclusa'), { recursive: true });

    const childRepo = path.join(projectRoot, 'authenticator');
    fs.mkdirSync(path.join(childRepo, '.eclusa'), { recursive: true });
    fs.mkdirSync(path.join(childRepo, '.git'), { recursive: true });

    assert.strictEqual(findProjectRoot(childRepo), childRepo);
  });

  test('does not walk past nested dir whose git root has .eclusa/ (#1362)', () => {
    // Workspace layout: parent has .eclusa/, child git repo also has .eclusa/
...
describe('reapStaleTempFiles', () => {
  test('removes stale eclusa-*.json files older than maxAgeMs', () => {
    const tmpDir = os.tmpdir();
    const stalePath = path.join(tmpDir, `eclusa-reap-test-${Date.now()}.json`);
    fs.writeFileSync(stalePath, '{}');
    // Set mtime to 10 minutes ago
    const oldTime = new Date(Date.now() - 10 * 60 * 1000);
    fs.utimesSync(stalePath, oldTime, oldTime);

    reapStaleTempFiles('eclusa-reap-test-', { maxAgeMs: 5 * 60 * 1000 });

    assert.ok(!fs.existsSync(stalePath), 'stale file should be removed');
  });

  test('preserves fresh eclusa-*.json files', () => {
    const tmpDir = os.tmpdir();
    const freshPath = path.join(tmpDir, `eclusa-reap-fresh-${Date.now()}.json`);
    fs.writeFileSync(freshPath, '{}');

    reapStaleTempFiles('eclusa-reap-fresh-', { maxAgeMs: 5 * 60 * 1000 });
  ...
  test('removes stale temp directories when present', () => {
    const tmpDir = os.tmpdir();
    const staleDir = fs.mkdtempSync(path.join(tmpDir, 'eclusa-reap-dir-'));
    fs.writeFileSync(path.join(staleDir, 'data.jsonl'), 'test');
    // Set mtime to 10 minutes ago
    const oldTime = new Date(Date.now() - 10 * 60 * 1000);
    fs.utimesSync(staleDir, oldTime, oldTime);

    reapStaleTempFiles('eclusa-reap-dir-', { maxAgeMs: 5 * 60 * 1000 });
  ...
  test('does not throw on empty or missing prefix matches', () => {
    assert.doesNotThrow(() => {
      reapStaleTempFiles('eclusa-nonexistent-prefix-xyz-', { maxAgeMs: 0 });
    });
  });
});
