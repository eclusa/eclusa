...
function createProjectWithState(tmpDir, roadmap, state) {
  if (roadmap) {
    fs.writeFileSync(path.join(tmpDir, '.eclusa', 'ROADMAP.md'), roadmap, 'utf-8');
  }
  if (state) {
    fs.writeFileSync(path.join(tmpDir, '.eclusa', 'STATE.md'), state, 'utf-8');
  }
}
...
describe('planningDir workstream awareness via env var', () => {
  ...
  before(() => {
    ...
    const wsDir = path.join(tmpDir, '.eclusa', 'workstreams', 'alpha');
    fs.mkdirSync(path.join(wsDir, 'phases'), { recursive: true });
    fs.writeFileSync(path.join(wsDir, 'STATE.md'), '# State\n**Status:** In progress\n**Current Phase:** 1\n');
    fs.writeFileSync(path.join(wsDir, 'ROADMAP.md'), '## Roadmap v1.0: Alpha\n### Phase 1: Setup\n');
    fs.writeFileSync(path.join(tmpDir, '.eclusa', 'active-workstream'), 'alpha\n');
  });

  after(() => cleanup(tmpDir));

  test('state json returns workstream-scoped state when ECLUSA_WORKSTREAM is set', () => {
    const result = runEclusaTools(['state', 'json', '--raw'], tmpDir, { ECLUSA_WORKSTREAM: 'alpha' });
    assert.ok(result.success, `state json failed: ${result.error}`);
    const data = JSON.parse(result.output);
    assert.ok(data.status || data.current_phase !== undefined, 'should return state data');
  });

  test('state json reads from flat .eclusa when no workstream set', () => {
    // Clear active-workstream so no auto-detection
    try { fs.unlinkSync(path.join(tmpDir, '.eclusa', 'active-workstream')); } catch {}
    const result = runEclusaTools(['state', 'json', '--raw'], tmpDir, { ECLUSA_WORKSTREAM: '' });
    // Should fail or return empty state since flat .eclusa/ has no STATE.md
    assert.ok(!result.success || result.output.includes('not found') || result.output === '{}',
      'should read from flat .eclusa/');
    // Restore
    fs.writeFileSync(path.join(tmpDir, '.eclusa', 'active-workstream'), 'alpha\n');
  });

  test('--ws flag overrides ECLUSA_WORKSTREAM env var', () => {
    // Create a second workstream
    const betaDir = path.join(tmpDir, '.eclusa', 'workstreams', 'beta');
    fs.mkdirSync(path.join(betaDir, 'phases'), { recursive: true });
    fs.writeFileSync(path.join(betaDir, 'STATE.md'), '# State\n**Status:** Beta active\n');

    const result = runEclusaTools(['state', 'json', '--raw', '--ws', 'beta'], tmpDir, { ECLUSA_WORKSTREAM: 'alpha' });
...
describe('workstream create', () => {
  ...
  test('creates a new workstream in clean project', () => {
    const result = runEclusaTools(['workstream', 'create', 'feature-x', '--raw'], tmpDir);
    ...
    assert.ok(fs.existsSync(path.join(tmpDir, '.eclusa', 'workstreams', 'feature-x', 'STATE.md')));
    assert.ok(fs.existsSync(path.join(tmpDir, '.eclusa', 'workstreams', 'feature-x', 'phases')));
  });

  test('sets created workstream as active', () => {
    const active = fs.readFileSync(path.join(tmpDir, '.eclusa', 'active-workstream'), 'utf-8').trim();
  ...
  test('creates second workstream', () => {
    const result = runEclusaTools(['workstream', 'create', 'feature-y', '--raw'], tmpDir);
...
describe('workstream create with migration', () => {
  ...
  before(() => {
    ...
    fs.writeFileSync(path.join(tmpDir, '.eclusa', 'ROADMAP.md'), '## Roadmap v1.0: Existing\n### Phase 1: A\n');
  ...
  test('migrates existing flat work to named workstream', () => {
    const result = runEclusaTools(['workstream', 'create', 'new-feature', '--migrate-name', 'existing-work', '--raw'], tmpDir);
    ...
    assert.ok(fs.existsSync(path.join(tmpDir, '.eclusa', 'workstreams', 'existing-work', 'ROADMAP.md')));
    assert.ok(fs.existsSync(path.join(tmpDir, '.eclusa', 'workstreams', 'existing-work', 'STATE.md')));
    // Shared files stay
    assert.ok(fs.existsSync(path.join(tmpDir, '.eclusa', 'PROJECT.md')));
  });
});
...
describe('workstream list', () => {
  ...
  before(() => {
    ...
    for (const ws of ['alpha', 'beta']) {
      const wsDir = path.join(tmpDir, '.eclusa', 'workstreams', ws);
      fs.mkdirSync(path.join(wsDir, 'phases'), { recursive: true });
  ...
  test('lists all workstreams', () => {
    const result = runEclusaTools(['workstream', 'list', '--raw'], tmpDir);
  ...
  describe('flat mode', () => {
    ...
    test('reports flat mode when no workstreams exist', () => {
      const result = runEclusaTools(['workstream', 'list', '--raw'], flatDir);
      assert.ok(result.success);
      const data = JSON.parse(result.output);
...
describe('workstream status', () => {
  ...
  test('returns detailed status for workstream', () => {
    const result = runEclusaTools(['workstream', 'status', 'alpha', '--raw'], tmpDir);
  ...
  test('returns not found for missing workstream', () => {
    const result = runEclusaTools(['workstream', 'status', 'nonexistent', '--raw'], tmpDir);
    assert.ok(result.success);
    const data = JSON.parse(result.output);
...
describe('workstream complete', () => {
  ...
  before(() => {
    tmpDir = createTempProject();
    const wsDir = path.join(tmpDir, '.eclusa', 'workstreams', 'done-ws');
    fs.mkdirSync(path.join(wsDir, 'phases'), { recursive: true });
    fs.writeFileSync(path.join(wsDir, 'STATE.md'), '# State\n**Status:** Complete\n');
    fs.writeFileSync(path.join(tmpDir, '.eclusa', 'active-workstream'), 'done-ws\n');
  });

  after(() => cleanup(tmpDir));

  test('archives workstream to milestones/', () => {
    const result = runEclusaTools(['workstream', 'complete', 'done-ws', '--raw'], tmpDir);
    assert.ok(result.success, `complete failed: ${result.error}`);
    const data = JSON.parse(result.output);
    assert.strictEqual(data.completed, true);
    assert.ok(data.archived_to.startsWith('.eclusa/milestones/ws-done-ws'));
    // Workstream dir should be gone
    assert.ok(!fs.existsSync(path.join(tmpDir, '.eclusa', 'workstreams', 'done-ws')));
  });

  test('clears active-workstream when completing active one', () => {
    assert.ok(!fs.existsSync(path.join(tmpDir, '.eclusa', 'active-workstream')));
  });
});
...
describe('workstream set/get', () => {
  ...
  test('sets active workstream', () => {
    const result = runEclusaTools(['workstream', 'set', 'ws-a', '--raw'], tmpDir);
    assert.ok(result.success);
    assert.strictEqual(result.output, 'ws-a');
  });

  test('gets active workstream', () => {
    const result = runEclusaTools(['workstream', 'get', '--raw'], tmpDir);
    assert.ok(result.success);
    assert.strictEqual(result.output, 'ws-a');
  });
});
...
describe('getOtherActiveWorkstreams', () => {
  ...
  before(() => {
    ...
    fs.writeFileSync(path.join(tmpDir, '.eclusa', 'workstreams', 'alpha', 'STATE.md'),
      '# State\n**Status:** In progress\n**Current Phase:** 3\n');
    fs.writeFileSync(path.join(tmpDir, '.eclusa', 'workstreams', 'beta', 'STATE.md'),
      '# State\n**Status:** In progress\n**Current Phase:** 5\n');
    fs.writeFileSync(path.join(tmpDir, '.eclusa', 'workstreams', 'gamma', 'STATE.md'),
...
describe('workstream progress', () => {
  ...
  before(() => {
    tmpDir = createTempProject();
    const wsDir = path.join(tmpDir, '.eclusa', 'workstreams', 'feature');
    ...
    fs.writeFileSync(path.join(wsDir, 'STATE.md'), '# State\n**Status:** In progress\n**Current Phase:** 2\n');
    fs.writeFileSync(path.join(wsDir, 'ROADMAP.md'), '## Roadmap\n### Phase 1: Init\n### Phase 2: Build\n');
    fs.writeFileSync(path.join(tmpDir, '.eclusa', 'active-workstream'), 'feature\n');
  });

  after(() => cleanup(tmpDir));

  test('returns progress summary', () => {
    const result = runEclusaTools(['workstream', 'progress', '--raw'], tmpDir);
...
// ─── Integration: eclusa-tools --ws flag ────────────────────────────────────────

describe('eclusa-tools --ws flag integration', () => {
  let tmpDir;

  before(() => {
    tmpDir = createTempProject();
    // Create a workstream with roadmap
    const wsDir = path.join(tmpDir, '.eclusa', 'workstreams', 'test-ws');
    fs.mkdirSync(path.join(wsDir, 'phases', '01-setup'), { recursive: true });
  ...
  test('find-phase resolves to workstream-scoped phases via --ws', () => {
    const result = runEclusaTools(['find-phase', '1', '--raw', '--ws', 'test-ws'], tmpDir);
  ...
  test('find-phase returns JSON with workstream path when not raw', () => {
    const result = runEclusaTools(['find-phase', '1', '--ws', 'test-ws'], tmpDir);
    assert.ok(result.success, `find-phase failed: ${result.error}`);
    const data = JSON.parse(result.output);
...
describe('path traversal rejection', () => {
  ...
  before(() => {
    tmpDir = createTempProject();
    fs.writeFileSync(path.join(tmpDir, '.eclusa', 'PROJECT.md'), '# Project\n');
    const wsDir = path.join(tmpDir, '.eclusa', 'workstreams', 'legit');
    fs.mkdirSync(path.join(wsDir, 'phases'), { recursive: true });
    fs.writeFileSync(path.join(wsDir, 'STATE.md'), '# State\n');
  });
  ...
  describe('--ws flag rejects traversal attempts', () => {
    for (const name of maliciousNames) {
      test(`rejects --ws=${name}`, () => {
        const result = runEclusaTools(['workstream', 'list', '--raw', '--ws', name], tmpDir);
  ...
  describe('ECLUSA_WORKSTREAM env var rejects traversal attempts', () => {
    for (const name of maliciousNames) {
      test(`rejects ECLUSA_WORKSTREAM=${name}`, () => {
        const result = runEclusaTools(['workstream', 'list', '--raw'], tmpDir, { ECLUSA_WORKSTREAM: name });
        assert.ok(!result.success, `should reject ECLUSA_WORKSTREAM=${name}`);
        assert.ok(result.error.includes('Invalid workstream name'), `error should mention invalid name for: ${name}`);
      });
    }
  });
  ...
  describe('cmdWorkstreamSet rejects traversal attempts', () => {
    for (const name of maliciousNames) {
      test(`rejects set ${name}`, () => {
        const result = runEclusaTools(['workstream', 'set', name, '--raw'], tmpDir);
  ...
  describe('getActiveWorkstream rejects poisoned active-workstream file', () => {
    for (const name of maliciousNames) {
      test(`rejects poisoned file containing ${name}`, () => {
        ...
        fs.writeFileSync(path.join(tmpDir, '.eclusa', 'active-workstream'), name + '\n');
        const result = runEclusaTools(['workstream', 'get'], tmpDir, { ECLUSA_WORKSTREAM: '' });
  ...
  describe('setActiveWorkstream rejects invalid names directly', () => {
    const { setActiveWorkstream } = require('../eclusa/bin/lib/core.cjs');
    for (const name of maliciousNames) {
