/**
 * Eclusa Tools Tests - Validate Health Command
 *
...
describe('validate health command', () => {
  ...
  test("returns 'broken' when .eclusa directory is missing", () => {
    // createTempProject creates .eclusa/phases — remove it entirely
    fs.rmSync(path.join(tmpDir, '.eclusa'), { recursive: true, force: true });

    const result = runEclusaTools('validate health', tmpDir);
  ...
  test('warns about incorrectly named phase directories', () => {
    ...
    fs.mkdirSync(path.join(tmpDir, '.eclusa', 'phases', 'bad_name'), { recursive: true });
  ...
  test('warns about phase on disk but not in ROADMAP', () => {
    ...
    fs.mkdirSync(path.join(tmpDir, '.eclusa', 'phases', '99-orphan'), { recursive: true });
  ...
  test("returns 'healthy' when all checks pass", () => {
    ...
    const phaseDir = path.join(tmpDir, '.eclusa', 'phases', '01-a');
    fs.mkdirSync(phaseDir, { recursive: true });
...
describe('validate health --repair command', () => {
  ...
  beforeEach(() => {
    ...
    // (E001, E003 are not repairable so we always need .eclusa/ and ROADMAP.md)
    writeMinimalProjectMd(tmpDir);
    writeMinimalRoadmap(tmpDir, ['1']);
    fs.mkdirSync(path.join(tmpDir, '.eclusa', 'phases', '01-a'), { recursive: true });
  });
  ...
  test('creates config.json with defaults when missing', () => {
    ...
    const result = runEclusaTools('validate health --repair', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
    ...
    assert.strictEqual(diskConfig.phase_branch_template, 'eclusa/phase-{phase}-{slug}');
    assert.strictEqual(diskConfig.milestone_branch_template, 'eclusa/{milestone}-{slug}');
  });
  ...
  test('regenerates STATE.md when missing', () => {
    writeValidConfigJson(tmpDir);
    // No STATE.md
    const statePath = path.join(tmpDir, '.eclusa', 'STATE.md');
