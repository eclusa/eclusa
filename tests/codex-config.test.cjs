/**
 * Eclusa Tools Tests - codex-config.cjs
 *
...
describe('getCodexSkillAdapterHeader', () => {
  test('contains all three sections', () => {
    const result = getCodexSkillAdapterHeader('eclusa-execute-phase');
  ...
  test('includes correct invocation syntax', () => {
    const result = getCodexSkillAdapterHeader('eclusa-plan-phase');
    assert.ok(result.includes('`$eclusa-plan-phase`'), 'has $skillName invocation');
    assert.ok(result.includes('{{ECLUSA_ARGS}}'), 'has ECLUSA_ARGS variable');
  });

  test('section B maps AskUserQuestion parameters', () => {
    const result = getCodexSkillAdapterHeader('eclusa-discuss-phase');
...
<role>
You are a Eclusa plan executor.
</role>`;
    ...
    assert.ok(result.includes('"eclusa-executor"'), 'has quoted name');
    assert.ok(result.includes('"Executes Eclusa plans with atomic commits"'), 'has quoted description');
    ...
    assert.ok(result.includes('role: eclusa-executor'), 'role matches agent name');
    assert.ok(result.includes('tools: Read, Write, Edit, Bash, Grep, Glob'), 'tools in role block');
    assert.ok(result.includes('purpose: Executes Eclusa plans with atomic commits'), 'purpose from description');
...
Run /eclusa:execute-phase to proceed.`;

    const result = convertClaudeAgentToCodexAgent(input);
    assert.ok(result.includes('$eclusa-execute-phase'), 'converts slash commands');
    assert.ok(!result.includes('/eclusa:execute-phase'), 'original slash command removed');
  });
...
<role>You check plans.</role>`;
  ...
  test('includes required name and description fields', () => {
    const result = generateCodexAgentToml('eclusa-executor', sampleAgent);
    assert.ok(result.includes('name = "eclusa-executor"'), 'has name');
  ...
  test('falls back to generated description when frontmatter is missing fields', () => {
    ...
    const result = generateCodexAgentToml('eclusa-unknown', minimalAgent);
    assert.ok(result.includes('name = "eclusa-unknown"'), 'falls back to agent name');
    assert.ok(result.includes('description = "Eclusa agent eclusa-unknown"'), 'falls back to synthetic description');
  });

  test('defaults unknown agents to read-only', () => {
    const result = generateCodexAgentToml('eclusa-unknown', sampleAgent);
...
describe('CODEX_AGENT_SANDBOX', () => {
  ...
  test('workspace-write agents have write tools', () => {
    const writeAgents = [
      'eclusa-executor', 'eclusa-planner', 'eclusa-phase-researcher',
      'eclusa-project-researcher', 'eclusa-research-synthesizer', 'eclusa-verifier',
      'eclusa-codebase-mapper', 'eclusa-roadmapper', 'eclusa-debugger',
    ];
  ...
  test('read-only agents have no write tools', () => {
    const readOnlyAgents = ['eclusa-plan-checker', 'eclusa-integration-checker'];
...
describe('generateCodexConfigBlock', () => {
  const agents = [
    { name: 'eclusa-executor', description: 'Executes plans' },
    { name: 'eclusa-planner', description: 'Creates plans' },
  ];

  test('starts with Eclusa marker', () => {
    const result = generateCodexConfigBlock(agents);
    assert.ok(result.startsWith(ECLUSA_CODEX_MARKER), 'starts with marker');
  });
  ...
  test('does not include feature flags or agents table header', () => {
    ...
    // Should not have bare [agents] table header (only [agents.eclusa-*] sections)
  ...
  test('includes per-agent sections with relative paths (no targetDir)', () => {
    const result = generateCodexConfigBlock(agents);
    assert.ok(result.includes('[agents.eclusa-executor]'), 'has executor section');
    assert.ok(result.includes('[agents.eclusa-planner]'), 'has planner section');
    assert.ok(result.includes('config_file = "agents/eclusa-executor.toml"'), 'relative config_file without targetDir');
  ...
  test('uses absolute config_file paths when targetDir is provided', () => {
    ...
    assert.ok(result.includes('config_file = "/home/user/.codex/agents/eclusa-executor.toml"'), 'absolute executor path');
    assert.ok(result.includes('config_file = "/home/user/.codex/agents/eclusa-planner.toml"'), 'absolute planner path');
...
describe('stripEclusaFromCodexConfig', () => {
  test('returns null for Eclusa-only config', () => {
    const content = `${ECLUSA_CODEX_MARKER}\n[features]\nmulti_agent = true\n`;
    const result = stripEclusaFromCodexConfig(content);
    assert.strictEqual(result, null, 'returns null when Eclusa-only');
  });
  ...
  test('preserves user content before marker', () => {
    ...
    assert.ok(!result.includes('multi_agent'), 'removes Eclusa content');
    assert.ok(!result.includes(ECLUSA_CODEX_MARKER), 'removes marker');
  });
  ...
  test('strips injected keys above marker on uninstall', () => {
    ...
    assert.ok(!result.includes(ECLUSA_CODEX_MARKER), 'strips marker');
  });

  test('removes [agents.eclusa-*] sections', () => {
    const content = `[agents.eclusa-executor]\ndescription = "test"\nconfig_file = "agents/eclusa-executor.toml"\n\n[agents.custom-agent]\ndescription = "user agent"\n`;
    const result = stripEclusaFromCodexConfig(content);
    assert.ok(!result.includes('[agents.eclusa-executor]'), 'removes Eclusa agent section');
...
describe('mergeCodexConfig', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'eclusa-codex-merge-'));
  });
  ...
  test('case 1: creates new config.toml', () => {
    ...
    assert.ok(content.includes(ECLUSA_CODEX_MARKER), 'has marker');
    assert.ok(content.includes('[agents.eclusa-executor]'), 'has agent');
    assert.ok(!content.includes('[features]'), 'no features section');
    assert.ok(!content.includes('multi_agent'), 'no multi_agent');
  });

  test('case 2: replaces existing Eclusa block', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    ...
    const newBlock = generateCodexConfigBlock([
      { name: 'eclusa-executor', description: 'Updated description' },
      { name: 'eclusa-planner', description: 'New agent' },
    ]);
    ...
    assert.ok(content.includes('[agents.eclusa-planner]'), 'has new agent');
    // Verify no duplicate markers
    const markerCount = (content.match(new RegExp(ECLUSA_CODEX_MARKER.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g')) || []).length;
    assert.strictEqual(markerCount, 1, 'exactly one marker');
  });

  test('case 3: appends to config without Eclusa marker', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    ...
    const content = fs.readFileSync(configPath, 'utf8');
    assert.ok(content.includes('[model]'), 'preserves user content');
    assert.ok(content.includes(ECLUSA_CODEX_MARKER), 'adds marker');
    assert.ok(content.includes('[agents.eclusa-executor]'), 'has agent');
  });

  test('case 3 with existing [features]: preserves user features, does not inject Eclusa keys', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    ...
    assert.ok(content.includes(ECLUSA_CODEX_MARKER), 'adds marker for agents block');
    assert.ok(content.includes('[agents.eclusa-executor]'), 'has agent');
  });

  test('case 3 strips existing [agents.eclusa-*] sections before appending fresh block', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    const existing = [
      '[model]',
      'name = "o3"',
    ...
    const content = fs.readFileSync(configPath, 'utf8');
    const eclusaAgentCount = (content.match(/^\[agents\.eclusa-executor\]\s*$/gm) || []).length;
    const markerCount = (content.match(new RegExp(ECLUSA_CODEX_MARKER.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g')) || []).length;

    assert.ok(content.includes('[model]'), 'preserves user content');
    assert.ok(content.includes('[agents.custom-agent]'), 'preserves non-Eclusa agent section');
    assert.strictEqual(eclusaAgentCount, 1, 'keeps exactly one Eclusa agent section');
    assert.strictEqual(markerCount, 1, 'adds exactly one marker block');
    assert.ok(!/\n{3,}# Eclusa Agent Configuration/.test(content), 'does not leave extra blank lines before marker block');
  });
  ...
  test('case 2 does not inject feature keys', () => {
    ...
    assert.ok(content.includes('[agents.eclusa-executor]'), 'has agent from fresh block');
  });

  test('case 2 strips leaked [agents] and [agents.eclusa-*] from before content', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    const brokenContent = [
      '[features]',
    ...
    assert.ok(!beforeMarker.includes('[agents.eclusa-'), 'no leaked [agents.eclusa-*] above marker');
  });

  test('case 2 strips leaked Eclusa-managed sections above marker in CRLF files', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    const brokenContent = [
      '[features]',
      'child_agents_md = false',
    ...
    assert.strictEqual(countMatches(beforeMarker, /^\[agents\.eclusa-executor\]\s*$/gm), 0, 'removes leaked Eclusa agent section above marker');
    assert.strictEqual(countMatches(content, /^\[agents\.eclusa-executor\]\s*$/gm), 1, 'keeps one managed agent section');
    assertUsesOnlyEol(content, '\r\n');
  });

  test('case 2 preserves user-authored [agents] tables while stripping leaked Eclusa sections in CRLF files', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    const brokenContent = [
      '[features]',
      'child_agents_md = false',
    ...
    assert.strictEqual(countMatches(beforeMarker, /^\[agents\.eclusa-executor\]\s*$/gm), 0, 'removes leaked Eclusa agent section above marker');
    assert.strictEqual(countMatches(content, /^\[agents\.eclusa-executor\]\s*$/gm), 1, 'keeps one managed agent section in the Eclusa block');
    assertUsesOnlyEol(content, '\r\n');
  });
  ...
  test('preserves CRLF when appending Eclusa block to existing config', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    fs.writeFileSync(configPath, '[model]\r\nname = "o3"\r\n', 'utf8');
    ...
    assert.ok(content.includes(`${ECLUSA_CODEX_MARKER}\r\n`), 'writes marker with CRLF');
    assertUsesOnlyEol(content, '\r\n');
  });

  test('uses the first newline style when appending Eclusa block to mixed-EOL configs', () => {
    const configPath = path.join(tmpDir, 'config.toml');
...
describe('installCodexConfig (integration)', () => {
  ...
  beforeEach(() => {
    tmpTarget = fs.mkdtempSync(path.join(os.tmpdir(), 'eclusa-codex-install-'));
  });
  ...
  (hasAgents ? test : test.skip)('generates config.toml and agent .toml files', () => {
    ...
    assert.ok(config.includes(ECLUSA_CODEX_MARKER), 'has Eclusa marker');
    assert.ok(config.includes('[agents.eclusa-executor]'), 'has executor agent');
    ...
    assert.ok(fs.existsSync(path.join(agentsDir, 'eclusa-executor.toml')), 'executor .toml exists');
    assert.ok(fs.existsSync(path.join(agentsDir, 'eclusa-plan-checker.toml')), 'plan-checker .toml exists');

    const executorToml = fs.readFileSync(path.join(agentsDir, 'eclusa-executor.toml'), 'utf8');
    assert.ok(executorToml.includes('name = "eclusa-executor"'), 'executor has name');
    assert.ok(executorToml.includes('description = "Executes Eclusa plans with atomic commits, deviation handling, checkpoint protocols, and state management. Spawned by execute-phase orchestrator or execute-plan command."'), 'executor has description');
    ...
    const checkerToml = fs.readFileSync(path.join(agentsDir, 'eclusa-plan-checker.toml'), 'utf8');
    assert.ok(checkerToml.includes('name = "eclusa-plan-checker"'), 'plan-checker has name');
...
describe('Codex install hook configuration (e2e)', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'eclusa-codex-e2e-'));
  ...
  test('fresh CODEX_HOME enables codex_hooks without draft root defaults', () => {
    ...
    assert.ok(content.includes('# Eclusa Hooks\n[[hooks]]\nevent = "SessionStart"\n'), 'writes Eclusa SessionStart hook block');
    assert.strictEqual(countMatches(content, /^codex_hooks = true$/gm), 1, 'writes one codex_hooks key');
    assert.strictEqual(countMatches(content, /eclusa-update-check\.js/g), 1, 'writes one Eclusa update hook');
  ...
  test('existing LF config without [features] gets one features block and preserves user content', () => {
    ...
    assert.strictEqual(countMatches(content, /eclusa-update-check\.js/g), 1, 'adds one Eclusa update hook');
    assertNoDraftRootKeys(content);
  });
  ...
  test('bare top-level keys are NOT trapped under [features] (#1202)', () => {
    ...
    // followed by [projects] section. Eclusa must not prepend [features] before
  ...
  test('existing [features] at EOF without trailing newline is updated in place', () => {
    ...
    assert.ok(content.indexOf('codex_hooks = true') < content.indexOf('[agents.eclusa-codebase-mapper]'), 'keeps codex_hooks before the next real table');
    assertNoDraftRootKeys(content);
  });
  ...
  test('existing empty [features] and codex_hooks = false are normalized and remain idempotent', () => {
    ...
    assert.ok(content.includes('command = "echo custom"'), 'preserves custom hook');
    assert.strictEqual(countMatches(content, /eclusa-update-check\.js/g), 1, 'does not duplicate Eclusa update hook');
    assertNoDraftRootKeys(content);
  });
  ...
  test('quoted [features] headers are recognized as the existing features table', () => {
    ...
    assert.strictEqual(countMatches(content, /eclusa-update-check\.js/g), 1, 'keeps one Eclusa update hook');
    assertNoDraftRootKeys(content);
  });
  ...
  test('quoted table headers containing # are parsed without treating # as a comment start', () => {
    ...
    assert.strictEqual(countMatches(content, /^codex_hooks = true$/gm), 1, 'adds one codex_hooks key');
    assert.strictEqual(countMatches(content, /eclusa-update-check\.js/g), 1, 'remains idempotent for the Eclusa hook block');
    assertNoDraftRootKeys(content);
  });
  ...
  test('existing dotted features config stays dotted and does not grow a [features] table', () => {
    ...
    assert.strictEqual(countMatches(content, /eclusa-update-check\.js/g), 1, 'adds one Eclusa update hook for dotted codex_hooks and remains idempotent');
    assertNoDraftRootKeys(content);
  });
  ...
  test('root inline-table features assignments are left untouched without appending invalid dotted keys or hooks', () => {
    ...
    assert.strictEqual(countMatches(content, /eclusa-update-check\.js/g), 0, 'does not add the Eclusa hook block when codex_hooks cannot be enabled safely');
    assert.ok(content.includes('[agents.eclusa-executor]'), 'still installs the managed agent block');
    assertNoDraftRootKeys(content);
  });
  ...
  test('quoted dotted codex_hooks keys stay dotted and are normalized without duplication', () => {
    ...
    assert.strictEqual(countMatches(content, /eclusa-update-check\.js/g), 1, 'adds one Eclusa update hook for quoted dotted codex_hooks and remains idempotent');
    assertNoDraftRootKeys(content);
  });
  ...
  test('existing CRLF codex_hooks = true stays single and preserves non-Eclusa hooks', () => {
    writeCodexConfig(codexHome, [
      '[features]',
      'codex_hooks = true',
      'other_feature = true',
    ...
    assert.ok(content.includes('other_feature = true'), 'preserves other feature keys');
    assert.strictEqual(countMatches(content, /echo custom-after-command/g), 1, 'preserves non-Eclusa hook exactly once');
    assert.strictEqual(countMatches(content, /eclusa-update-check\.js/g), 1, 'keeps one Eclusa update hook');
    assertUsesOnlyEol(content, '\r\n');
    assertNoDraftRootKeys(content);
  });
  ...
  test('codex_hooks = true with an inline comment is treated as enabled for hook installation', () => {
    ...
    assert.ok(content.includes('other_feature = true'), 'preserves other feature keys');
    assert.strictEqual(countMatches(content, /eclusa-update-check\.js/g), 1, 'adds the Eclusa update hook once');
    assertNoDraftRootKeys(content);
  });
  ...
  test('mixed-EOL configs use the first newline style for inserted Codex content', () => {
    ...
    assert.ok(content.includes(`# Eclusa Agent Configuration — managed by eclusa installer\n`), 'writes the managed agent block using the first newline style');
    assert.ok(content.includes('# Eclusa Hooks\n[[hooks]]\nevent = "SessionStart"\n'), 'writes the Eclusa hook block using the first newline style');
    ...
    assert.strictEqual(countMatches(content, /eclusa-update-check\.js/g), 1, 'does not duplicate the Eclusa hook block');
    assertNoDraftRootKeys(content);
  });
});
...
describe('Codex uninstall symmetry for hook-enabled configs', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'eclusa-codex-uninstall-'));
    codexHome = path.join(tmpDir, 'codex-home');
  });
  ...
  test('fresh install removes the Eclusa-added codex_hooks feature on uninstall', () => {
    runCodexInstall(codexHome);

    const cleaned = stripEclusaFromCodexConfig(readCodexConfig(codexHome));
    assert.strictEqual(cleaned, null, 'fresh Eclusa-only config strips back to nothing');
  });
  ...
  test('install then uninstall removes [features].codex_hooks while preserving other feature keys, comments, hooks, and CRLF', () => {
    ...
    assert.strictEqual(countMatches(cleaned, /^codex_hooks = true$/gm), 0, 'removes the Eclusa-added codex_hooks key');
    ...
    assert.strictEqual(countMatches(cleaned, /echo custom-after-command/g), 1, 'preserves non-Eclusa hooks');
    assert.strictEqual(countMatches(cleaned, /eclusa-update-check\.js/g), 0, 'removes only the Eclusa update hook');
    assert.strictEqual(countMatches(cleaned, /\[agents\.eclusa-/g), 0, 'removes managed Eclusa agent sections');
  ...
  test('install then uninstall removes dotted features.codex_hooks without creating a [features] table', () => {
    ...
    assert.strictEqual(countMatches(cleaned, /^features\.codex_hooks = true$/gm), 0, 'removes the dotted Eclusa codex_hooks key');
    assert.strictEqual(countMatches(cleaned, /^\[features\]\s*$/gm), 0, 'does not leave behind a [features] table');
    assert.strictEqual(countMatches(cleaned, /echo custom-after-command/g), 1, 'preserves non-Eclusa hooks');
    assert.strictEqual(countMatches(cleaned, /eclusa-update-check\.js/g), 0, 'removes the Eclusa update hook');
  });
  ...
  test('install then uninstall keeps mixed-EOL user content stable while removing Eclusa hook state', () => {
