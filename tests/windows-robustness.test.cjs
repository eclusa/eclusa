 ...
 * 4. findProjectRoot detects .git at same level as .eclusa/
 * 5. @file: handoff present in all workflows that call init
 *
 * Regression tests for: https://github.com/eclusa/eclusa/issues/1343
 */
...
describe('workflow shell robustness', () => {
  ...
  test('glob loops in resume-project.md have existence guard', () => {
    ...
    for (const block of blocks) {
      // Look for `for ... in .eclusa/` glob loops
      const forLoopMatch = block.code.match(/for\s+\w+\s+in\s+\.eclusa\/[^;]+;\s*do/);
      if (forLoopMatch) {
...
describe('@file: handoff in workflows', () => {
  test('all workflows calling eclusa-tools init have @file: handler', () => {
    const workflowFiles = fs.readdirSync(WORKFLOWS_DIR)
      .filter(f => f.endsWith('.md'));
    ...
    for (const wf of workflowFiles) {
      ...
      // Check if this workflow calls eclusa-tools.cjs init
      if (/INIT=\$\(node.*eclusa-tools.*\binit\b/.test(content)) {
    ...
    assert.strictEqual(
      missing.length, 0,
      `Workflows calling eclusa-tools init without @file: handler (large output will be truncated):\n  ${missing.join('\n  ')}`
    );
  });
});
